...
IfcBooleanOperand::~IfcBooleanOperand() {}
shared_ptr<IfcBooleanOperand> IfcBooleanOperand::createObjectFromStepData( const std::wstring& arg, const std::map<int,shared_ptr<IfcPPEntity> >& map )
{
	// Read SELECT TYPE
	...
	else
	{
		...
		if( result_object )
		{
			...
			shared_ptr<IfcBooleanOperand> result_ptr_self = dynamic_pointer_cast<IfcBooleanOperand>( result_ptr );
			if( result_ptr_self )
			{
		...
		strs << "unhandled inline argument: " << arg << " in function IfcBooleanOperand::readStepData" << std::endl;
		throw IfcPPException( strs.str() );
	}
	return shared_ptr<IfcBooleanOperand>();
}
