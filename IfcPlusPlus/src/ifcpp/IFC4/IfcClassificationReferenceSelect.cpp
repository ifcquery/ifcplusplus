...
IfcClassificationReferenceSelect::~IfcClassificationReferenceSelect() {}
shared_ptr<IfcClassificationReferenceSelect> IfcClassificationReferenceSelect::createObjectFromStepData( const std::wstring& arg, const std::map<int,shared_ptr<IfcPPEntity> >& map )
{
	// Read SELECT TYPE
	...
	else
	{
		...
		if( result_object )
		{
			...
			shared_ptr<IfcClassificationReferenceSelect> result_ptr_self = dynamic_pointer_cast<IfcClassificationReferenceSelect>( result_ptr );
			if( result_ptr_self )
			{
		...
		strs << "unhandled inline argument: " << arg << " in function IfcClassificationReferenceSelect::readStepData" << std::endl;
		throw IfcPPException( strs.str() );
	}
	return shared_ptr<IfcClassificationReferenceSelect>();
}
