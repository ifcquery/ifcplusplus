...
IfcColourOrFactor::~IfcColourOrFactor() {}
shared_ptr<IfcColourOrFactor> IfcColourOrFactor::createObjectFromStepData( const std::wstring& arg, const std::map<int,shared_ptr<IfcPPEntity> >& map )
{
	// Read SELECT TYPE
	...
	else
	{
		...
		if( result_object )
		{
			...
			shared_ptr<IfcColourOrFactor> result_ptr_self = dynamic_pointer_cast<IfcColourOrFactor>( result_ptr );
			if( result_ptr_self )
			{
