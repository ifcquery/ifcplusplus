...
IfcCurveOnSurface::~IfcCurveOnSurface() {}
shared_ptr<IfcCurveOnSurface> IfcCurveOnSurface::createObjectFromStepData( const std::wstring& arg, const std::map<int,shared_ptr<IfcPPEntity> >& map )
{
	// Read SELECT TYPE
	...
	else
	{
		...
		if( result_object )
		{
			...
			shared_ptr<IfcCurveOnSurface> result_ptr_self = dynamic_pointer_cast<IfcCurveOnSurface>( result_ptr );
			if( result_ptr_self )
			{
		...
		strs << "unhandled inline argument: " << arg << " in function IfcCurveOnSurface::readStepData" << std::endl;
		throw IfcPPException( strs.str() );
	}
	return shared_ptr<IfcCurveOnSurface>();
}
