...
IfcDefinitionSelect::~IfcDefinitionSelect() {}
shared_ptr<IfcDefinitionSelect> IfcDefinitionSelect::createObjectFromStepData( const std::wstring& arg, const std::map<int,shared_ptr<IfcPPEntity> >& map )
{
	// Read SELECT TYPE
	...
	else
	{
		...
		if( result_object )
		{
			...
			shared_ptr<IfcDefinitionSelect> result_ptr_self = dynamic_pointer_cast<IfcDefinitionSelect>( result_ptr );
			if( result_ptr_self )
			{
