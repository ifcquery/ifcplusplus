...
IfcGeometricSetSelect::~IfcGeometricSetSelect() {}
shared_ptr<IfcGeometricSetSelect> IfcGeometricSetSelect::createObjectFromStepData( const std::wstring& arg, const std::map<int,shared_ptr<IfcPPEntity> >& map )
{
	// Read SELECT TYPE
	...
	else
	{
		...
		if( result_object )
		{
			...
			shared_ptr<IfcGeometricSetSelect> result_ptr_self = dynamic_pointer_cast<IfcGeometricSetSelect>( result_ptr );
			if( result_ptr_self )
			{
