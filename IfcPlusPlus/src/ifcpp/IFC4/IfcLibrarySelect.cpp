...
IfcLibrarySelect::~IfcLibrarySelect() {}
shared_ptr<IfcLibrarySelect> IfcLibrarySelect::createObjectFromStepData( const std::wstring& arg, const std::map<int,shared_ptr<IfcPPEntity> >& map )
{
	// Read SELECT TYPE
	...
	else
	{
		...
		if( result_object )
		{
			...
			shared_ptr<IfcLibrarySelect> result_ptr_self = dynamic_pointer_cast<IfcLibrarySelect>( result_ptr );
			if( result_ptr_self )
			{
		...
		strs << "unhandled inline argument: " << arg << " in function IfcLibrarySelect::readStepData" << std::endl;
		throw IfcPPException( strs.str() );
	}
	return shared_ptr<IfcLibrarySelect>();
}
