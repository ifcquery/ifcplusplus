...
IfcMetricValueSelect::~IfcMetricValueSelect() {}
shared_ptr<IfcMetricValueSelect> IfcMetricValueSelect::createObjectFromStepData( const std::wstring& arg, const std::map<int,shared_ptr<IfcPPEntity> >& map )
{
	// Read SELECT TYPE
	...
	else
	{
		...
		if( result_object )
		{
			...
			shared_ptr<IfcMetricValueSelect> result_ptr_self = dynamic_pointer_cast<IfcMetricValueSelect>( result_ptr );
			if( result_ptr_self )
			{
		...
		strs << "unhandled inline argument: " << arg << " in function IfcMetricValueSelect::readStepData" << std::endl;
		throw IfcPPException( strs.str() );
	}
	return shared_ptr<IfcMetricValueSelect>();
}
