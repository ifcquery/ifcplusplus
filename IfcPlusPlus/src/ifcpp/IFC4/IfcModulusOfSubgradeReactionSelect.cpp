...
IfcModulusOfSubgradeReactionSelect::~IfcModulusOfSubgradeReactionSelect() {}
shared_ptr<IfcModulusOfSubgradeReactionSelect> IfcModulusOfSubgradeReactionSelect::createObjectFromStepData( const std::wstring& arg, const std::map<int,shared_ptr<IfcPPEntity> >& map )
{
	// Read SELECT TYPE
	...
	else
	{
		...
		if( result_object )
		{
			...
			shared_ptr<IfcModulusOfSubgradeReactionSelect> result_ptr_self = dynamic_pointer_cast<IfcModulusOfSubgradeReactionSelect>( result_ptr );
			if( result_ptr_self )
			{
		...
		strs << "unhandled inline argument: " << arg << " in function IfcModulusOfSubgradeReactionSelect::readStepData" << std::endl;
		throw IfcPPException( strs.str() );
	}
	return shared_ptr<IfcModulusOfSubgradeReactionSelect>();
}
