...
IfcModulusOfTranslationalSubgradeReactionSelect::~IfcModulusOfTranslationalSubgradeReactionSelect() {}
shared_ptr<IfcModulusOfTranslationalSubgradeReactionSelect> IfcModulusOfTranslationalSubgradeReactionSelect::createObjectFromStepData( const std::wstring& arg, const std::map<int,shared_ptr<IfcPPEntity> >& map )
{
	// Read SELECT TYPE
	...
	else
	{
		...
		if( result_object )
		{
			...
			shared_ptr<IfcModulusOfTranslationalSubgradeReactionSelect> result_ptr_self = dynamic_pointer_cast<IfcModulusOfTranslationalSubgradeReactionSelect>( result_ptr );
			if( result_ptr_self )
			{
		...
		strs << "unhandled inline argument: " << arg << " in function IfcModulusOfTranslationalSubgradeReactionSelect::readStepData" << std::endl;
		throw IfcPPException( strs.str() );
	}
	return shared_ptr<IfcModulusOfTranslationalSubgradeReactionSelect>();
}
