...
IfcPermeableCoveringProperties::~IfcPermeableCoveringProperties() {}
shared_ptr<IfcPPObject> IfcPermeableCoveringProperties::getDeepCopy()
{
	...
	if( m_OperationType ) { copy_self->m_OperationType = dynamic_pointer_cast<IfcPermeableCoveringOperationEnum>( m_OperationType->getDeepCopy() ); }
	if( m_PanelPosition ) { copy_self->m_PanelPosition = dynamic_pointer_cast<IfcWindowPanelPositionEnum>( m_PanelPosition->getDeepCopy() ); }
	if( m_FrameDepth ) { copy_self->m_FrameDepth = dynamic_pointer_cast<IfcPositiveLengthMeasure>( m_FrameDepth->getDeepCopy() ); }
	if( m_FrameThickness ) { copy_self->m_FrameThickness = dynamic_pointer_cast<IfcPositiveLengthMeasure>( m_FrameThickness->getDeepCopy() ); }
...
void IfcPermeableCoveringProperties::readStepArguments( const std::vector<std::wstring>& args, const std::map<int,shared_ptr<IfcPPEntity> >& map )
{
	...
	if( num_args<9 ){ std::stringstream strserr; strserr << "Wrong parameter count for entity IfcPermeableCoveringProperties, expecting 9, having " << num_args << ". Object id: " << getId() << std::endl; throw IfcPPException( strserr.str().c_str() ); }
	#ifdef _DEBUG
	...
	m_OperationType = IfcPermeableCoveringOperationEnum::createObjectFromStepData( args[4] );
