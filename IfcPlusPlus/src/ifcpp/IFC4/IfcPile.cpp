...
IfcPile::~IfcPile() {}
shared_ptr<IfcPPObject> IfcPile::getDeepCopy()
{
	...
	if( m_PredefinedType ) { copy_self->m_PredefinedType = dynamic_pointer_cast<IfcPileTypeEnum>( m_PredefinedType->getDeepCopy() ); }
	if( m_ConstructionType ) { copy_self->m_ConstructionType = dynamic_pointer_cast<IfcPileConstructionEnum>( m_ConstructionType->getDeepCopy() ); }
	return copy_self;
}
...
void IfcPile::readStepArguments( const std::vector<std::wstring>& args, const std::map<int,shared_ptr<IfcPPEntity> >& map )
{
	...
	if( num_args<10 ){ std::stringstream strserr; strserr << "Wrong parameter count for entity IfcPile, expecting 10, having " << num_args << ". Object id: " << getId() << std::endl; throw IfcPPException( strserr.str().c_str() ); }
	#ifdef _DEBUG
	...
	m_PredefinedType = IfcPileTypeEnum::createObjectFromStepData( args[8] );
	m_ConstructionType = IfcPileConstructionEnum::createObjectFromStepData( args[9] );
}
