...
IfcPipeFitting::IfcPipeFitting( int id ) { m_id = id; }
IfcPipeFitting::~IfcPipeFitting() {}

// method setEntity takes over all attributes from another instance of the class
void IfcPipeFitting::setEntity( shared_ptr<IfcPPEntity> other_entity )
{
...
void IfcPipeFitting::readStepArguments( const std::vector<std::string>& args, const std::map<int,shared_ptr<IfcPPEntity> >& map )
{
	const int num_args = (int)args.size();
	if( num_args<9 ){ std::stringstream strserr; strserr << "Wrong parameter count for entity IfcPipeFitting, expecting 9, having " << num_args << ". Object id: " << getId() << std::endl; throw IfcPPException( strserr.str().c_str() ); }
	#ifdef _DEBUG
	...
	m_PredefinedType = IfcPipeFittingTypeEnum::createObjectFromStepData( args[8] );
}
