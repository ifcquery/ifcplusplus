...
IfcPointOrVertexPoint::~IfcPointOrVertexPoint() {}
shared_ptr<IfcPointOrVertexPoint> IfcPointOrVertexPoint::createObjectFromStepData( const std::wstring& arg, const std::map<int,shared_ptr<IfcPPEntity> >& map )
{
	// Read SELECT TYPE
	...
	else
	{
		...
		if( result_object )
		{
			...
			shared_ptr<IfcPointOrVertexPoint> result_ptr_self = dynamic_pointer_cast<IfcPointOrVertexPoint>( result_ptr );
			if( result_ptr_self )
			{
		...
		strs << "unhandled inline argument: " << arg << " in function IfcPointOrVertexPoint::readStepData" << std::endl;
		throw IfcPPException( strs.str() );
	}
	return shared_ptr<IfcPointOrVertexPoint>();
}
