...
IfcProductRepresentationSelect::~IfcProductRepresentationSelect() {}
shared_ptr<IfcProductRepresentationSelect> IfcProductRepresentationSelect::createObjectFromStepData( const std::wstring& arg, const std::map<int,shared_ptr<IfcPPEntity> >& map )
{
	// Read SELECT TYPE
	...
	else
	{
		...
		if( result_object )
		{
			...
			shared_ptr<IfcProductRepresentationSelect> result_ptr_self = dynamic_pointer_cast<IfcProductRepresentationSelect>( result_ptr );
			if( result_ptr_self )
			{
		...
		strs << "unhandled inline argument: " << arg << " in function IfcProductRepresentationSelect::readStepData" << std::endl;
		throw IfcPPException( strs.str() );
	}
	return shared_ptr<IfcProductRepresentationSelect>();
}
