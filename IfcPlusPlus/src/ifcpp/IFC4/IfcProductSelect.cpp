...
IfcProductSelect::~IfcProductSelect() {}
shared_ptr<IfcProductSelect> IfcProductSelect::createObjectFromStepData( const std::wstring& arg, const std::map<int,shared_ptr<IfcPPEntity> >& map )
{
	// Read SELECT TYPE
	...
	else
	{
		...
		if( result_object )
		{
			...
			shared_ptr<IfcProductSelect> result_ptr_self = dynamic_pointer_cast<IfcProductSelect>( result_ptr );
			if( result_ptr_self )
			{
		...
		strs << "unhandled inline argument: " << arg << " in function IfcProductSelect::readStepData" << std::endl;
		throw IfcPPException( strs.str() );
	}
	return shared_ptr<IfcProductSelect>();
}
