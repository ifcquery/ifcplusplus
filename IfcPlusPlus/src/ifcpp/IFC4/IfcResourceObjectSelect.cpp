...
IfcResourceObjectSelect::~IfcResourceObjectSelect() {}
shared_ptr<IfcResourceObjectSelect> IfcResourceObjectSelect::createObjectFromStepData( const std::wstring& arg, const std::map<int,shared_ptr<IfcPPEntity> >& map )
{
	// Read SELECT TYPE
	...
	else
	{
		...
		if( result_object )
		{
			...
			shared_ptr<IfcResourceObjectSelect> result_ptr_self = dynamic_pointer_cast<IfcResourceObjectSelect>( result_ptr );
			if( result_ptr_self )
			{
		...
		strs << "unhandled inline argument: " << arg << " in function IfcResourceObjectSelect::readStepData" << std::endl;
		throw IfcPPException( strs.str() );
	}
	return shared_ptr<IfcResourceObjectSelect>();
}
