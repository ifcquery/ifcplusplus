...
IfcSimpleValue::~IfcSimpleValue() {}
shared_ptr<IfcSimpleValue> IfcSimpleValue::createObjectFromStepData( const std::wstring& arg, const std::map<int,shared_ptr<IfcPPEntity> >& map )
{
	// Read SELECT TYPE
	...
	else
	{
		...
		if( result_object )
		{
			...
			shared_ptr<IfcSimpleValue> result_ptr_self = dynamic_pointer_cast<IfcSimpleValue>( result_ptr );
			if( result_ptr_self )
			{
		...
		strs << "unhandled inline argument: " << arg << " in function IfcSimpleValue::readStepData" << std::endl;
		throw IfcPPException( strs.str() );
	}
	return shared_ptr<IfcSimpleValue>();
}
