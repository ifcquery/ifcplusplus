...
IfcStyleAssignmentSelect::~IfcStyleAssignmentSelect() {}
shared_ptr<IfcStyleAssignmentSelect> IfcStyleAssignmentSelect::createObjectFromStepData( const std::wstring& arg, const std::map<int,shared_ptr<IfcPPEntity> >& map )
{
	// Read SELECT TYPE
	...
	else
	{
		...
		if( result_object )
		{
			...
			shared_ptr<IfcStyleAssignmentSelect> result_ptr_self = dynamic_pointer_cast<IfcStyleAssignmentSelect>( result_ptr );
			if( result_ptr_self )
			{
		...
		strs << "unhandled inline argument: " << arg << " in function IfcStyleAssignmentSelect::readStepData" << std::endl;
		throw IfcPPException( strs.str() );
	}
	return shared_ptr<IfcStyleAssignmentSelect>();
}
