...
IfcTextFontSelect::~IfcTextFontSelect() {}
shared_ptr<IfcTextFontSelect> IfcTextFontSelect::createObjectFromStepData( const std::wstring& arg, const std::map<int,shared_ptr<IfcPPEntity> >& map )
{
	// Read SELECT TYPE
	...
	else
	{
		...
		if( result_object )
		{
			...
			shared_ptr<IfcTextFontSelect> result_ptr_self = dynamic_pointer_cast<IfcTextFontSelect>( result_ptr );
			if( result_ptr_self )
			{
		...
		strs << "unhandled inline argument: " << arg << " in function IfcTextFontSelect::readStepData" << std::endl;
		throw IfcPPException( strs.str() );
	}
	return shared_ptr<IfcTextFontSelect>();
}
