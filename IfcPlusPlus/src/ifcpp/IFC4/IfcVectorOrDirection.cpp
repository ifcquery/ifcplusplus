...
IfcVectorOrDirection::~IfcVectorOrDirection() {}
shared_ptr<IfcVectorOrDirection> IfcVectorOrDirection::createObjectFromStepData( const std::wstring& arg, const std::map<int,shared_ptr<IfcPPEntity> >& map )
{
	// Read SELECT TYPE
	...
	else
	{
		...
		if( result_object )
		{
			...
			shared_ptr<IfcVectorOrDirection> result_ptr_self = dynamic_pointer_cast<IfcVectorOrDirection>( result_ptr );
			if( result_ptr_self )
			{
		...
		strs << "unhandled inline argument: " << arg << " in function IfcVectorOrDirection::readStepData" << std::endl;
		throw IfcPPException( strs.str() );
	}
	return shared_ptr<IfcVectorOrDirection>();
}
