...
void IfcActor::readStepArguments( const std::vector<std::wstring>& args, const std::map<int,shared_ptr<BuildingEntity> >& map )
{
	...
	if( num_args != 6 ){ std::stringstream err; err << "Wrong parameter count for entity IfcActor, expecting 6, having " << num_args << ". Entity ID: " << m_entity_id << std::endl; throw BuildingException( err.str() ); }
...
void IfcActor::getAttributes( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes ) const
{
	...
	vec_attributes.emplace_back( "TheActor", m_TheActor );
}
void IfcActor::getAttributesInverse( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes_inverse ) const
{
	...
	if( !m_IsActingUpon_inverse.empty() )
	{
		...
		for(const auto & i : m_IsActingUpon_inverse)
		{
			if( !i.expired() )
			{
				IsActingUpon_inverse_vec_obj->m_vec.push_back( shared_ptr<IfcRelAssignsToActor>( i ) );
			}
		}
		vec_attributes_inverse.emplace_back( "IsActingUpon_inverse", IsActingUpon_inverse_vec_obj );
	}
}
