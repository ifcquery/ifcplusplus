...
shared_ptr<BuildingObject> IfcAlignment2DHorizontal::getDeepCopy( BuildingCopyOptions& options )
{
	...
	for( size_t ii=0; ii<m_Segments.size(); ++ii )
	{
		...
		if( item_ii )
		{
			copy_self->m_Segments.push_back( dynamic_pointer_cast<IfcAlignment2DHorizontalSegment>(item_ii->getDeepCopy(options) ) );
		}
	}
	return copy_self;
}
...
void IfcAlignment2DHorizontal::getStepParameter( std::stringstream& stream, bool ) const { stream << "#" << m_entity_id; }
...
void IfcAlignment2DHorizontal::getAttributes( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes )
{
...
void IfcAlignment2DHorizontal::getAttributesInverse( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes_inverse )
{
	IfcGeometricRepresentationItem::getAttributesInverse( vec_attributes_inverse );
	if( m_ToAlignmentCurve_inverse.size() > 0 )
	{
		...
		for( size_t i=0; i<m_ToAlignmentCurve_inverse.size(); ++i )
		{
			...
			{
				ToAlignmentCurve_inverse_vec_obj->m_vec.push_back( shared_ptr<IfcAlignmentCurve>( m_ToAlignmentCurve_inverse[i] ) );
			}
		}
		vec_attributes_inverse.push_back( std::make_pair( "ToAlignmentCurve_inverse", ToAlignmentCurve_inverse_vec_obj ) );
	}
}
