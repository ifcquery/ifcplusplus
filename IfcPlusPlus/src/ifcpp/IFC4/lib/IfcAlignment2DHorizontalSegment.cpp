...
void IfcAlignment2DHorizontalSegment::getStepParameter( std::stringstream& stream, bool ) const { stream << "#" << m_entity_id; }
...
void IfcAlignment2DHorizontalSegment::getAttributes( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes )
{
	IfcAlignment2DSegment::getAttributes( vec_attributes );
	vec_attributes.push_back( std::make_pair( "CurveGeometry", m_CurveGeometry ) );
}
void IfcAlignment2DHorizontalSegment::getAttributesInverse( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes_inverse )
{
	IfcAlignment2DSegment::getAttributesInverse( vec_attributes_inverse );
	if( m_ToHorizontal_inverse.size() > 0 )
	{
		...
		for( size_t i=0; i<m_ToHorizontal_inverse.size(); ++i )
		{
			...
			{
				ToHorizontal_inverse_vec_obj->m_vec.push_back( shared_ptr<IfcAlignment2DHorizontal>( m_ToHorizontal_inverse[i] ) );
			}
		}
		vec_attributes_inverse.push_back( std::make_pair( "ToHorizontal_inverse", ToHorizontal_inverse_vec_obj ) );
	}
}
