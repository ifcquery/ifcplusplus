...
shared_ptr<BuildingObject> IfcAlignment2DVertical::getDeepCopy( BuildingCopyOptions& options )
{
	...
	for( size_t ii=0; ii<m_Segments.size(); ++ii )
	{
		...
		if( item_ii )
		{
			copy_self->m_Segments.push_back( dynamic_pointer_cast<IfcAlignment2DVerticalSegment>(item_ii->getDeepCopy(options) ) );
		}
	}
	return copy_self;
}
...
void IfcAlignment2DVertical::getStepParameter( std::stringstream& stream, bool ) const { stream << "#" << m_entity_id; }
...
void IfcAlignment2DVertical::getAttributes( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes )
{
...
void IfcAlignment2DVertical::getAttributesInverse( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes_inverse )
{
