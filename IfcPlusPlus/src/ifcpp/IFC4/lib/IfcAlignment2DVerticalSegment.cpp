...
void IfcAlignment2DVerticalSegment::getStepParameter( std::stringstream& stream, bool ) const { stream << "#" << m_entity_id; }
...
void IfcAlignment2DVerticalSegment::getAttributes( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes )
{
	IfcAlignment2DSegment::getAttributes( vec_attributes );
	vec_attributes.push_back( std::make_pair( "StartDistAlong", m_StartDistAlong ) );
	vec_attributes.push_back( std::make_pair( "HorizontalLength", m_HorizontalLength ) );
	vec_attributes.push_back( std::make_pair( "StartHeight", m_StartHeight ) );
	vec_attributes.push_back( std::make_pair( "StartGradient", m_StartGradient ) );
}
void IfcAlignment2DVerticalSegment::getAttributesInverse( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes_inverse )
{
	IfcAlignment2DSegment::getAttributesInverse( vec_attributes_inverse );
	if( m_ToVertical_inverse.size() > 0 )
	{
		...
		for( size_t i=0; i<m_ToVertical_inverse.size(); ++i )
		{
			...
			{
				ToVertical_inverse_vec_obj->m_vec.push_back( shared_ptr<IfcAlignment2DVertical>( m_ToVertical_inverse[i] ) );
			}
		}
		vec_attributes_inverse.push_back( std::make_pair( "ToVertical_inverse", ToVertical_inverse_vec_obj ) );
	}
}
