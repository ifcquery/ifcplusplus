...
void IfcAlignmentCurve::getStepParameter( std::stringstream& stream, bool ) const { stream << "#" << m_entity_id; }
...
void IfcAlignmentCurve::getAttributes( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes )
{
	IfcBoundedCurve::getAttributes( vec_attributes );
	vec_attributes.push_back( std::make_pair( "Horizontal", m_Horizontal ) );
	vec_attributes.push_back( std::make_pair( "Vertical", m_Vertical ) );
	vec_attributes.push_back( std::make_pair( "Tag", m_Tag ) );
}
void IfcAlignmentCurve::getAttributesInverse( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes_inverse )
{
...
void IfcAlignmentCurve::setInverseCounterparts( shared_ptr<BuildingEntity> ptr_self_entity )
{
	...
	if( m_Horizontal )
	{
		m_Horizontal->m_ToAlignmentCurve_inverse.push_back( ptr_self );
	}
	if( m_Vertical )
	{
		m_Vertical->m_ToAlignmentCurve_inverse.push_back( ptr_self );
	}
}
