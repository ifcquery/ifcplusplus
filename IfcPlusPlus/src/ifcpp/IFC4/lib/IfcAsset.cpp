...
void IfcAsset::readStepArguments( const std::vector<std::wstring>& args, const std::map<int,shared_ptr<BuildingEntity> >& map )
{
	...
	if( num_args != 14 ){ std::stringstream err; err << "Wrong parameter count for entity IfcAsset, expecting 14, having " << num_args << ". Entity ID: " << m_entity_id << std::endl; throw BuildingException( err.str() ); }
...
void IfcAsset::getAttributes( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes ) const
{
	...
	vec_attributes.emplace_back( "OriginalValue", m_OriginalValue );
	vec_attributes.emplace_back( "CurrentValue", m_CurrentValue );
	vec_attributes.emplace_back( "TotalReplacementCost", m_TotalReplacementCost );
	vec_attributes.emplace_back( "Owner", m_Owner );
	vec_attributes.emplace_back( "User", m_User );
	vec_attributes.emplace_back( "ResponsiblePerson", m_ResponsiblePerson );
	vec_attributes.emplace_back( "IncorporationDate", m_IncorporationDate );
	vec_attributes.emplace_back( "DepreciatedValue", m_DepreciatedValue );
}
