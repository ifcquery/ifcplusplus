...
IfcBoundaryFaceCondition::IfcBoundaryFaceCondition() = default;
...
void IfcBoundaryFaceCondition::readStepArguments( const std::vector<std::wstring>& args, const std::map<int,shared_ptr<BuildingEntity> >& map )
{
	...
	if( num_args != 4 ){ std::stringstream err; err << "Wrong parameter count for entity IfcBoundaryFaceCondition, expecting 4, having " << num_args << ". Entity ID: " << m_entity_id << std::endl; throw BuildingException( err.str() ); }
...
void IfcBoundaryFaceCondition::getAttributes( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes ) const
{
	...
	vec_attributes.emplace_back( "TranslationalStiffnessByAreaX", m_TranslationalStiffnessByAreaX );
	vec_attributes.emplace_back( "TranslationalStiffnessByAreaY", m_TranslationalStiffnessByAreaY );
	vec_attributes.emplace_back( "TranslationalStiffnessByAreaZ", m_TranslationalStiffnessByAreaZ );
}
...
void IfcBoundaryFaceCondition::setInverseCounterparts( shared_ptr<BuildingEntity> ptr_self_entity )
{
