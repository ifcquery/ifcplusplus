...
// ENTITY IfcBridge 
IfcBridge::IfcBridge( int id ) { m_entity_id = id; }
shared_ptr<BuildingObject> IfcBridge::getDeepCopy( BuildingCopyOptions& options )
{
...
void IfcBridge::getStepParameter( std::stringstream& stream, bool /*is_select_type*/ ) const { stream << "#" << m_entity_id; }
const std::wstring IfcBridge::toString() const { return L"IfcBridge"; }
void IfcBridge::readStepArguments( const std::vector<std::wstring>& args, const std::map<int,shared_ptr<BuildingEntity> >& map )
{
	const size_t num_args = args.size();
	if( num_args != 10 ){ std::stringstream err; err << "Wrong parameter count for entity IfcBridge, expecting 10, having " << num_args << ". Entity ID: " << m_entity_id << std::endl; throw BuildingException( err.str().c_str() ); }
	...
	m_PredefinedType = IfcBridgeTypeEnum::createObjectFromSTEP( args[9], map );
}
void IfcBridge::getAttributes( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes ) const
{
	IfcFacility::getAttributes( vec_attributes );
	vec_attributes.emplace_back( std::make_pair( "PredefinedType", m_PredefinedType ) );
}
void IfcBridge::getAttributesInverse( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes_inverse ) const
{
	IfcFacility::getAttributesInverse( vec_attributes_inverse );
}
void IfcBridge::setInverseCounterparts( shared_ptr<BuildingEntity> ptr_self_entity )
{
	IfcFacility::setInverseCounterparts( ptr_self_entity );
}
void IfcBridge::unlinkFromInverseCounterparts()
{
