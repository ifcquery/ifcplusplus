...
// ENTITY IfcBuilding 
IfcBuilding::IfcBuilding() = default;
...
void IfcBuilding::readStepArguments( const std::vector<std::wstring>& args, const std::map<int,shared_ptr<BuildingEntity> >& map )
{
	...
	if( num_args != 12 ){ std::stringstream err; err << "Wrong parameter count for entity IfcBuilding, expecting 12, having " << num_args << ". Entity ID: " << m_entity_id << std::endl; throw BuildingException( err.str() ); }
...
void IfcBuilding::getAttributes( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes ) const
{
	...
	vec_attributes.emplace_back( "ElevationOfRefHeight", m_ElevationOfRefHeight );
	vec_attributes.emplace_back( "ElevationOfTerrain", m_ElevationOfTerrain );
	vec_attributes.emplace_back( "BuildingAddress", m_BuildingAddress );
}
