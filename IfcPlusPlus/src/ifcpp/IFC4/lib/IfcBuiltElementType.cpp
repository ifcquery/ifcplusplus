...
IfcBuiltElementType::IfcBuiltElementType( int id ) { m_entity_id = id; }
shared_ptr<BuildingObject> IfcBuiltElementType::getDeepCopy( BuildingCopyOptions& options )
{
	shared_ptr<IfcBuiltElementType> copy_self( new IfcBuiltElementType() );
	if( m_GlobalId )
	{
...
void IfcBuiltElementType::getStepLine( std::stringstream& stream ) const
{
	stream << "#" << m_entity_id << "= IFCBUILTELEMENTTYPE" << "(";
...
void IfcBuiltElementType::getStepParameter( std::stringstream& stream, bool /*is_select_type*/ ) const { stream << "#" << m_entity_id; }
const std::wstring IfcBuiltElementType::toString() const { return L"IfcBuiltElementType"; }
void IfcBuiltElementType::readStepArguments( const std::vector<std::wstring>& args, const std::map<int,shared_ptr<BuildingEntity> >& map )
{
	const size_t num_args = args.size();
	if( num_args != 9 ){ std::stringstream err; err << "Wrong parameter count for entity IfcBuiltElementType, expecting 9, having " << num_args << ". Entity ID: " << m_entity_id << std::endl; throw BuildingException( err.str().c_str() ); }
...
void IfcBuiltElementType::getAttributes( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes ) const
{
	IfcElementType::getAttributes( vec_attributes );
}
void IfcBuiltElementType::getAttributesInverse( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes_inverse ) const
{
	IfcElementType::getAttributesInverse( vec_attributes_inverse );
}
void IfcBuiltElementType::setInverseCounterparts( shared_ptr<BuildingEntity> ptr_self_entity )
{
	IfcElementType::setInverseCounterparts( ptr_self_entity );
}
void IfcBuiltElementType::unlinkFromInverseCounterparts()
{
