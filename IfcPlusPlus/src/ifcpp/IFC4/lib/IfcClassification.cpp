...
IfcClassification::IfcClassification() = default;
...
shared_ptr<BuildingObject> IfcClassification::getDeepCopy( BuildingCopyOptions& options )
{
	...
	for(auto item_ii : m_ReferenceTokens)
	{
			if( item_ii )
		{
...
void IfcClassification::readStepArguments( const std::vector<std::wstring>& args, const std::map<int,shared_ptr<BuildingEntity> >& map )
{
	...
	if( num_args != 7 ){ std::stringstream err; err << "Wrong parameter count for entity IfcClassification, expecting 7, having " << num_args << ". Entity ID: " << m_entity_id << std::endl; throw BuildingException( err.str() ); }
...
void IfcClassification::getAttributes( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes ) const
{
	...
	vec_attributes.emplace_back( "Source", m_Source );
	vec_attributes.emplace_back( "Edition", m_Edition );
	vec_attributes.emplace_back( "EditionDate", m_EditionDate );
	...
	if( !m_ReferenceTokens.empty() )
	{
		...
		vec_attributes.emplace_back( "ReferenceTokens", ReferenceTokens_vec_object );
	}
}
void IfcClassification::getAttributesInverse( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes_inverse ) const
{
	...
	if( !m_ClassificationForObjects_inverse.empty() )
	{
		...
		for(const auto & i : m_ClassificationForObjects_inverse)
		{
			if( !i.expired() )
			{
				ClassificationForObjects_inverse_vec_obj->m_vec.push_back( shared_ptr<IfcRelAssociatesClassification>( i ) );
			}
		}
		vec_attributes_inverse.emplace_back( "ClassificationForObjects_inverse", ClassificationForObjects_inverse_vec_obj );
	}
