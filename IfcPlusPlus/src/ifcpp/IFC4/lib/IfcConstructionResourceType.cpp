...
IfcConstructionResourceType::IfcConstructionResourceType() = default;
...
void IfcConstructionResourceType::readStepArguments( const std::vector<std::wstring>& args, const std::map<int,shared_ptr<BuildingEntity> >& map )
{
	...
	if( num_args != 11 ){ std::stringstream err; err << "Wrong parameter count for entity IfcConstructionResourceType, expecting 11, having " << num_args << ". Entity ID: " << m_entity_id << std::endl; throw BuildingException( err.str() ); }
...
void IfcConstructionResourceType::getAttributes( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes ) const
{
	...
	if( !m_BaseCosts.empty() )
	{
		...
		vec_attributes.emplace_back( "BaseCosts", BaseCosts_vec_object );
	}
	vec_attributes.emplace_back( "BaseQuantity", m_BaseQuantity );
}
