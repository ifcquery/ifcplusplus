...
void IfcContext::readStepArguments( const std::vector<std::wstring>& args, const std::map<int,shared_ptr<BuildingEntity> >& map )
{
	...
	if( num_args != 9 ){ std::stringstream err; err << "Wrong parameter count for entity IfcContext, expecting 9, having " << num_args << ". Entity ID: " << m_entity_id << std::endl; throw BuildingException( err.str() ); }
...
void IfcContext::getAttributes( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes ) const
{
	...
	vec_attributes.emplace_back( "Phase", m_Phase );
	if( !m_RepresentationContexts.empty() )
	{
		...
		vec_attributes.emplace_back( "RepresentationContexts", RepresentationContexts_vec_object );
	}
	vec_attributes.emplace_back( "UnitsInContext", m_UnitsInContext );
}
void IfcContext::getAttributesInverse( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes_inverse ) const
{
	...
	if( !m_Declares_inverse.empty() )
	{
		...
		for(const auto & i : m_Declares_inverse)
		{
			if( !i.expired() )
			{
				Declares_inverse_vec_obj->m_vec.push_back( shared_ptr<IfcRelDeclares>( i ) );
			}
		}
