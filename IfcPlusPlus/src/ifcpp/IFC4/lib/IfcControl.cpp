...
void IfcControl::readStepArguments( const std::vector<std::wstring>& args, const std::map<int,shared_ptr<BuildingEntity> >& map )
{
	...
	if( num_args != 6 ){ std::stringstream err; err << "Wrong parameter count for entity IfcControl, expecting 6, having " << num_args << ". Entity ID: " << m_entity_id << std::endl; throw BuildingException( err.str() ); }
...
void IfcControl::getAttributesInverse( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes_inverse ) const
{
	...
	if( !m_Controls_inverse.empty() )
	{
		...
		for(const auto & i : m_Controls_inverse)
		{
			if( !i.expired() )
			{
				Controls_inverse_vec_obj->m_vec.push_back( shared_ptr<IfcRelAssignsToControl>( i ) );
			}
		}
		vec_attributes_inverse.emplace_back( "Controls_inverse", Controls_inverse_vec_obj );
	}
}
