...
IfcConveyorSegment::IfcConveyorSegment( int id ) { m_entity_id = id; }
shared_ptr<BuildingObject> IfcConveyorSegment::getDeepCopy( BuildingCopyOptions& options )
{
	shared_ptr<IfcConveyorSegment> copy_self( new IfcConveyorSegment() );
	if( m_GlobalId )
	{
...
void IfcConveyorSegment::getStepLine( std::stringstream& stream ) const
{
	stream << "#" << m_entity_id << "= IFCCONVEYORSEGMENT" << "(";
...
void IfcConveyorSegment::getStepParameter( std::stringstream& stream, bool /*is_select_type*/ ) const { stream << "#" << m_entity_id; }
const std::wstring IfcConveyorSegment::toString() const { return L"IfcConveyorSegment"; }
void IfcConveyorSegment::readStepArguments( const std::vector<std::wstring>& args, const std::map<int,shared_ptr<BuildingEntity> >& map )
{
	const size_t num_args = args.size();
	if( num_args != 9 ){ std::stringstream err; err << "Wrong parameter count for entity IfcConveyorSegment, expecting 9, having " << num_args << ". Entity ID: " << m_entity_id << std::endl; throw BuildingException( err.str().c_str() ); }
	...
	m_PredefinedType = IfcConveyorSegmentTypeEnum::createObjectFromSTEP( args[8], map );
}
void IfcConveyorSegment::getAttributes( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes ) const
{
	IfcFlowSegment::getAttributes( vec_attributes );
	vec_attributes.emplace_back( std::make_pair( "PredefinedType", m_PredefinedType ) );
}
void IfcConveyorSegment::getAttributesInverse( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes_inverse ) const
{
	IfcFlowSegment::getAttributesInverse( vec_attributes_inverse );
}
void IfcConveyorSegment::setInverseCounterparts( shared_ptr<BuildingEntity> ptr_self_entity )
{
	IfcFlowSegment::setInverseCounterparts( ptr_self_entity );
}
void IfcConveyorSegment::unlinkFromInverseCounterparts()
{
