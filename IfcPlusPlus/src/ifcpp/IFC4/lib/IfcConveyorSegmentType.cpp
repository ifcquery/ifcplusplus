...
IfcConveyorSegmentType::IfcConveyorSegmentType( int id ) { m_entity_id = id; }
shared_ptr<BuildingObject> IfcConveyorSegmentType::getDeepCopy( BuildingCopyOptions& options )
{
	shared_ptr<IfcConveyorSegmentType> copy_self( new IfcConveyorSegmentType() );
	if( m_GlobalId )
	{
	...
	if( m_PredefinedType ) { copy_self->m_PredefinedType = dynamic_pointer_cast<IfcConveyorSegmentTypeEnum>( m_PredefinedType->getDeepCopy(options) ); }
	return copy_self;
}
void IfcConveyorSegmentType::getStepLine( std::stringstream& stream ) const
{
	stream << "#" << m_entity_id << "= IFCCONVEYORSEGMENTTYPE" << "(";
...
void IfcConveyorSegmentType::getStepParameter( std::stringstream& stream, bool /*is_select_type*/ ) const { stream << "#" << m_entity_id; }
const std::wstring IfcConveyorSegmentType::toString() const { return L"IfcConveyorSegmentType"; }
void IfcConveyorSegmentType::readStepArguments( const std::vector<std::wstring>& args, const std::map<int,shared_ptr<BuildingEntity> >& map )
{
	const size_t num_args = args.size();
	if( num_args != 10 ){ std::stringstream err; err << "Wrong parameter count for entity IfcConveyorSegmentType, expecting 10, having " << num_args << ". Entity ID: " << m_entity_id << std::endl; throw BuildingException( err.str().c_str() ); }
	...
	m_PredefinedType = IfcConveyorSegmentTypeEnum::createObjectFromSTEP( args[9], map );
}
void IfcConveyorSegmentType::getAttributes( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes ) const
{
	IfcFlowSegmentType::getAttributes( vec_attributes );
	vec_attributes.emplace_back( std::make_pair( "PredefinedType", m_PredefinedType ) );
}
void IfcConveyorSegmentType::getAttributesInverse( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes_inverse ) const
{
	IfcFlowSegmentType::getAttributesInverse( vec_attributes_inverse );
}
void IfcConveyorSegmentType::setInverseCounterparts( shared_ptr<BuildingEntity> ptr_self_entity )
{
	IfcFlowSegmentType::setInverseCounterparts( ptr_self_entity );
}
void IfcConveyorSegmentType::unlinkFromInverseCounterparts()
{
