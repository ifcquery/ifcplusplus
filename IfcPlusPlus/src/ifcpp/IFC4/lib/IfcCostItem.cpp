...
// ENTITY IfcCostItem 
IfcCostItem::IfcCostItem() = default;
...
shared_ptr<BuildingObject> IfcCostItem::getDeepCopy( BuildingCopyOptions& options )
{
	...
	for(auto item_ii : m_CostQuantities)
	{
			if( item_ii )
		{
...
void IfcCostItem::readStepArguments( const std::vector<std::wstring>& args, const std::map<int,shared_ptr<BuildingEntity> >& map )
{
	...
	if( num_args != 9 ){ std::stringstream err; err << "Wrong parameter count for entity IfcCostItem, expecting 9, having " << num_args << ". Entity ID: " << m_entity_id << std::endl; throw BuildingException( err.str() ); }
...
void IfcCostItem::getAttributes( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes ) const
{
	...
	if( !m_CostValues.empty() )
	{
		...
		vec_attributes.emplace_back( "CostValues", CostValues_vec_object );
	}
	if( !m_CostQuantities.empty() )
	{
		...
		vec_attributes.emplace_back( "CostQuantities", CostQuantities_vec_object );
	}
}
