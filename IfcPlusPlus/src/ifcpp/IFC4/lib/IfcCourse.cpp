...
// ENTITY IfcCourse 
IfcCourse::IfcCourse( int id ) { m_entity_id = id; }
shared_ptr<BuildingObject> IfcCourse::getDeepCopy( BuildingCopyOptions& options )
{
	shared_ptr<IfcCourse> copy_self( new IfcCourse() );
	if( m_GlobalId )
	{
...
void IfcCourse::getStepLine( std::stringstream& stream ) const
{
	stream << "#" << m_entity_id << "= IFCCOURSE" << "(";
...
void IfcCourse::getStepParameter( std::stringstream& stream, bool /*is_select_type*/ ) const { stream << "#" << m_entity_id; }
const std::wstring IfcCourse::toString() const { return L"IfcCourse"; }
void IfcCourse::readStepArguments( const std::vector<std::wstring>& args, const std::map<int,shared_ptr<BuildingEntity> >& map )
{
	const size_t num_args = args.size();
	if( num_args != 9 ){ std::stringstream err; err << "Wrong parameter count for entity IfcCourse, expecting 9, having " << num_args << ". Entity ID: " << m_entity_id << std::endl; throw BuildingException( err.str().c_str() ); }
	...
	m_PredefinedType = IfcCourseTypeEnum::createObjectFromSTEP( args[8], map );
}
void IfcCourse::getAttributes( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes ) const
{
	IfcBuiltElement::getAttributes( vec_attributes );
	vec_attributes.emplace_back( std::make_pair( "PredefinedType", m_PredefinedType ) );
}
void IfcCourse::getAttributesInverse( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes_inverse ) const
{
	IfcBuiltElement::getAttributesInverse( vec_attributes_inverse );
}
void IfcCourse::setInverseCounterparts( shared_ptr<BuildingEntity> ptr_self_entity )
{
	IfcBuiltElement::setInverseCounterparts( ptr_self_entity );
}
void IfcCourse::unlinkFromInverseCounterparts()
{
