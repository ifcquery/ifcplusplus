...
// ENTITY IfcCovering 
IfcCovering::IfcCovering() = default;
...
void IfcCovering::readStepArguments( const std::vector<std::wstring>& args, const std::map<int,shared_ptr<BuildingEntity> >& map )
{
	...
	if( num_args != 9 ){ std::stringstream err; err << "Wrong parameter count for entity IfcCovering, expecting 9, having " << num_args << ". Entity ID: " << m_entity_id << std::endl; throw BuildingException( err.str() ); }
...
void IfcCovering::getAttributesInverse( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes_inverse ) const
{
	...
	if( !m_CoversSpaces_inverse.empty() )
	{
		...
		for(const auto & i : m_CoversSpaces_inverse)
		{
			if( !i.expired() )
			{
				CoversSpaces_inverse_vec_obj->m_vec.push_back( shared_ptr<IfcRelCoversSpaces>( i ) );
			}
		}
		vec_attributes_inverse.emplace_back( "CoversSpaces_inverse", CoversSpaces_inverse_vec_obj );
	}
	if( !m_CoversElements_inverse.empty() )
	{
		...
		for(const auto & i : m_CoversElements_inverse)
		{
			if( !i.expired() )
			{
				CoversElements_inverse_vec_obj->m_vec.push_back( shared_ptr<IfcRelCoversBldgElements>( i ) );
			}
		}
		vec_attributes_inverse.emplace_back( "CoversElements_inverse", CoversElements_inverse_vec_obj );
	}
}
