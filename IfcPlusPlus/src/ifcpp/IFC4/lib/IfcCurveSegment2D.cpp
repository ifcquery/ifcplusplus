...
void IfcCurveSegment2D::getStepParameter( std::stringstream& stream, bool ) const { stream << "#" << m_entity_id; }
...
void IfcCurveSegment2D::getAttributes( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes )
{
	IfcBoundedCurve::getAttributes( vec_attributes );
	vec_attributes.push_back( std::make_pair( "StartPoint", m_StartPoint ) );
	vec_attributes.push_back( std::make_pair( "StartDirection", m_StartDirection ) );
	vec_attributes.push_back( std::make_pair( "SegmentLength", m_SegmentLength ) );
}
void IfcCurveSegment2D::getAttributesInverse( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes_inverse )
{
