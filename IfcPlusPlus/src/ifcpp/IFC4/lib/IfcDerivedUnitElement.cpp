...
IfcDerivedUnitElement::IfcDerivedUnitElement() = default;
...
void IfcDerivedUnitElement::readStepArguments( const std::vector<std::wstring>& args, const std::map<int,shared_ptr<BuildingEntity> >& map )
{
	...
	if( num_args != 2 ){ std::stringstream err; err << "Wrong parameter count for entity IfcDerivedUnitElement, expecting 2, having " << num_args << ". Entity ID: " << m_entity_id << std::endl; throw BuildingException( err.str() ); }
...
void IfcDerivedUnitElement::getAttributes( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes ) const
{
	...
	vec_attributes.emplace_back( "Exponent", std::make_shared<IntegerAttribute>( m_Exponent ) );
}
...
void IfcDerivedUnitElement::setInverseCounterparts( shared_ptr<BuildingEntity>  /*ptr_self*/)
{
}
