...
IfcDistributionElement::IfcDistributionElement() = default;
...
void IfcDistributionElement::readStepArguments( const std::vector<std::wstring>& args, const std::map<int,shared_ptr<BuildingEntity> >& map )
{
	...
	if( num_args != 8 ){ std::stringstream err; err << "Wrong parameter count for entity IfcDistributionElement, expecting 8, having " << num_args << ". Entity ID: " << m_entity_id << std::endl; throw BuildingException( err.str() ); }
...
void IfcDistributionElement::getAttributesInverse( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes_inverse ) const
{
	...
	if( !m_HasPorts_inverse.empty() )
	{
		...
		for(const auto & i : m_HasPorts_inverse)
		{
			if( !i.expired() )
			{
				HasPorts_inverse_vec_obj->m_vec.push_back( shared_ptr<IfcRelConnectsPortToElement>( i ) );
			}
		}
		vec_attributes_inverse.emplace_back( "HasPorts_inverse", HasPorts_inverse_vec_obj );
	}
}
