...
IfcDistributionPort::IfcDistributionPort() = default;
...
void IfcDistributionPort::readStepArguments( const std::vector<std::wstring>& args, const std::map<int,shared_ptr<BuildingEntity> >& map )
{
	...
	if( num_args != 10 ){ std::stringstream err; err << "Wrong parameter count for entity IfcDistributionPort, expecting 10, having " << num_args << ". Entity ID: " << m_entity_id << std::endl; throw BuildingException( err.str() ); }
...
void IfcDistributionPort::getAttributes( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes ) const
{
	...
	vec_attributes.emplace_back( "FlowDirection", m_FlowDirection );
	vec_attributes.emplace_back( "PredefinedType", m_PredefinedType );
	vec_attributes.emplace_back( "SystemType", m_SystemType );
}
