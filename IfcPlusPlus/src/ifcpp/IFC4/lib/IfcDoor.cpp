...
void IfcDoor::readStepArguments( const std::vector<std::wstring>& args, const std::map<int,shared_ptr<BuildingEntity> >& map )
{
	...
	if( num_args != 13 ){ std::stringstream err; err << "Wrong parameter count for entity IfcDoor, expecting 13, having " << num_args << ". Entity ID: " << m_entity_id << std::endl; throw BuildingException( err.str() ); }
...
void IfcDoor::getAttributes( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes ) const
{
	...
	vec_attributes.emplace_back( "OverallHeight", m_OverallHeight );
	vec_attributes.emplace_back( "OverallWidth", m_OverallWidth );
	vec_attributes.emplace_back( "PredefinedType", m_PredefinedType );
	vec_attributes.emplace_back( "OperationType", m_OperationType );
	vec_attributes.emplace_back( "UserDefinedOperationType", m_UserDefinedOperationType );
}
