...
IfcDoorPanelProperties::IfcDoorPanelProperties() = default;
...
void IfcDoorPanelProperties::readStepArguments( const std::vector<std::wstring>& args, const std::map<int,shared_ptr<BuildingEntity> >& map )
{
	...
	if( num_args != 9 ){ std::stringstream err; err << "Wrong parameter count for entity IfcDoorPanelProperties, expecting 9, having " << num_args << ". Entity ID: " << m_entity_id << std::endl; throw BuildingException( err.str() ); }
...
void IfcDoorPanelProperties::getAttributes( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes ) const
{
	...
	vec_attributes.emplace_back( "PanelDepth", m_PanelDepth );
	vec_attributes.emplace_back( "PanelOperation", m_PanelOperation );
	vec_attributes.emplace_back( "PanelWidth", m_PanelWidth );
