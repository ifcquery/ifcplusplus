...
IfcEarthworksCut::IfcEarthworksCut( int id ) { m_entity_id = id; }
shared_ptr<BuildingObject> IfcEarthworksCut::getDeepCopy( BuildingCopyOptions& options )
{
	shared_ptr<IfcEarthworksCut> copy_self( new IfcEarthworksCut() );
	if( m_GlobalId )
	{
	...
	if( m_PredefinedType ) { copy_self->m_PredefinedType = dynamic_pointer_cast<IfcEarthworksCutTypeEnum>( m_PredefinedType->getDeepCopy(options) ); }
	return copy_self;
}
void IfcEarthworksCut::getStepLine( std::stringstream& stream ) const
{
	stream << "#" << m_entity_id << "= IFCEARTHWORKSCUT" << "(";
...
void IfcEarthworksCut::getStepParameter( std::stringstream& stream, bool /*is_select_type*/ ) const { stream << "#" << m_entity_id; }
const std::wstring IfcEarthworksCut::toString() const { return L"IfcEarthworksCut"; }
void IfcEarthworksCut::readStepArguments( const std::vector<std::wstring>& args, const std::map<int,shared_ptr<BuildingEntity> >& map )
{
	const size_t num_args = args.size();
	if( num_args != 9 ){ std::stringstream err; err << "Wrong parameter count for entity IfcEarthworksCut, expecting 9, having " << num_args << ". Entity ID: " << m_entity_id << std::endl; throw BuildingException( err.str().c_str() ); }
	...
	m_PredefinedType = IfcEarthworksCutTypeEnum::createObjectFromSTEP( args[8], map );
}
void IfcEarthworksCut::getAttributes( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes ) const
{
	IfcFeatureElementSubtraction::getAttributes( vec_attributes );
	vec_attributes.emplace_back( std::make_pair( "PredefinedType", m_PredefinedType ) );
}
void IfcEarthworksCut::getAttributesInverse( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes_inverse ) const
{
	IfcFeatureElementSubtraction::getAttributesInverse( vec_attributes_inverse );
}
void IfcEarthworksCut::setInverseCounterparts( shared_ptr<BuildingEntity> ptr_self_entity )
{
	IfcFeatureElementSubtraction::setInverseCounterparts( ptr_self_entity );
}
void IfcEarthworksCut::unlinkFromInverseCounterparts()
{
