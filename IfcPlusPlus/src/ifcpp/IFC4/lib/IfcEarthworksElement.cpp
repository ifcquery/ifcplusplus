...
IfcEarthworksElement::IfcEarthworksElement( int id ) { m_entity_id = id; }
shared_ptr<BuildingObject> IfcEarthworksElement::getDeepCopy( BuildingCopyOptions& options )
{
	shared_ptr<IfcEarthworksElement> copy_self( new IfcEarthworksElement() );
	if( m_GlobalId )
	{
...
void IfcEarthworksElement::getStepLine( std::stringstream& stream ) const
{
	stream << "#" << m_entity_id << "= IFCEARTHWORKSELEMENT" << "(";
...
void IfcEarthworksElement::getStepParameter( std::stringstream& stream, bool /*is_select_type*/ ) const { stream << "#" << m_entity_id; }
const std::wstring IfcEarthworksElement::toString() const { return L"IfcEarthworksElement"; }
void IfcEarthworksElement::readStepArguments( const std::vector<std::wstring>& args, const std::map<int,shared_ptr<BuildingEntity> >& map )
{
	const size_t num_args = args.size();
	if( num_args != 8 ){ std::stringstream err; err << "Wrong parameter count for entity IfcEarthworksElement, expecting 8, having " << num_args << ". Entity ID: " << m_entity_id << std::endl; throw BuildingException( err.str().c_str() ); }
...
void IfcEarthworksElement::getAttributes( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes ) const
{
	IfcBuiltElement::getAttributes( vec_attributes );
}
void IfcEarthworksElement::getAttributesInverse( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes_inverse ) const
{
	IfcBuiltElement::getAttributesInverse( vec_attributes_inverse );
}
void IfcEarthworksElement::setInverseCounterparts( shared_ptr<BuildingEntity> ptr_self_entity )
{
	IfcBuiltElement::setInverseCounterparts( ptr_self_entity );
}
void IfcEarthworksElement::unlinkFromInverseCounterparts()
{
