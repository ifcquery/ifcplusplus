...
IfcElementQuantity::IfcElementQuantity() = default;
...
void IfcElementQuantity::readStepArguments( const std::vector<std::wstring>& args, const std::map<int,shared_ptr<BuildingEntity> >& map )
{
	...
	if( num_args != 6 ){ std::stringstream err; err << "Wrong parameter count for entity IfcElementQuantity, expecting 6, having " << num_args << ". Entity ID: " << m_entity_id << std::endl; throw BuildingException( err.str() ); }
...
void IfcElementQuantity::getAttributes( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes ) const
{
	...
	vec_attributes.emplace_back( "MethodOfMeasurement", m_MethodOfMeasurement );
	if( !m_Quantities.empty() )
	{
		...
		vec_attributes.emplace_back( "Quantities", Quantities_vec_object );
	}
}
