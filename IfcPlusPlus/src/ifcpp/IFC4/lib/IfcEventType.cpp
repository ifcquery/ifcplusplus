...
// ENTITY IfcEventType 
IfcEventType::IfcEventType() = default;
...
void IfcEventType::readStepArguments( const std::vector<std::wstring>& args, const std::map<int,shared_ptr<BuildingEntity> >& map )
{
	...
	if( num_args != 12 ){ std::stringstream err; err << "Wrong parameter count for entity IfcEventType, expecting 12, having " << num_args << ". Entity ID: " << m_entity_id << std::endl; throw BuildingException( err.str() ); }
...
void IfcEventType::getAttributes( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes ) const
{
	...
	vec_attributes.emplace_back( "EventTriggerType", m_EventTriggerType );
	vec_attributes.emplace_back( "UserDefinedEventTriggerType", m_UserDefinedEventTriggerType );
}
