...
// ENTITY IfcFacility 
IfcFacility::IfcFacility( int id ) { m_entity_id = id; }
shared_ptr<BuildingObject> IfcFacility::getDeepCopy( BuildingCopyOptions& options )
{
	shared_ptr<IfcFacility> copy_self( new IfcFacility() );
	if( m_GlobalId )
	{
...
void IfcFacility::getStepLine( std::stringstream& stream ) const
{
	stream << "#" << m_entity_id << "= IFCFACILITY" << "(";
...
void IfcFacility::getStepParameter( std::stringstream& stream, bool /*is_select_type*/ ) const { stream << "#" << m_entity_id; }
const std::wstring IfcFacility::toString() const { return L"IfcFacility"; }
void IfcFacility::readStepArguments( const std::vector<std::wstring>& args, const std::map<int,shared_ptr<BuildingEntity> >& map )
{
	const size_t num_args = args.size();
	if( num_args != 9 ){ std::stringstream err; err << "Wrong parameter count for entity IfcFacility, expecting 9, having " << num_args << ". Entity ID: " << m_entity_id << std::endl; throw BuildingException( err.str().c_str() ); }
...
void IfcFacility::getAttributes( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes ) const
{
	IfcSpatialStructureElement::getAttributes( vec_attributes );
}
void IfcFacility::getAttributesInverse( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes_inverse ) const
{
	IfcSpatialStructureElement::getAttributesInverse( vec_attributes_inverse );
}
void IfcFacility::setInverseCounterparts( shared_ptr<BuildingEntity> ptr_self_entity )
{
	IfcSpatialStructureElement::setInverseCounterparts( ptr_self_entity );
}
void IfcFacility::unlinkFromInverseCounterparts()
{
