...
IfcFeatureElementSubtraction::IfcFeatureElementSubtraction() = default;
...
void IfcFeatureElementSubtraction::readStepArguments( const std::vector<std::wstring>& args, const std::map<int,shared_ptr<BuildingEntity> >& map )
{
	...
	if( num_args != 8 ){ std::stringstream err; err << "Wrong parameter count for entity IfcFeatureElementSubtraction, expecting 8, having " << num_args << ". Entity ID: " << m_entity_id << std::endl; throw BuildingException( err.str() ); }
...
void IfcFeatureElementSubtraction::getAttributesInverse( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes_inverse ) const
{
	...
	vec_attributes_inverse.emplace_back( "VoidsElements_inverse", shared_ptr<BuildingEntity>( m_VoidsElements_inverse ) );
}
