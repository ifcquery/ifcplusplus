...
IfcGeotechnicalElement::IfcGeotechnicalElement( int id ) { m_entity_id = id; }
shared_ptr<BuildingObject> IfcGeotechnicalElement::getDeepCopy( BuildingCopyOptions& options )
{
	shared_ptr<IfcGeotechnicalElement> copy_self( new IfcGeotechnicalElement() );
	if( m_GlobalId )
	{
...
void IfcGeotechnicalElement::getStepLine( std::stringstream& stream ) const
{
	stream << "#" << m_entity_id << "= IFCGEOTECHNICALELEMENT" << "(";
...
void IfcGeotechnicalElement::getStepParameter( std::stringstream& stream, bool /*is_select_type*/ ) const { stream << "#" << m_entity_id; }
const std::wstring IfcGeotechnicalElement::toString() const { return L"IfcGeotechnicalElement"; }
void IfcGeotechnicalElement::readStepArguments( const std::vector<std::wstring>& args, const std::map<int,shared_ptr<BuildingEntity> >& map )
{
	const size_t num_args = args.size();
	if( num_args != 8 ){ std::stringstream err; err << "Wrong parameter count for entity IfcGeotechnicalElement, expecting 8, having " << num_args << ". Entity ID: " << m_entity_id << std::endl; throw BuildingException( err.str().c_str() ); }
...
void IfcGeotechnicalElement::getAttributes( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes ) const
{
	IfcElement::getAttributes( vec_attributes );
}
void IfcGeotechnicalElement::getAttributesInverse( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes_inverse ) const
{
	IfcElement::getAttributesInverse( vec_attributes_inverse );
}
void IfcGeotechnicalElement::setInverseCounterparts( shared_ptr<BuildingEntity> ptr_self_entity )
{
	IfcElement::setInverseCounterparts( ptr_self_entity );
}
void IfcGeotechnicalElement::unlinkFromInverseCounterparts()
{
