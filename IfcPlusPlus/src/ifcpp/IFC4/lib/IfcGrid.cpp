...
void IfcGrid::readStepArguments( const std::vector<std::wstring>& args, const std::map<int,shared_ptr<BuildingEntity> >& map )
{
	...
	if( num_args != 11 ){ std::stringstream err; err << "Wrong parameter count for entity IfcGrid, expecting 11, having " << num_args << ". Entity ID: " << m_entity_id << std::endl; throw BuildingException( err.str() ); }
...
void IfcGrid::getAttributes( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes ) const
{
	...
	if( !m_UAxes.empty() )
	{
		...
		vec_attributes.emplace_back( "UAxes", UAxes_vec_object );
	}
	if( !m_VAxes.empty() )
	{
		...
		vec_attributes.emplace_back( "VAxes", VAxes_vec_object );
	}
	if( !m_WAxes.empty() )
	{
		...
		vec_attributes.emplace_back( "WAxes", WAxes_vec_object );
	}
...
void IfcGrid::setInverseCounterparts( shared_ptr<BuildingEntity> ptr_self_entity )
{
	...
	for(auto & m_UAxe : m_UAxes)
	{
		if( m_UAxe )
		{
			m_UAxe->m_PartOfU_inverse.push_back( ptr_self );
		}
	}
	for(auto & m_VAxe : m_VAxes)
	{
		if( m_VAxe )
		{
			m_VAxe->m_PartOfV_inverse.push_back( ptr_self );
		}
	}
	for(auto & m_WAxe : m_WAxes)
	{
		if( m_WAxe )
		{
			m_WAxe->m_PartOfW_inverse.push_back( ptr_self );
		}
	}
}
void IfcGrid::unlinkFromInverseCounterparts()
{
	...
	for(auto & m_UAxe : m_UAxes)
	{
		if( m_UAxe )
		{
			std::vector<weak_ptr<IfcGrid> >& PartOfU_inverse = m_UAxe->m_PartOfU_inverse;
	...
	for(auto & m_VAxe : m_VAxes)
	{
		if( m_VAxe )
		{
			std::vector<weak_ptr<IfcGrid> >& PartOfV_inverse = m_VAxe->m_PartOfV_inverse;
	...
	for(auto & m_WAxe : m_WAxes)
	{
		if( m_WAxe )
		{
			std::vector<weak_ptr<IfcGrid> >& PartOfW_inverse = m_WAxe->m_PartOfW_inverse;
