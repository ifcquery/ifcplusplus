...
void IfcGroup::readStepArguments( const std::vector<std::wstring>& args, const std::map<int,shared_ptr<BuildingEntity> >& map )
{
	...
	if( num_args != 5 ){ std::stringstream err; err << "Wrong parameter count for entity IfcGroup, expecting 5, having " << num_args << ". Entity ID: " << m_entity_id << std::endl; throw BuildingException( err.str() ); }
...
void IfcGroup::getAttributesInverse( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes_inverse ) const
{
	...
	if( !m_IsGroupedBy_inverse.empty() )
	{
		...
		for(const auto & i : m_IsGroupedBy_inverse)
		{
			if( !i.expired() )
			{
				IsGroupedBy_inverse_vec_obj->m_vec.push_back( shared_ptr<IfcRelAssignsToGroup>( i ) );
			}
		}
		vec_attributes_inverse.emplace_back( "IsGroupedBy_inverse", IsGroupedBy_inverse_vec_obj );
	}
}
