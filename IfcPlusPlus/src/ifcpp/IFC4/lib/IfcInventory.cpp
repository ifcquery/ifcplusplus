...
// ENTITY IfcInventory 
IfcInventory::IfcInventory() = default;
...
shared_ptr<BuildingObject> IfcInventory::getDeepCopy( BuildingCopyOptions& options )
{
	...
	for(auto item_ii : m_ResponsiblePersons)
	{
			if( item_ii )
		{
...
void IfcInventory::readStepArguments( const std::vector<std::wstring>& args, const std::map<int,shared_ptr<BuildingEntity> >& map )
{
	...
	if( num_args != 11 ){ std::stringstream err; err << "Wrong parameter count for entity IfcInventory, expecting 11, having " << num_args << ". Entity ID: " << m_entity_id << std::endl; throw BuildingException( err.str() ); }
...
void IfcInventory::getAttributes( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes ) const
{
	...
	vec_attributes.emplace_back( "Jurisdiction", m_Jurisdiction );
	if( !m_ResponsiblePersons.empty() )
	{
		...
		vec_attributes.emplace_back( "ResponsiblePersons", ResponsiblePersons_vec_object );
	}
	vec_attributes.emplace_back( "LastUpdateDate", m_LastUpdateDate );
