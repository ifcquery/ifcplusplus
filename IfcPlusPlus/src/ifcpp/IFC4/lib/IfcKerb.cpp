...
// ENTITY IfcKerb 
IfcKerb::IfcKerb( int id ) { m_entity_id = id; }
shared_ptr<BuildingObject> IfcKerb::getDeepCopy( BuildingCopyOptions& options )
{
	shared_ptr<IfcKerb> copy_self( new IfcKerb() );
	if( m_GlobalId )
	{
...
void IfcKerb::getStepLine( std::stringstream& stream ) const
{
	stream << "#" << m_entity_id << "= IFCKERB" << "(";
...
void IfcKerb::getStepParameter( std::stringstream& stream, bool /*is_select_type*/ ) const { stream << "#" << m_entity_id; }
const std::wstring IfcKerb::toString() const { return L"IfcKerb"; }
void IfcKerb::readStepArguments( const std::vector<std::wstring>& args, const std::map<int,shared_ptr<BuildingEntity> >& map )
{
	const size_t num_args = args.size();
	if( num_args != 9 ){ std::stringstream err; err << "Wrong parameter count for entity IfcKerb, expecting 9, having " << num_args << ". Entity ID: " << m_entity_id << std::endl; throw BuildingException( err.str().c_str() ); }
	...
	m_Mountable = IfcBoolean::createObjectFromSTEP( args[8], map );
}
void IfcKerb::getAttributes( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes ) const
{
	IfcBuiltElement::getAttributes( vec_attributes );
	vec_attributes.emplace_back( std::make_pair( "Mountable", m_Mountable ) );
}
void IfcKerb::getAttributesInverse( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes_inverse ) const
{
	IfcBuiltElement::getAttributesInverse( vec_attributes_inverse );
}
void IfcKerb::setInverseCounterparts( shared_ptr<BuildingEntity> ptr_self_entity )
{
	IfcBuiltElement::setInverseCounterparts( ptr_self_entity );
}
void IfcKerb::unlinkFromInverseCounterparts()
{
