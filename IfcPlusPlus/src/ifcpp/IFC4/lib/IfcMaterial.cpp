...
// ENTITY IfcMaterial 
IfcMaterial::IfcMaterial() = default;
...
void IfcMaterial::readStepArguments( const std::vector<std::wstring>& args, const std::map<int,shared_ptr<BuildingEntity> >& map )
{
	...
	if( num_args != 3 ){ std::stringstream err; err << "Wrong parameter count for entity IfcMaterial, expecting 3, having " << num_args << ". Entity ID: " << m_entity_id << std::endl; throw BuildingException( err.str() ); }
...
void IfcMaterial::getAttributesInverse( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes_inverse ) const
{
	...
	if( !m_HasRepresentation_inverse.empty() )
	{
		...
		for(const auto & i : m_HasRepresentation_inverse)
		{
			if( !i.expired() )
			{
				HasRepresentation_inverse_vec_obj->m_vec.push_back( shared_ptr<IfcMaterialDefinitionRepresentation>( i ) );
			}
		}
		vec_attributes_inverse.emplace_back( "HasRepresentation_inverse", HasRepresentation_inverse_vec_obj );
	}
	if( !m_IsRelatedWith_inverse.empty() )
	{
		...
		for(const auto & i : m_IsRelatedWith_inverse)
		{
			if( !i.expired() )
			{
				IsRelatedWith_inverse_vec_obj->m_vec.push_back( shared_ptr<IfcMaterialRelationship>( i ) );
			}
		}
	...
	if( !m_RelatesTo_inverse.empty() )
	{
		...
		for(const auto & i : m_RelatesTo_inverse)
		{
			if( !i.expired() )
			{
				RelatesTo_inverse_vec_obj->m_vec.push_back( shared_ptr<IfcMaterialRelationship>( i ) );
			}
		}
		vec_attributes_inverse.emplace_back( "RelatesTo_inverse", RelatesTo_inverse_vec_obj );
	}
}
