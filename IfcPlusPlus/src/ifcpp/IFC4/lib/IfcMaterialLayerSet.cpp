...
IfcMaterialLayerSet::IfcMaterialLayerSet() = default;
...
shared_ptr<BuildingObject> IfcMaterialLayerSet::getDeepCopy( BuildingCopyOptions& options )
{
	...
	for(auto item_ii : m_MaterialLayers)
	{
			if( item_ii )
		{
...
void IfcMaterialLayerSet::getStepParameter( std::stringstream& stream, bool  /*is_select_type*/) const { stream << "#" << m_entity_id; }
...
void IfcMaterialLayerSet::readStepArguments( const std::vector<std::wstring>& args, const std::map<int,shared_ptr<BuildingEntity> >& map )
{
	...
	if( num_args != 3 ){ std::stringstream err; err << "Wrong parameter count for entity IfcMaterialLayerSet, expecting 3, having " << num_args << ". Entity ID: " << m_entity_id << std::endl; throw BuildingException( err.str() ); }
...
void IfcMaterialLayerSet::getAttributes( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes ) const
{
	...
	if( !m_MaterialLayers.empty() )
	{
		...
		vec_attributes.emplace_back( "MaterialLayers", MaterialLayers_vec_object );
	}
	vec_attributes.emplace_back( "LayerSetName", m_LayerSetName );
...
void IfcMaterialLayerSet::setInverseCounterparts( shared_ptr<BuildingEntity> ptr_self_entity )
{
	...
	for(auto & m_MaterialLayer : m_MaterialLayers)
	{
		if( m_MaterialLayer )
		{
			m_MaterialLayer->m_ToMaterialLayerSet_inverse = ptr_self;
		}
	}
}
void IfcMaterialLayerSet::unlinkFromInverseCounterparts()
{
	...
	for(auto & m_MaterialLayer : m_MaterialLayers)
	{
		if( m_MaterialLayer )
		{
			if( !m_MaterialLayer->m_ToMaterialLayerSet_inverse.expired() )
			{
				shared_ptr<IfcMaterialLayerSet> self_candidate( m_MaterialLayer->m_ToMaterialLayerSet_inverse );
				if( self_candidate.get() == this )
				{
					weak_ptr<IfcMaterialLayerSet>& self_candidate_weak = m_MaterialLayer->m_ToMaterialLayerSet_inverse;
