...
void IfcObject::readStepArguments( const std::vector<std::wstring>& args, const std::map<int,shared_ptr<BuildingEntity> >& map )
{
	...
	if( num_args != 5 ){ std::stringstream err; err << "Wrong parameter count for entity IfcObject, expecting 5, having " << num_args << ". Entity ID: " << m_entity_id << std::endl; throw BuildingException( err.str() ); }
...
void IfcObject::getAttributes( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes ) const
{
	...
	vec_attributes.emplace_back( "ObjectType", m_ObjectType );
}
void IfcObject::getAttributesInverse( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes_inverse ) const
{
	...
	if( !m_IsDeclaredBy_inverse.empty() )
	{
		...
		for(const auto & i : m_IsDeclaredBy_inverse)
		{
			if( !i.expired() )
			{
				IsDeclaredBy_inverse_vec_obj->m_vec.push_back( shared_ptr<IfcRelDefinesByObject>( i ) );
			}
		}
		vec_attributes_inverse.emplace_back( "IsDeclaredBy_inverse", IsDeclaredBy_inverse_vec_obj );
	}
	if( !m_Declares_inverse.empty() )
	{
		shared_ptr<AttributeObjectVector> Declares_inverse_vec_obj( new AttributeObjectVector() );
		for(const auto & i : m_Declares_inverse)
		{
			if( !i.expired() )
			{
				Declares_inverse_vec_obj->m_vec.push_back( shared_ptr<IfcRelDefinesByObject>( i ) );
			}
		}
		vec_attributes_inverse.emplace_back( "Declares_inverse", Declares_inverse_vec_obj );
	}
	if( !m_IsTypedBy_inverse.empty() )
	{
		shared_ptr<AttributeObjectVector> IsTypedBy_inverse_vec_obj( new AttributeObjectVector() );
		for(const auto & i : m_IsTypedBy_inverse)
		{
			if( !i.expired() )
			{
				IsTypedBy_inverse_vec_obj->m_vec.push_back( shared_ptr<IfcRelDefinesByType>( i ) );
			}
		}
		vec_attributes_inverse.emplace_back( "IsTypedBy_inverse", IsTypedBy_inverse_vec_obj );
	}
	if( !m_IsDefinedBy_inverse.empty() )
	{
		...
		for(const auto & i : m_IsDefinedBy_inverse)
		{
			if( !i.expired() )
			{
				IsDefinedBy_inverse_vec_obj->m_vec.push_back( shared_ptr<IfcRelDefinesByProperties>( i ) );
			}
		}
