...
shared_ptr<BuildingObject> IfcOffsetCurveByDistances::getDeepCopy( BuildingCopyOptions& options )
{
	...
	for( size_t ii=0; ii<m_OffsetValues.size(); ++ii )
	{
		...
		if( item_ii )
		{
			copy_self->m_OffsetValues.push_back( dynamic_pointer_cast<IfcDistanceExpression>(item_ii->getDeepCopy(options) ) );
		}
	}
...
void IfcOffsetCurveByDistances::getStepParameter( std::stringstream& stream, bool ) const { stream << "#" << m_entity_id; }
...
void IfcOffsetCurveByDistances::getAttributes( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes )
{
...
void IfcOffsetCurveByDistances::getAttributesInverse( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes_inverse )
{
