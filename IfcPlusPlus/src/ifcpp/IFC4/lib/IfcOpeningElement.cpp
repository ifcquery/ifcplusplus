...
IfcOpeningElement::IfcOpeningElement() = default;
...
void IfcOpeningElement::readStepArguments( const std::vector<std::wstring>& args, const std::map<int,shared_ptr<BuildingEntity> >& map )
{
	...
	if( num_args != 9 ){ std::stringstream err; err << "Wrong parameter count for entity IfcOpeningElement, expecting 9, having " << num_args << ". Entity ID: " << m_entity_id << std::endl; throw BuildingException( err.str() ); }
...
void IfcOpeningElement::getAttributesInverse( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes_inverse ) const
{
	...
	if( !m_HasFillings_inverse.empty() )
	{
		...
		for(const auto & i : m_HasFillings_inverse)
		{
			if( !i.expired() )
			{
				HasFillings_inverse_vec_obj->m_vec.push_back( shared_ptr<IfcRelFillsElement>( i ) );
			}
		}
		vec_attributes_inverse.emplace_back( "HasFillings_inverse", HasFillings_inverse_vec_obj );
	}
}
