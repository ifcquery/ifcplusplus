...
// ENTITY IfcPavement 
IfcPavement::IfcPavement( int id ) { m_entity_id = id; }
shared_ptr<BuildingObject> IfcPavement::getDeepCopy( BuildingCopyOptions& options )
{
	shared_ptr<IfcPavement> copy_self( new IfcPavement() );
	if( m_GlobalId )
	{
...
void IfcPavement::getStepLine( std::stringstream& stream ) const
{
	stream << "#" << m_entity_id << "= IFCPAVEMENT" << "(";
...
void IfcPavement::getStepParameter( std::stringstream& stream, bool /*is_select_type*/ ) const { stream << "#" << m_entity_id; }
const std::wstring IfcPavement::toString() const { return L"IfcPavement"; }
void IfcPavement::readStepArguments( const std::vector<std::wstring>& args, const std::map<int,shared_ptr<BuildingEntity> >& map )
{
	const size_t num_args = args.size();
	if( num_args != 9 ){ std::stringstream err; err << "Wrong parameter count for entity IfcPavement, expecting 9, having " << num_args << ". Entity ID: " << m_entity_id << std::endl; throw BuildingException( err.str().c_str() ); }
	...
	m_Flexible = IfcBoolean::createObjectFromSTEP( args[8], map );
}
void IfcPavement::getAttributes( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes ) const
{
	IfcBuiltElement::getAttributes( vec_attributes );
	vec_attributes.emplace_back( std::make_pair( "Flexible", m_Flexible ) );
}
void IfcPavement::getAttributesInverse( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes_inverse ) const
{
	IfcBuiltElement::getAttributesInverse( vec_attributes_inverse );
}
void IfcPavement::setInverseCounterparts( shared_ptr<BuildingEntity> ptr_self_entity )
{
	IfcBuiltElement::setInverseCounterparts( ptr_self_entity );
}
void IfcPavement::unlinkFromInverseCounterparts()
{
