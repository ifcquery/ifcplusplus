...
IfcPermeableCoveringProperties::IfcPermeableCoveringProperties() = default;
...
void IfcPermeableCoveringProperties::readStepArguments( const std::vector<std::wstring>& args, const std::map<int,shared_ptr<BuildingEntity> >& map )
{
	...
	if( num_args != 9 ){ std::stringstream err; err << "Wrong parameter count for entity IfcPermeableCoveringProperties, expecting 9, having " << num_args << ". Entity ID: " << m_entity_id << std::endl; throw BuildingException( err.str() ); }
...
void IfcPermeableCoveringProperties::getAttributes( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes ) const
{
	...
	vec_attributes.emplace_back( "PanelPosition", m_PanelPosition );
	vec_attributes.emplace_back( "FrameDepth", m_FrameDepth );
	vec_attributes.emplace_back( "FrameThickness", m_FrameThickness );
