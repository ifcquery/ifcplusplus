...
void IfcPort::readStepArguments( const std::vector<std::wstring>& args, const std::map<int,shared_ptr<BuildingEntity> >& map )
{
	...
	if( num_args != 7 ){ std::stringstream err; err << "Wrong parameter count for entity IfcPort, expecting 7, having " << num_args << ". Entity ID: " << m_entity_id << std::endl; throw BuildingException( err.str() ); }
...
void IfcPort::getAttributesInverse( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes_inverse ) const
{
	...
	if( !m_ContainedIn_inverse.empty() )
	{
		...
		for(const auto & i : m_ContainedIn_inverse)
		{
			if( !i.expired() )
			{
				ContainedIn_inverse_vec_obj->m_vec.push_back( shared_ptr<IfcRelConnectsPortToElement>( i ) );
			}
		}
		vec_attributes_inverse.emplace_back( "ContainedIn_inverse", ContainedIn_inverse_vec_obj );
	}
	if( !m_ConnectedFrom_inverse.empty() )
	{
		...
		for(const auto & i : m_ConnectedFrom_inverse)
		{
			if( !i.expired() )
			{
				ConnectedFrom_inverse_vec_obj->m_vec.push_back( shared_ptr<IfcRelConnectsPorts>( i ) );
			}
		}
	...
	if( !m_ConnectedTo_inverse.empty() )
	{
		...
		for(const auto & i : m_ConnectedTo_inverse)
		{
			if( !i.expired() )
			{
				ConnectedTo_inverse_vec_obj->m_vec.push_back( shared_ptr<IfcRelConnectsPorts>( i ) );
			}
		}
