...
IfcPositioningElement::IfcPositioningElement( int id ) { m_entity_id = id; }
shared_ptr<BuildingObject> IfcPositioningElement::getDeepCopy( BuildingCopyOptions& options )
{
	shared_ptr<IfcPositioningElement> copy_self( new IfcPositioningElement() );
	if( m_GlobalId )
	{
...
{
	stream << "#" << m_entity_id << "= IFCPOSITIONINGELEMENT" << "(";
...
void IfcPositioningElement::getStepParameter( std::stringstream& stream, bool /*is_select_type*/ ) const { stream << "#" << m_entity_id; }
const std::wstring IfcPositioningElement::toString() const { return L"IfcPositioningElement"; }
void IfcPositioningElement::readStepArguments( const std::vector<std::wstring>& args, const std::map<int,shared_ptr<BuildingEntity> >& map )
{
	const size_t num_args = args.size();
	if( num_args != 7 ){ std::stringstream err; err << "Wrong parameter count for entity IfcPositioningElement, expecting 7, having " << num_args << ". Entity ID: " << m_entity_id << std::endl; throw BuildingException( err.str().c_str() ); }
...
void IfcPositioningElement::getAttributes( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes ) const
{
	IfcProduct::getAttributes( vec_attributes );
}
void IfcPositioningElement::getAttributesInverse( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes_inverse ) const
{
...
void IfcPositioningElement::unlinkFromInverseCounterparts()
{
