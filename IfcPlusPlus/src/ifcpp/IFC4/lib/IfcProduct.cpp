...
void IfcProduct::readStepArguments( const std::vector<std::wstring>& args, const std::map<int,shared_ptr<BuildingEntity> >& map )
{
	...
	if( num_args != 7 ){ std::stringstream err; err << "Wrong parameter count for entity IfcProduct, expecting 7, having " << num_args << ". Entity ID: " << m_entity_id << std::endl; throw BuildingException( err.str() ); }
...
void IfcProduct::getAttributes( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes ) const
{
	...
	vec_attributes.emplace_back( "ObjectPlacement", m_ObjectPlacement );
	vec_attributes.emplace_back( "Representation", m_Representation );
}
void IfcProduct::getAttributesInverse( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes_inverse ) const
{
	...
	if( !m_ReferencedBy_inverse.empty() )
	{
		...
		for(const auto & i : m_ReferencedBy_inverse)
		{
			if( !i.expired() )
			{
				ReferencedBy_inverse_vec_obj->m_vec.push_back( shared_ptr<IfcRelAssignsToProduct>( i ) );
			}
		}
		vec_attributes_inverse.emplace_back( "ReferencedBy_inverse", ReferencedBy_inverse_vec_obj );
	}
}
