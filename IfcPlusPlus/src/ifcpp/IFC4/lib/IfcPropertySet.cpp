...
IfcPropertySet::IfcPropertySet() = default;
...
void IfcPropertySet::readStepArguments( const std::vector<std::wstring>& args, const std::map<int,shared_ptr<BuildingEntity> >& map )
{
	...
	if( num_args != 5 ){ std::stringstream err; err << "Wrong parameter count for entity IfcPropertySet, expecting 5, having " << num_args << ". Entity ID: " << m_entity_id << std::endl; throw BuildingException( err.str() ); }
...
void IfcPropertySet::setInverseCounterparts( shared_ptr<BuildingEntity> ptr_self_entity )
{
	...
	for(auto & m_HasPropertie : m_HasProperties)
	{
		if( m_HasPropertie )
		{
			m_HasPropertie->m_PartOfPset_inverse.push_back( ptr_self );
		}
	}
}
void IfcPropertySet::unlinkFromInverseCounterparts()
{
	...
	for(auto & m_HasPropertie : m_HasProperties)
	{
		if( m_HasPropertie )
		{
			std::vector<weak_ptr<IfcPropertySet> >& PartOfPset_inverse = m_HasPropertie->m_PartOfPset_inverse;
