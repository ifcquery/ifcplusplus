...
IfcPropertySetDefinition::IfcPropertySetDefinition() = default;
...
void IfcPropertySetDefinition::readStepArguments( const std::vector<std::wstring>& args, const std::map<int,shared_ptr<BuildingEntity> >& map )
{
	...
	if( num_args != 4 ){ std::stringstream err; err << "Wrong parameter count for entity IfcPropertySetDefinition, expecting 4, having " << num_args << ". Entity ID: " << m_entity_id << std::endl; throw BuildingException( err.str() ); }
...
void IfcPropertySetDefinition::getAttributesInverse( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes_inverse ) const
{
	...
	if( !m_DefinesType_inverse.empty() )
	{
		...
		for(const auto & i : m_DefinesType_inverse)
		{
			if( !i.expired() )
			{
				DefinesType_inverse_vec_obj->m_vec.push_back( shared_ptr<IfcTypeObject>( i ) );
			}
		}
		vec_attributes_inverse.emplace_back( "DefinesType_inverse", DefinesType_inverse_vec_obj );
	}
	if( !m_IsDefinedBy_inverse.empty() )
	{
		shared_ptr<AttributeObjectVector> IsDefinedBy_inverse_vec_obj( new AttributeObjectVector() );
		for(const auto & i : m_IsDefinedBy_inverse)
		{
			if( !i.expired() )
			{
				IsDefinedBy_inverse_vec_obj->m_vec.push_back( shared_ptr<IfcRelDefinesByTemplate>( i ) );
			}
		}
		vec_attributes_inverse.emplace_back( "IsDefinedBy_inverse", IsDefinedBy_inverse_vec_obj );
	}
	if( !m_DefinesOccurrence_inverse.empty() )
	{
		...
		for(const auto & i : m_DefinesOccurrence_inverse)
		{
			if( !i.expired() )
			{
				DefinesOccurrence_inverse_vec_obj->m_vec.push_back( shared_ptr<IfcRelDefinesByProperties>( i ) );
			}
		}
		vec_attributes_inverse.emplace_back( "DefinesOccurrence_inverse", DefinesOccurrence_inverse_vec_obj );
	}
}
