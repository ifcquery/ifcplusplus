...
IfcPropertySetTemplate::IfcPropertySetTemplate() = default;
...
void IfcPropertySetTemplate::readStepArguments( const std::vector<std::wstring>& args, const std::map<int,shared_ptr<BuildingEntity> >& map )
{
	...
	if( num_args != 7 ){ std::stringstream err; err << "Wrong parameter count for entity IfcPropertySetTemplate, expecting 7, having " << num_args << ". Entity ID: " << m_entity_id << std::endl; throw BuildingException( err.str() ); }
...
void IfcPropertySetTemplate::getAttributes( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes ) const
{
	...
	vec_attributes.emplace_back( "ApplicableEntity", m_ApplicableEntity );
...
void IfcPropertySetTemplate::getAttributesInverse( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes_inverse ) const
{
	...
	if( !m_Defines_inverse.empty() )
	{
		...
		for(const auto & i : m_Defines_inverse)
		{
			if( !i.expired() )
			{
				Defines_inverse_vec_obj->m_vec.push_back( shared_ptr<IfcRelDefinesByTemplate>( i ) );
			}
		}
		vec_attributes_inverse.emplace_back( "Defines_inverse", Defines_inverse_vec_obj );
	}
}
void IfcPropertySetTemplate::setInverseCounterparts( shared_ptr<BuildingEntity> ptr_self_entity )
{
	...
	for(auto & m_HasPropertyTemplate : m_HasPropertyTemplates)
	{
		...
		{
			m_HasPropertyTemplate->m_PartOfPsetTemplate_inverse.push_back( ptr_self );
		}
	}
}
void IfcPropertySetTemplate::unlinkFromInverseCounterparts()
{
	...
	for(auto & m_HasPropertyTemplate : m_HasPropertyTemplates)
	{
		if( m_HasPropertyTemplate )
		{
			std::vector<weak_ptr<IfcPropertySetTemplate> >& PartOfPsetTemplate_inverse = m_HasPropertyTemplate->m_PartOfPsetTemplate_inverse;
