...
IfcPropertyTemplate::IfcPropertyTemplate() = default;
...
void IfcPropertyTemplate::readStepArguments( const std::vector<std::wstring>& args, const std::map<int,shared_ptr<BuildingEntity> >& map )
{
	...
	if( num_args != 4 ){ std::stringstream err; err << "Wrong parameter count for entity IfcPropertyTemplate, expecting 4, having " << num_args << ". Entity ID: " << m_entity_id << std::endl; throw BuildingException( err.str() ); }
...
void IfcPropertyTemplate::getAttributesInverse( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes_inverse ) const
{
	...
	if( !m_PartOfComplexTemplate_inverse.empty() )
	{
		...
		for(const auto & i : m_PartOfComplexTemplate_inverse)
		{
			if( !i.expired() )
			{
				PartOfComplexTemplate_inverse_vec_obj->m_vec.push_back( shared_ptr<IfcComplexPropertyTemplate>( i ) );
			}
		}
		vec_attributes_inverse.emplace_back( "PartOfComplexTemplate_inverse", PartOfComplexTemplate_inverse_vec_obj );
	}
	if( !m_PartOfPsetTemplate_inverse.empty() )
	{
		...
		for(const auto & i : m_PartOfPsetTemplate_inverse)
		{
			if( !i.expired() )
			{
				PartOfPsetTemplate_inverse_vec_obj->m_vec.push_back( shared_ptr<IfcPropertySetTemplate>( i ) );
			}
		}
		vec_attributes_inverse.emplace_back( "PartOfPsetTemplate_inverse", PartOfPsetTemplate_inverse_vec_obj );
	}
}
