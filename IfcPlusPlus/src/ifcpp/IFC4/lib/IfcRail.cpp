...
// ENTITY IfcRail 
IfcRail::IfcRail( int id ) { m_entity_id = id; }
shared_ptr<BuildingObject> IfcRail::getDeepCopy( BuildingCopyOptions& options )
{
	shared_ptr<IfcRail> copy_self( new IfcRail() );
	if( m_GlobalId )
	{
...
void IfcRail::getStepLine( std::stringstream& stream ) const
{
	stream << "#" << m_entity_id << "= IFCRAIL" << "(";
...
void IfcRail::getStepParameter( std::stringstream& stream, bool /*is_select_type*/ ) const { stream << "#" << m_entity_id; }
const std::wstring IfcRail::toString() const { return L"IfcRail"; }
void IfcRail::readStepArguments( const std::vector<std::wstring>& args, const std::map<int,shared_ptr<BuildingEntity> >& map )
{
	const size_t num_args = args.size();
	if( num_args != 9 ){ std::stringstream err; err << "Wrong parameter count for entity IfcRail, expecting 9, having " << num_args << ". Entity ID: " << m_entity_id << std::endl; throw BuildingException( err.str().c_str() ); }
	...
	m_PredefinedType = IfcRailTypeEnum::createObjectFromSTEP( args[8], map );
}
void IfcRail::getAttributes( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes ) const
{
	IfcBuiltElement::getAttributes( vec_attributes );
	vec_attributes.emplace_back( std::make_pair( "PredefinedType", m_PredefinedType ) );
}
void IfcRail::getAttributesInverse( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes_inverse ) const
{
	IfcBuiltElement::getAttributesInverse( vec_attributes_inverse );
}
void IfcRail::setInverseCounterparts( shared_ptr<BuildingEntity> ptr_self_entity )
{
	IfcBuiltElement::setInverseCounterparts( ptr_self_entity );
}
void IfcRail::unlinkFromInverseCounterparts()
{
