...
// ENTITY IfcRailway 
IfcRailway::IfcRailway( int id ) { m_entity_id = id; }
shared_ptr<BuildingObject> IfcRailway::getDeepCopy( BuildingCopyOptions& options )
{
	shared_ptr<IfcRailway> copy_self( new IfcRailway() );
	if( m_GlobalId )
	{
...
void IfcRailway::getStepLine( std::stringstream& stream ) const
{
	stream << "#" << m_entity_id << "= IFCRAILWAY" << "(";
...
void IfcRailway::getStepParameter( std::stringstream& stream, bool /*is_select_type*/ ) const { stream << "#" << m_entity_id; }
const std::wstring IfcRailway::toString() const { return L"IfcRailway"; }
void IfcRailway::readStepArguments( const std::vector<std::wstring>& args, const std::map<int,shared_ptr<BuildingEntity> >& map )
{
	const size_t num_args = args.size();
	if( num_args != 9 ){ std::stringstream err; err << "Wrong parameter count for entity IfcRailway, expecting 9, having " << num_args << ". Entity ID: " << m_entity_id << std::endl; throw BuildingException( err.str().c_str() ); }
...
void IfcRailway::getAttributes( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes ) const
{
	IfcFacility::getAttributes( vec_attributes );
}
void IfcRailway::getAttributesInverse( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes_inverse ) const
{
	IfcFacility::getAttributesInverse( vec_attributes_inverse );
}
void IfcRailway::setInverseCounterparts( shared_ptr<BuildingEntity> ptr_self_entity )
{
	IfcFacility::setInverseCounterparts( ptr_self_entity );
}
void IfcRailway::unlinkFromInverseCounterparts()
{
