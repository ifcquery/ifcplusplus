...
// ENTITY IfcReference 
IfcReference::IfcReference() = default;
...
shared_ptr<BuildingObject> IfcReference::getDeepCopy( BuildingCopyOptions& options )
{
	...
	for(auto item_ii : m_ListPositions)
	{
			if( item_ii )
		{
...
void IfcReference::readStepArguments( const std::vector<std::wstring>& args, const std::map<int,shared_ptr<BuildingEntity> >& map )
{
	...
	if( num_args != 5 ){ std::stringstream err; err << "Wrong parameter count for entity IfcReference, expecting 5, having " << num_args << ". Entity ID: " << m_entity_id << std::endl; throw BuildingException( err.str() ); }
...
{
	vec_attributes.emplace_back( "TypeIdentifier", m_TypeIdentifier );
	vec_attributes.emplace_back( "AttributeIdentifier", m_AttributeIdentifier );
	vec_attributes.emplace_back( "InstanceName", m_InstanceName );
	if( !m_ListPositions.empty() )
	{
		...
		vec_attributes.emplace_back( "ListPositions", ListPositions_vec_object );
	}
	vec_attributes.emplace_back( "InnerReference", m_InnerReference );
}
...
void IfcReference::setInverseCounterparts( shared_ptr<BuildingEntity>  /*ptr_self*/)
{
}
