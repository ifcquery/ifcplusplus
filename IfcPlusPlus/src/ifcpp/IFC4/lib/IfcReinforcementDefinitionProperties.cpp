...
IfcReinforcementDefinitionProperties::IfcReinforcementDefinitionProperties() = default;
...
shared_ptr<BuildingObject> IfcReinforcementDefinitionProperties::getDeepCopy( BuildingCopyOptions& options )
{
	...
	for(auto item_ii : m_ReinforcementSectionDefinitions)
	{
			if( item_ii )
		{
...
void IfcReinforcementDefinitionProperties::readStepArguments( const std::vector<std::wstring>& args, const std::map<int,shared_ptr<BuildingEntity> >& map )
{
	...
	if( num_args != 6 ){ std::stringstream err; err << "Wrong parameter count for entity IfcReinforcementDefinitionProperties, expecting 6, having " << num_args << ". Entity ID: " << m_entity_id << std::endl; throw BuildingException( err.str() ); }
...
void IfcReinforcementDefinitionProperties::getAttributes( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes ) const
{
	...
	vec_attributes.emplace_back( "DefinitionType", m_DefinitionType );
	if( !m_ReinforcementSectionDefinitions.empty() )
	{
		...
		vec_attributes.emplace_back( "ReinforcementSectionDefinitions", ReinforcementSectionDefinitions_vec_object );
	}
}
