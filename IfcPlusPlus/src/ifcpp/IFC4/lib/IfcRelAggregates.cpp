...
IfcRelAggregates::IfcRelAggregates() = default;
...
void IfcRelAggregates::readStepArguments( const std::vector<std::wstring>& args, const std::map<int,shared_ptr<BuildingEntity> >& map )
{
	...
	if( num_args != 6 ){ std::stringstream err; err << "Wrong parameter count for entity IfcRelAggregates, expecting 6, having " << num_args << ". Entity ID: " << m_entity_id << std::endl; throw BuildingException( err.str() ); }
...
void IfcRelAggregates::setInverseCounterparts( shared_ptr<BuildingEntity> ptr_self_entity )
{
	...
	for(auto & m_RelatedObject : m_RelatedObjects)
	{
		if( m_RelatedObject )
		{
			m_RelatedObject->m_Decomposes_inverse.push_back( ptr_self );
		}
	}
	if( m_RelatingObject )
	{
...
void IfcRelAggregates::unlinkFromInverseCounterparts()
{
	...
	for(auto & m_RelatedObject : m_RelatedObjects)
	{
		if( m_RelatedObject )
		{
			std::vector<weak_ptr<IfcRelAggregates> >& Decomposes_inverse = m_RelatedObject->m_Decomposes_inverse;
