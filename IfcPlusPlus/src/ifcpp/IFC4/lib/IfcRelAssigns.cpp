...
IfcRelAssigns::IfcRelAssigns() = default;
...
void IfcRelAssigns::readStepArguments( const std::vector<std::wstring>& args, const std::map<int,shared_ptr<BuildingEntity> >& map )
{
	...
	if( num_args != 6 ){ std::stringstream err; err << "Wrong parameter count for entity IfcRelAssigns, expecting 6, having " << num_args << ". Entity ID: " << m_entity_id << std::endl; throw BuildingException( err.str() ); }
...
void IfcRelAssigns::getAttributes( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes ) const
{
	...
	vec_attributes.emplace_back( "RelatedObjectsType", m_RelatedObjectsType );
}
...
void IfcRelAssigns::setInverseCounterparts( shared_ptr<BuildingEntity> ptr_self_entity )
{
	...
	for(auto & m_RelatedObject : m_RelatedObjects)
	{
		if( m_RelatedObject )
		{
			m_RelatedObject->m_HasAssignments_inverse.push_back( ptr_self );
		}
	}
}
void IfcRelAssigns::unlinkFromInverseCounterparts()
{
	...
	for(auto & m_RelatedObject : m_RelatedObjects)
	{
		if( m_RelatedObject )
		{
			std::vector<weak_ptr<IfcRelAssigns> >& HasAssignments_inverse = m_RelatedObject->m_HasAssignments_inverse;
