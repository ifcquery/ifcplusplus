...
IfcRelConnectsPathElements::IfcRelConnectsPathElements() = default;
...
shared_ptr<BuildingObject> IfcRelConnectsPathElements::getDeepCopy( BuildingCopyOptions& options )
{
	...
	for(auto item_ii : m_RelatingPriorities)
	{
			if( item_ii )
		{
	...
	for(auto item_ii : m_RelatedPriorities)
	{
			if( item_ii )
		{
...
void IfcRelConnectsPathElements::readStepArguments( const std::vector<std::wstring>& args, const std::map<int,shared_ptr<BuildingEntity> >& map )
{
	...
	if( num_args != 11 ){ std::stringstream err; err << "Wrong parameter count for entity IfcRelConnectsPathElements, expecting 11, having " << num_args << ". Entity ID: " << m_entity_id << std::endl; throw BuildingException( err.str() ); }
...
void IfcRelConnectsPathElements::getAttributes( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes ) const
{
	...
	if( !m_RelatingPriorities.empty() )
	{
		...
		vec_attributes.emplace_back( "RelatingPriorities", RelatingPriorities_vec_object );
	}
	if( !m_RelatedPriorities.empty() )
	{
		...
		vec_attributes.emplace_back( "RelatedPriorities", RelatedPriorities_vec_object );
	}
	vec_attributes.emplace_back( "RelatedConnectionType", m_RelatedConnectionType );
	vec_attributes.emplace_back( "RelatingConnectionType", m_RelatingConnectionType );
}
