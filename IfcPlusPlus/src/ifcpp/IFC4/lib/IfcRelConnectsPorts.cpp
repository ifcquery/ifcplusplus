...
IfcRelConnectsPorts::IfcRelConnectsPorts() = default;
...
void IfcRelConnectsPorts::readStepArguments( const std::vector<std::wstring>& args, const std::map<int,shared_ptr<BuildingEntity> >& map )
{
	...
	if( num_args != 7 ){ std::stringstream err; err << "Wrong parameter count for entity IfcRelConnectsPorts, expecting 7, having " << num_args << ". Entity ID: " << m_entity_id << std::endl; throw BuildingException( err.str() ); }
...
void IfcRelConnectsPorts::getAttributes( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes ) const
{
	...
	vec_attributes.emplace_back( "RelatingPort", m_RelatingPort );
	vec_attributes.emplace_back( "RelatedPort", m_RelatedPort );
	vec_attributes.emplace_back( "RealizingElement", m_RealizingElement );
}
