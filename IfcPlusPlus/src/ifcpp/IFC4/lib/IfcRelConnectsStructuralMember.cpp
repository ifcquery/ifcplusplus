...
IfcRelConnectsStructuralMember::IfcRelConnectsStructuralMember() = default;
...
void IfcRelConnectsStructuralMember::readStepArguments( const std::vector<std::wstring>& args, const std::map<int,shared_ptr<BuildingEntity> >& map )
{
	...
	if( num_args != 10 ){ std::stringstream err; err << "Wrong parameter count for entity IfcRelConnectsStructuralMember, expecting 10, having " << num_args << ". Entity ID: " << m_entity_id << std::endl; throw BuildingException( err.str() ); }
...
void IfcRelConnectsStructuralMember::getAttributes( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes ) const
{
	...
	vec_attributes.emplace_back( "RelatingStructuralMember", m_RelatingStructuralMember );
	vec_attributes.emplace_back( "RelatedStructuralConnection", m_RelatedStructuralConnection );
	vec_attributes.emplace_back( "AppliedCondition", m_AppliedCondition );
	vec_attributes.emplace_back( "AdditionalConditions", m_AdditionalConditions );
	vec_attributes.emplace_back( "SupportedLength", m_SupportedLength );
	vec_attributes.emplace_back( "ConditionCoordinateSystem", m_ConditionCoordinateSystem );
}
