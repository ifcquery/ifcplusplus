...
IfcRelConnectsWithRealizingElements::IfcRelConnectsWithRealizingElements() = default;
...
shared_ptr<BuildingObject> IfcRelConnectsWithRealizingElements::getDeepCopy( BuildingCopyOptions& options )
{
	...
	for(auto item_ii : m_RealizingElements)
	{
			if( item_ii )
		{
...
void IfcRelConnectsWithRealizingElements::readStepArguments( const std::vector<std::wstring>& args, const std::map<int,shared_ptr<BuildingEntity> >& map )
{
	...
	if( num_args != 9 ){ std::stringstream err; err << "Wrong parameter count for entity IfcRelConnectsWithRealizingElements, expecting 9, having " << num_args << ". Entity ID: " << m_entity_id << std::endl; throw BuildingException( err.str() ); }
...
void IfcRelConnectsWithRealizingElements::getAttributes( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes ) const
{
	...
	if( !m_RealizingElements.empty() )
	{
		...
		vec_attributes.emplace_back( "RealizingElements", RealizingElements_vec_object );
	}
	vec_attributes.emplace_back( "ConnectionType", m_ConnectionType );
}
...
void IfcRelConnectsWithRealizingElements::setInverseCounterparts( shared_ptr<BuildingEntity> ptr_self_entity )
{
	...
	for(auto & m_RealizingElement : m_RealizingElements)
	{
		if( m_RealizingElement )
		{
			m_RealizingElement->m_IsConnectionRealization_inverse.push_back( ptr_self );
		}
	}
}
void IfcRelConnectsWithRealizingElements::unlinkFromInverseCounterparts()
{
	...
	for(auto & m_RealizingElement : m_RealizingElements)
	{
		if( m_RealizingElement )
		{
			std::vector<weak_ptr<IfcRelConnectsWithRealizingElements> >& IsConnectionRealization_inverse = m_RealizingElement->m_IsConnectionRealization_inverse;
