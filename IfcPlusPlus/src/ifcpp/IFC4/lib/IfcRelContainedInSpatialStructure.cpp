...
IfcRelContainedInSpatialStructure::IfcRelContainedInSpatialStructure() = default;
...
shared_ptr<BuildingObject> IfcRelContainedInSpatialStructure::getDeepCopy( BuildingCopyOptions& options )
{
	...
	for(auto item_ii : m_RelatedElements)
	{
			if( item_ii )
		{
...
void IfcRelContainedInSpatialStructure::readStepArguments( const std::vector<std::wstring>& args, const std::map<int,shared_ptr<BuildingEntity> >& map )
{
	...
	if( num_args != 6 ){ std::stringstream err; err << "Wrong parameter count for entity IfcRelContainedInSpatialStructure, expecting 6, having " << num_args << ". Entity ID: " << m_entity_id << std::endl; throw BuildingException( err.str() ); }
...
void IfcRelContainedInSpatialStructure::getAttributes( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes ) const
{
	...
	if( !m_RelatedElements.empty() )
	{
		...
		vec_attributes.emplace_back( "RelatedElements", RelatedElements_vec_object );
	}
	vec_attributes.emplace_back( "RelatingStructure", m_RelatingStructure );
}
...
void IfcRelContainedInSpatialStructure::setInverseCounterparts( shared_ptr<BuildingEntity> ptr_self_entity )
{
	...
	for(const auto & m_RelatedElement : m_RelatedElements)
	{
		shared_ptr<IfcAnnotation>  RelatedElements_IfcAnnotation = dynamic_pointer_cast<IfcAnnotation>( m_RelatedElement );
		if( RelatedElements_IfcAnnotation )
		{
			RelatedElements_IfcAnnotation->m_ContainedInStructure_inverse.push_back( ptr_self );
		}
		shared_ptr<IfcElement>  RelatedElements_IfcElement = dynamic_pointer_cast<IfcElement>( m_RelatedElement );
		if( RelatedElements_IfcElement )
		{
			RelatedElements_IfcElement->m_ContainedInStructure_inverse.push_back( ptr_self );
		}
		shared_ptr<IfcGrid>  RelatedElements_IfcGrid = dynamic_pointer_cast<IfcGrid>( m_RelatedElement );
