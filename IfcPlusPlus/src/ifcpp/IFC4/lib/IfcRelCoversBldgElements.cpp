...
IfcRelCoversBldgElements::IfcRelCoversBldgElements() = default;
...
shared_ptr<BuildingObject> IfcRelCoversBldgElements::getDeepCopy( BuildingCopyOptions& options )
{
	...
	for(auto item_ii : m_RelatedCoverings)
	{
			if( item_ii )
		{
...
void IfcRelCoversBldgElements::readStepArguments( const std::vector<std::wstring>& args, const std::map<int,shared_ptr<BuildingEntity> >& map )
{
	...
	if( num_args != 6 ){ std::stringstream err; err << "Wrong parameter count for entity IfcRelCoversBldgElements, expecting 6, having " << num_args << ". Entity ID: " << m_entity_id << std::endl; throw BuildingException( err.str() ); }
...
void IfcRelCoversBldgElements::getAttributes( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes ) const
{
	...
	vec_attributes.emplace_back( "RelatingBuildingElement", m_RelatingBuildingElement );
	if( !m_RelatedCoverings.empty() )
	{
		...
		vec_attributes.emplace_back( "RelatedCoverings", RelatedCoverings_vec_object );
	}
}
...
void IfcRelCoversBldgElements::setInverseCounterparts( shared_ptr<BuildingEntity> ptr_self_entity )
{
	...
	for(auto & m_RelatedCovering : m_RelatedCoverings)
	{
		if( m_RelatedCovering )
		{
			m_RelatedCovering->m_CoversElements_inverse.push_back( ptr_self );
		}
	}
...
void IfcRelCoversBldgElements::unlinkFromInverseCounterparts()
{
	...
	for(auto & m_RelatedCovering : m_RelatedCoverings)
	{
		if( m_RelatedCovering )
		{
			std::vector<weak_ptr<IfcRelCoversBldgElements> >& CoversElements_inverse = m_RelatedCovering->m_CoversElements_inverse;
