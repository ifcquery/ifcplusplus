...
IfcRelDeclares::IfcRelDeclares() = default;
...
shared_ptr<BuildingObject> IfcRelDeclares::getDeepCopy( BuildingCopyOptions& options )
{
	...
	for(auto item_ii : m_RelatedDefinitions)
	{
			if( item_ii )
		{
...
void IfcRelDeclares::readStepArguments( const std::vector<std::wstring>& args, const std::map<int,shared_ptr<BuildingEntity> >& map )
{
	...
	if( num_args != 6 ){ std::stringstream err; err << "Wrong parameter count for entity IfcRelDeclares, expecting 6, having " << num_args << ". Entity ID: " << m_entity_id << std::endl; throw BuildingException( err.str() ); }
...
void IfcRelDeclares::getAttributes( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes ) const
{
	...
	vec_attributes.emplace_back( "RelatingContext", m_RelatingContext );
	if( !m_RelatedDefinitions.empty() )
	{
		...
		vec_attributes.emplace_back( "RelatedDefinitions", RelatedDefinitions_vec_object );
	}
}
...
void IfcRelDeclares::setInverseCounterparts( shared_ptr<BuildingEntity> ptr_self_entity )
{
	...
	for(const auto & m_RelatedDefinition : m_RelatedDefinitions)
	{
		shared_ptr<IfcObjectDefinition>  RelatedDefinitions_IfcObjectDefinition = dynamic_pointer_cast<IfcObjectDefinition>( m_RelatedDefinition );
		if( RelatedDefinitions_IfcObjectDefinition )
		{
			RelatedDefinitions_IfcObjectDefinition->m_HasContext_inverse.push_back( ptr_self );
		}
		shared_ptr<IfcPropertyDefinition>  RelatedDefinitions_IfcPropertyDefinition = dynamic_pointer_cast<IfcPropertyDefinition>( m_RelatedDefinition );
