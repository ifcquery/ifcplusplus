...
IfcRelDefinesByObject::IfcRelDefinesByObject() = default;
...
shared_ptr<BuildingObject> IfcRelDefinesByObject::getDeepCopy( BuildingCopyOptions& options )
{
	...
	for(auto item_ii : m_RelatedObjects)
	{
			if( item_ii )
		{
...
void IfcRelDefinesByObject::readStepArguments( const std::vector<std::wstring>& args, const std::map<int,shared_ptr<BuildingEntity> >& map )
{
	...
	if( num_args != 6 ){ std::stringstream err; err << "Wrong parameter count for entity IfcRelDefinesByObject, expecting 6, having " << num_args << ". Entity ID: " << m_entity_id << std::endl; throw BuildingException( err.str() ); }
...
void IfcRelDefinesByObject::getAttributes( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes ) const
{
	...
	if( !m_RelatedObjects.empty() )
	{
		...
		vec_attributes.emplace_back( "RelatedObjects", RelatedObjects_vec_object );
	}
	vec_attributes.emplace_back( "RelatingObject", m_RelatingObject );
}
...
void IfcRelDefinesByObject::setInverseCounterparts( shared_ptr<BuildingEntity> ptr_self_entity )
{
	...
	for(auto & m_RelatedObject : m_RelatedObjects)
	{
		if( m_RelatedObject )
		{
			m_RelatedObject->m_IsDeclaredBy_inverse.push_back( ptr_self );
		}
	}
	if( m_RelatingObject )
	{
...
void IfcRelDefinesByObject::unlinkFromInverseCounterparts()
{
	...
	for(auto & m_RelatedObject : m_RelatedObjects)
	{
		if( m_RelatedObject )
		{
			std::vector<weak_ptr<IfcRelDefinesByObject> >& IsDeclaredBy_inverse = m_RelatedObject->m_IsDeclaredBy_inverse;
