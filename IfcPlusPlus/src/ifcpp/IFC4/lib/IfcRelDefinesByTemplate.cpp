...
IfcRelDefinesByTemplate::IfcRelDefinesByTemplate() = default;
...
shared_ptr<BuildingObject> IfcRelDefinesByTemplate::getDeepCopy( BuildingCopyOptions& options )
{
	...
	for(auto item_ii : m_RelatedPropertySets)
	{
			if( item_ii )
		{
...
void IfcRelDefinesByTemplate::readStepArguments( const std::vector<std::wstring>& args, const std::map<int,shared_ptr<BuildingEntity> >& map )
{
	...
	if( num_args != 6 ){ std::stringstream err; err << "Wrong parameter count for entity IfcRelDefinesByTemplate, expecting 6, having " << num_args << ". Entity ID: " << m_entity_id << std::endl; throw BuildingException( err.str() ); }
...
void IfcRelDefinesByTemplate::getAttributes( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes ) const
{
	...
	if( !m_RelatedPropertySets.empty() )
	{
		...
		vec_attributes.emplace_back( "RelatedPropertySets", RelatedPropertySets_vec_object );
	}
	vec_attributes.emplace_back( "RelatingTemplate", m_RelatingTemplate );
}
...
void IfcRelDefinesByTemplate::setInverseCounterparts( shared_ptr<BuildingEntity> ptr_self_entity )
{
	...
	for(auto & m_RelatedPropertySet : m_RelatedPropertySets)
	{
		if( m_RelatedPropertySet )
		{
			m_RelatedPropertySet->m_IsDefinedBy_inverse.push_back( ptr_self );
		}
	}
...
void IfcRelDefinesByTemplate::unlinkFromInverseCounterparts()
{
	...
	for(auto & m_RelatedPropertySet : m_RelatedPropertySets)
	{
		if( m_RelatedPropertySet )
		{
			std::vector<weak_ptr<IfcRelDefinesByTemplate> >& IsDefinedBy_inverse = m_RelatedPropertySet->m_IsDefinedBy_inverse;
