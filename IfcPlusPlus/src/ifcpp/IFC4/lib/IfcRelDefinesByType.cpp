...
IfcRelDefinesByType::IfcRelDefinesByType() = default;
...
void IfcRelDefinesByType::readStepArguments( const std::vector<std::wstring>& args, const std::map<int,shared_ptr<BuildingEntity> >& map )
{
	...
	if( num_args != 6 ){ std::stringstream err; err << "Wrong parameter count for entity IfcRelDefinesByType, expecting 6, having " << num_args << ". Entity ID: " << m_entity_id << std::endl; throw BuildingException( err.str() ); }
...
void IfcRelDefinesByType::getAttributes( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes ) const
{
	...
	vec_attributes.emplace_back( "RelatingType", m_RelatingType );
}
...
void IfcRelDefinesByType::setInverseCounterparts( shared_ptr<BuildingEntity> ptr_self_entity )
{
	...
	for(auto & m_RelatedObject : m_RelatedObjects)
	{
		if( m_RelatedObject )
		{
			m_RelatedObject->m_IsTypedBy_inverse.push_back( ptr_self );
		}
	}
	if( m_RelatingType )
	{
...
void IfcRelDefinesByType::unlinkFromInverseCounterparts()
{
	...
	for(auto & m_RelatedObject : m_RelatedObjects)
	{
		if( m_RelatedObject )
		{
			std::vector<weak_ptr<IfcRelDefinesByType> >& IsTypedBy_inverse = m_RelatedObject->m_IsTypedBy_inverse;
