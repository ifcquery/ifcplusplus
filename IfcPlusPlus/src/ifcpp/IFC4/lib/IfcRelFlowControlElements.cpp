...
IfcRelFlowControlElements::IfcRelFlowControlElements() = default;
...
shared_ptr<BuildingObject> IfcRelFlowControlElements::getDeepCopy( BuildingCopyOptions& options )
{
	...
	for(auto item_ii : m_RelatedControlElements)
	{
			if( item_ii )
		{
...
void IfcRelFlowControlElements::readStepArguments( const std::vector<std::wstring>& args, const std::map<int,shared_ptr<BuildingEntity> >& map )
{
	...
	if( num_args != 6 ){ std::stringstream err; err << "Wrong parameter count for entity IfcRelFlowControlElements, expecting 6, having " << num_args << ". Entity ID: " << m_entity_id << std::endl; throw BuildingException( err.str() ); }
...
void IfcRelFlowControlElements::getAttributes( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes ) const
{
	...
	if( !m_RelatedControlElements.empty() )
	{
		...
		vec_attributes.emplace_back( "RelatedControlElements", RelatedControlElements_vec_object );
	}
	vec_attributes.emplace_back( "RelatingFlowElement", m_RelatingFlowElement );
}
...
void IfcRelFlowControlElements::setInverseCounterparts( shared_ptr<BuildingEntity> ptr_self_entity )
{
	...
	for(auto & m_RelatedControlElement : m_RelatedControlElements)
	{
		if( m_RelatedControlElement )
		{
			m_RelatedControlElement->m_AssignedToFlowElement_inverse.push_back( ptr_self );
		}
	}
...
void IfcRelFlowControlElements::unlinkFromInverseCounterparts()
{
	...
	for(auto & m_RelatedControlElement : m_RelatedControlElements)
	{
		if( m_RelatedControlElement )
		{
			std::vector<weak_ptr<IfcRelFlowControlElements> >& AssignedToFlowElement_inverse = m_RelatedControlElement->m_AssignedToFlowElement_inverse;
