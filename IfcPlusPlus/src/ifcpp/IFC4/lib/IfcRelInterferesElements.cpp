...
IfcRelInterferesElements::IfcRelInterferesElements() = default;
...
void IfcRelInterferesElements::readStepArguments( const std::vector<std::wstring>& args, const std::map<int,shared_ptr<BuildingEntity> >& map )
{
	...
	if( num_args != 9 ){ std::stringstream err; err << "Wrong parameter count for entity IfcRelInterferesElements, expecting 9, having " << num_args << ". Entity ID: " << m_entity_id << std::endl; throw BuildingException( err.str() ); }
...
void IfcRelInterferesElements::getAttributes( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes ) const
{
	...
	vec_attributes.emplace_back( "RelatedElement", m_RelatedElement );
	vec_attributes.emplace_back( "InterferenceGeometry", m_InterferenceGeometry );
	vec_attributes.emplace_back( "InterferenceType", m_InterferenceType );
	vec_attributes.emplace_back( "ImpliedOrder", std::make_shared<LogicalAttribute>( m_ImpliedOrder ) );
}
