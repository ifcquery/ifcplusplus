...
IfcRelPositions::IfcRelPositions( int id ) { m_entity_id = id; }
shared_ptr<BuildingObject> IfcRelPositions::getDeepCopy( BuildingCopyOptions& options )
{
...
void IfcRelPositions::getStepLine( std::stringstream& stream ) const
{
	stream << "#" << m_entity_id << "= IFCRELPOSITIONS" << "(";
	...
	stream << ",";
	if( m_RelatingPositioningElement ) { stream << "#" << m_RelatingPositioningElement->m_entity_id; } else { stream << "$"; }
	stream << ",";
	writeEntityList( stream, m_RelatedProducts );
	stream << ");";
}
void IfcRelPositions::getStepParameter( std::stringstream& stream, bool /*is_select_type*/ ) const { stream << "#" << m_entity_id; }
const std::wstring IfcRelPositions::toString() const { return L"IfcRelPositions"; }
void IfcRelPositions::readStepArguments( const std::vector<std::wstring>& args, const std::map<int,shared_ptr<BuildingEntity> >& map )
{
	const size_t num_args = args.size();
	if( num_args != 6 ){ std::stringstream err; err << "Wrong parameter count for entity IfcRelPositions, expecting 6, having " << num_args << ". Entity ID: " << m_entity_id << std::endl; throw BuildingException( err.str().c_str() ); }
	m_GlobalId = IfcGloballyUniqueId::createObjectFromSTEP( args[0], map );
	readEntityReference( args[1], m_OwnerHistory, map );
	m_Name = IfcLabel::createObjectFromSTEP( args[2], map );
	m_Description = IfcText::createObjectFromSTEP( args[3], map );
	readEntityReference( args[4], m_RelatingPositioningElement, map );
	readEntityReferenceList( args[5], m_RelatedProducts, map );
}
void IfcRelPositions::getAttributes( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes ) const
{
...
void IfcRelPositions::getAttributesInverse( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes_inverse ) const
{
	IfcRelConnects::getAttributesInverse( vec_attributes_inverse );
}
void IfcRelPositions::setInverseCounterparts( shared_ptr<BuildingEntity> ptr_self_entity )
{
...
void IfcRelPositions::unlinkFromInverseCounterparts()
{
