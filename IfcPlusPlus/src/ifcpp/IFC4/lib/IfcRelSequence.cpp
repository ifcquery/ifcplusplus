...
IfcRelSequence::IfcRelSequence() = default;
...
void IfcRelSequence::readStepArguments( const std::vector<std::wstring>& args, const std::map<int,shared_ptr<BuildingEntity> >& map )
{
	...
	if( num_args != 9 ){ std::stringstream err; err << "Wrong parameter count for entity IfcRelSequence, expecting 9, having " << num_args << ". Entity ID: " << m_entity_id << std::endl; throw BuildingException( err.str() ); }
...
void IfcRelSequence::getAttributes( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes ) const
{
	...
	vec_attributes.emplace_back( "RelatingProcess", m_RelatingProcess );
	vec_attributes.emplace_back( "RelatedProcess", m_RelatedProcess );
	vec_attributes.emplace_back( "TimeLag", m_TimeLag );
	vec_attributes.emplace_back( "SequenceType", m_SequenceType );
	vec_attributes.emplace_back( "UserDefinedSequenceType", m_UserDefinedSequenceType );
}
