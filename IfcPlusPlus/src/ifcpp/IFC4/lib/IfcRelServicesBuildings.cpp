...
IfcRelServicesBuildings::IfcRelServicesBuildings() = default;
...
shared_ptr<BuildingObject> IfcRelServicesBuildings::getDeepCopy( BuildingCopyOptions& options )
{
	...
	for(auto item_ii : m_RelatedBuildings)
	{
			if( item_ii )
		{
...
void IfcRelServicesBuildings::readStepArguments( const std::vector<std::wstring>& args, const std::map<int,shared_ptr<BuildingEntity> >& map )
{
	...
	if( num_args != 6 ){ std::stringstream err; err << "Wrong parameter count for entity IfcRelServicesBuildings, expecting 6, having " << num_args << ". Entity ID: " << m_entity_id << std::endl; throw BuildingException( err.str() ); }
...
void IfcRelServicesBuildings::getAttributes( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes ) const
{
	...
	vec_attributes.emplace_back( "RelatingSystem", m_RelatingSystem );
	if( !m_RelatedBuildings.empty() )
	{
		...
		vec_attributes.emplace_back( "RelatedBuildings", RelatedBuildings_vec_object );
	}
}
...
void IfcRelServicesBuildings::setInverseCounterparts( shared_ptr<BuildingEntity> ptr_self_entity )
{
	...
	for(auto & m_RelatedBuilding : m_RelatedBuildings)
	{
		if( m_RelatedBuilding )
		{
			m_RelatedBuilding->m_ServicedBySystems_inverse.push_back( ptr_self );
		}
	}
	if( m_RelatingSystem )
	{
...
void IfcRelServicesBuildings::unlinkFromInverseCounterparts()
{
	...
	for(auto & m_RelatedBuilding : m_RelatedBuildings)
	{
		if( m_RelatedBuilding )
		{
			std::vector<weak_ptr<IfcRelServicesBuildings> >& ServicedBySystems_inverse = m_RelatedBuilding->m_ServicedBySystems_inverse;
