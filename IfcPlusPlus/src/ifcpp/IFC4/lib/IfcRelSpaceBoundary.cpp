...
IfcRelSpaceBoundary::IfcRelSpaceBoundary() = default;
...
void IfcRelSpaceBoundary::readStepArguments( const std::vector<std::wstring>& args, const std::map<int,shared_ptr<BuildingEntity> >& map )
{
	...
	if( num_args != 9 ){ std::stringstream err; err << "Wrong parameter count for entity IfcRelSpaceBoundary, expecting 9, having " << num_args << ". Entity ID: " << m_entity_id << std::endl; throw BuildingException( err.str() ); }
...
void IfcRelSpaceBoundary::getAttributes( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes ) const
{
	...
	vec_attributes.emplace_back( "RelatedBuildingElement", m_RelatedBuildingElement );
	vec_attributes.emplace_back( "ConnectionGeometry", m_ConnectionGeometry );
	vec_attributes.emplace_back( "PhysicalOrVirtualBoundary", m_PhysicalOrVirtualBoundary );
	vec_attributes.emplace_back( "InternalOrExternalBoundary", m_InternalOrExternalBoundary );
}
