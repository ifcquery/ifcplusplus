...
IfcRelSpaceBoundary1stLevel::IfcRelSpaceBoundary1stLevel() = default;
...
void IfcRelSpaceBoundary1stLevel::readStepArguments( const std::vector<std::wstring>& args, const std::map<int,shared_ptr<BuildingEntity> >& map )
{
	...
	if( num_args != 10 ){ std::stringstream err; err << "Wrong parameter count for entity IfcRelSpaceBoundary1stLevel, expecting 10, having " << num_args << ". Entity ID: " << m_entity_id << std::endl; throw BuildingException( err.str() ); }
...
void IfcRelSpaceBoundary1stLevel::getAttributes( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes ) const
{
	...
	vec_attributes.emplace_back( "ParentBoundary", m_ParentBoundary );
}
void IfcRelSpaceBoundary1stLevel::getAttributesInverse( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes_inverse ) const
{
	...
	if( !m_InnerBoundaries_inverse.empty() )
	{
		...
		for(const auto & i : m_InnerBoundaries_inverse)
		{
			if( !i.expired() )
			{
				InnerBoundaries_inverse_vec_obj->m_vec.push_back( shared_ptr<IfcRelSpaceBoundary1stLevel>( i ) );
			}
		}
		vec_attributes_inverse.emplace_back( "InnerBoundaries_inverse", InnerBoundaries_inverse_vec_obj );
	}
}
