...
IfcRelSpaceBoundary2ndLevel::IfcRelSpaceBoundary2ndLevel() = default;
...
void IfcRelSpaceBoundary2ndLevel::readStepArguments( const std::vector<std::wstring>& args, const std::map<int,shared_ptr<BuildingEntity> >& map )
{
	...
	if( num_args != 11 ){ std::stringstream err; err << "Wrong parameter count for entity IfcRelSpaceBoundary2ndLevel, expecting 11, having " << num_args << ". Entity ID: " << m_entity_id << std::endl; throw BuildingException( err.str() ); }
...
void IfcRelSpaceBoundary2ndLevel::getAttributes( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes ) const
{
	...
	vec_attributes.emplace_back( "CorrespondingBoundary", m_CorrespondingBoundary );
}
void IfcRelSpaceBoundary2ndLevel::getAttributesInverse( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes_inverse ) const
{
	...
	if( !m_Corresponds_inverse.empty() )
	{
		...
		for(const auto & i : m_Corresponds_inverse)
		{
			if( !i.expired() )
			{
				Corresponds_inverse_vec_obj->m_vec.push_back( shared_ptr<IfcRelSpaceBoundary2ndLevel>( i ) );
			}
		}
		vec_attributes_inverse.emplace_back( "Corresponds_inverse", Corresponds_inverse_vec_obj );
	}
}
