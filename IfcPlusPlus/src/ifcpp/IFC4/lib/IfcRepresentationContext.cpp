...
IfcRepresentationContext::IfcRepresentationContext() = default;
...
void IfcRepresentationContext::readStepArguments( const std::vector<std::wstring>& args, const std::map<int,shared_ptr<BuildingEntity> >& map )
{
	...
	if( num_args != 2 ){ std::stringstream err; err << "Wrong parameter count for entity IfcRepresentationContext, expecting 2, having " << num_args << ". Entity ID: " << m_entity_id << std::endl; throw BuildingException( err.str() ); }
...
{
	vec_attributes.emplace_back( "ContextIdentifier", m_ContextIdentifier );
	vec_attributes.emplace_back( "ContextType", m_ContextType );
}
void IfcRepresentationContext::getAttributesInverse( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes_inverse ) const
{
	if( !m_RepresentationsInContext_inverse.empty() )
	{
		...
		for(const auto & i : m_RepresentationsInContext_inverse)
		{
			if( !i.expired() )
			{
				RepresentationsInContext_inverse_vec_obj->m_vec.push_back( shared_ptr<IfcRepresentation>( i ) );
			}
		}
		vec_attributes_inverse.emplace_back( "RepresentationsInContext_inverse", RepresentationsInContext_inverse_vec_obj );
	}
}
void IfcRepresentationContext::setInverseCounterparts( shared_ptr<BuildingEntity>  /*ptr_self*/)
{
}
