...
void IfcRoot::readStepArguments( const std::vector<std::wstring>& args, const std::map<int,shared_ptr<BuildingEntity> >& map )
{
	...
	if( num_args != 4 ){ std::stringstream err; err << "Wrong parameter count for entity IfcRoot, expecting 4, having " << num_args << ". Entity ID: " << m_entity_id << std::endl; throw BuildingException( err.str() ); }
...
{
	vec_attributes.emplace_back( "GlobalId", m_GlobalId );
	vec_attributes.emplace_back( "OwnerHistory", m_OwnerHistory );
...
void IfcRoot::setInverseCounterparts( shared_ptr<BuildingEntity>  /*ptr_self*/)
{
}
