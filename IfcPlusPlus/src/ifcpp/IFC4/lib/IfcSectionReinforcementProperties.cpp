...
IfcSectionReinforcementProperties::IfcSectionReinforcementProperties() = default;
...
shared_ptr<BuildingObject> IfcSectionReinforcementProperties::getDeepCopy( BuildingCopyOptions& options )
{
	...
	for(auto item_ii : m_CrossSectionReinforcementDefinitions)
	{
			if( item_ii )
		{
...
void IfcSectionReinforcementProperties::readStepArguments( const std::vector<std::wstring>& args, const std::map<int,shared_ptr<BuildingEntity> >& map )
{
	...
	if( num_args != 6 ){ std::stringstream err; err << "Wrong parameter count for entity IfcSectionReinforcementProperties, expecting 6, having " << num_args << ". Entity ID: " << m_entity_id << std::endl; throw BuildingException( err.str() ); }
...
void IfcSectionReinforcementProperties::getAttributes( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes ) const
{
	...
	vec_attributes.emplace_back( "LongitudinalStartPosition", m_LongitudinalStartPosition );
	vec_attributes.emplace_back( "LongitudinalEndPosition", m_LongitudinalEndPosition );
	vec_attributes.emplace_back( "TransversePosition", m_TransversePosition );
	vec_attributes.emplace_back( "ReinforcementRole", m_ReinforcementRole );
	vec_attributes.emplace_back( "SectionDefinition", m_SectionDefinition );
	if( !m_CrossSectionReinforcementDefinitions.empty() )
	{
		...
		vec_attributes.emplace_back( "CrossSectionReinforcementDefinitions", CrossSectionReinforcementDefinitions_vec_object );
	}
}
