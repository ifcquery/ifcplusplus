...
// ENTITY IfcSign 
IfcSign::IfcSign( int id ) { m_entity_id = id; }
shared_ptr<BuildingObject> IfcSign::getDeepCopy( BuildingCopyOptions& options )
{
	shared_ptr<IfcSign> copy_self( new IfcSign() );
	if( m_GlobalId )
	{
...
void IfcSign::getStepLine( std::stringstream& stream ) const
{
	stream << "#" << m_entity_id << "= IFCSIGN" << "(";
...
void IfcSign::getStepParameter( std::stringstream& stream, bool /*is_select_type*/ ) const { stream << "#" << m_entity_id; }
const std::wstring IfcSign::toString() const { return L"IfcSign"; }
void IfcSign::readStepArguments( const std::vector<std::wstring>& args, const std::map<int,shared_ptr<BuildingEntity> >& map )
{
	const size_t num_args = args.size();
	if( num_args != 9 ){ std::stringstream err; err << "Wrong parameter count for entity IfcSign, expecting 9, having " << num_args << ". Entity ID: " << m_entity_id << std::endl; throw BuildingException( err.str().c_str() ); }
	...
	m_PredefinedType = IfcSignTypeEnum::createObjectFromSTEP( args[8], map );
}
void IfcSign::getAttributes( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes ) const
{
	IfcElementComponent::getAttributes( vec_attributes );
	vec_attributes.emplace_back( std::make_pair( "PredefinedType", m_PredefinedType ) );
}
void IfcSign::getAttributesInverse( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes_inverse ) const
{
	IfcElementComponent::getAttributesInverse( vec_attributes_inverse );
}
void IfcSign::setInverseCounterparts( shared_ptr<BuildingEntity> ptr_self_entity )
{
	IfcElementComponent::setInverseCounterparts( ptr_self_entity );
}
void IfcSign::unlinkFromInverseCounterparts()
{
