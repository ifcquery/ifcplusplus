...
// ENTITY IfcSignType 
IfcSignType::IfcSignType( int id ) { m_entity_id = id; }
shared_ptr<BuildingObject> IfcSignType::getDeepCopy( BuildingCopyOptions& options )
{
	shared_ptr<IfcSignType> copy_self( new IfcSignType() );
	if( m_GlobalId )
	{
	...
	if( m_PredefinedType ) { copy_self->m_PredefinedType = dynamic_pointer_cast<IfcSignTypeEnum>( m_PredefinedType->getDeepCopy(options) ); }
	return copy_self;
}
void IfcSignType::getStepLine( std::stringstream& stream ) const
{
	stream << "#" << m_entity_id << "= IFCSIGNTYPE" << "(";
...
void IfcSignType::getStepParameter( std::stringstream& stream, bool /*is_select_type*/ ) const { stream << "#" << m_entity_id; }
const std::wstring IfcSignType::toString() const { return L"IfcSignType"; }
void IfcSignType::readStepArguments( const std::vector<std::wstring>& args, const std::map<int,shared_ptr<BuildingEntity> >& map )
{
	const size_t num_args = args.size();
	if( num_args != 10 ){ std::stringstream err; err << "Wrong parameter count for entity IfcSignType, expecting 10, having " << num_args << ". Entity ID: " << m_entity_id << std::endl; throw BuildingException( err.str().c_str() ); }
	...
	m_PredefinedType = IfcSignTypeEnum::createObjectFromSTEP( args[9], map );
}
void IfcSignType::getAttributes( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes ) const
{
	IfcElementComponentType::getAttributes( vec_attributes );
	vec_attributes.emplace_back( std::make_pair( "PredefinedType", m_PredefinedType ) );
}
void IfcSignType::getAttributesInverse( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes_inverse ) const
{
	IfcElementComponentType::getAttributesInverse( vec_attributes_inverse );
}
void IfcSignType::setInverseCounterparts( shared_ptr<BuildingEntity> ptr_self_entity )
{
	IfcElementComponentType::setInverseCounterparts( ptr_self_entity );
}
void IfcSignType::unlinkFromInverseCounterparts()
{
