...
// ENTITY IfcSignal 
IfcSignal::IfcSignal( int id ) { m_entity_id = id; }
shared_ptr<BuildingObject> IfcSignal::getDeepCopy( BuildingCopyOptions& options )
{
	shared_ptr<IfcSignal> copy_self( new IfcSignal() );
	if( m_GlobalId )
	{
...
void IfcSignal::getStepLine( std::stringstream& stream ) const
{
	stream << "#" << m_entity_id << "= IFCSIGNAL" << "(";
...
void IfcSignal::getStepParameter( std::stringstream& stream, bool /*is_select_type*/ ) const { stream << "#" << m_entity_id; }
const std::wstring IfcSignal::toString() const { return L"IfcSignal"; }
void IfcSignal::readStepArguments( const std::vector<std::wstring>& args, const std::map<int,shared_ptr<BuildingEntity> >& map )
{
	const size_t num_args = args.size();
	if( num_args != 9 ){ std::stringstream err; err << "Wrong parameter count for entity IfcSignal, expecting 9, having " << num_args << ". Entity ID: " << m_entity_id << std::endl; throw BuildingException( err.str().c_str() ); }
	...
	m_PredefinedType = IfcSignalTypeEnum::createObjectFromSTEP( args[8], map );
}
void IfcSignal::getAttributes( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes ) const
{
	IfcFlowTerminal::getAttributes( vec_attributes );
	vec_attributes.emplace_back( std::make_pair( "PredefinedType", m_PredefinedType ) );
}
void IfcSignal::getAttributesInverse( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes_inverse ) const
{
	IfcFlowTerminal::getAttributesInverse( vec_attributes_inverse );
}
void IfcSignal::setInverseCounterparts( shared_ptr<BuildingEntity> ptr_self_entity )
{
	IfcFlowTerminal::setInverseCounterparts( ptr_self_entity );
}
void IfcSignal::unlinkFromInverseCounterparts()
{
