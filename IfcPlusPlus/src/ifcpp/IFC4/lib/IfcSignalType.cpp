...
IfcSignalType::IfcSignalType( int id ) { m_entity_id = id; }
shared_ptr<BuildingObject> IfcSignalType::getDeepCopy( BuildingCopyOptions& options )
{
	shared_ptr<IfcSignalType> copy_self( new IfcSignalType() );
	if( m_GlobalId )
	{
	...
	if( m_PredefinedType ) { copy_self->m_PredefinedType = dynamic_pointer_cast<IfcSignalTypeEnum>( m_PredefinedType->getDeepCopy(options) ); }
	return copy_self;
}
void IfcSignalType::getStepLine( std::stringstream& stream ) const
{
	stream << "#" << m_entity_id << "= IFCSIGNALTYPE" << "(";
...
void IfcSignalType::getStepParameter( std::stringstream& stream, bool /*is_select_type*/ ) const { stream << "#" << m_entity_id; }
const std::wstring IfcSignalType::toString() const { return L"IfcSignalType"; }
void IfcSignalType::readStepArguments( const std::vector<std::wstring>& args, const std::map<int,shared_ptr<BuildingEntity> >& map )
{
	const size_t num_args = args.size();
	if( num_args != 10 ){ std::stringstream err; err << "Wrong parameter count for entity IfcSignalType, expecting 10, having " << num_args << ". Entity ID: " << m_entity_id << std::endl; throw BuildingException( err.str().c_str() ); }
	...
	m_PredefinedType = IfcSignalTypeEnum::createObjectFromSTEP( args[9], map );
}
void IfcSignalType::getAttributes( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes ) const
{
	IfcFlowTerminalType::getAttributes( vec_attributes );
	vec_attributes.emplace_back( std::make_pair( "PredefinedType", m_PredefinedType ) );
}
void IfcSignalType::getAttributesInverse( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes_inverse ) const
{
	IfcFlowTerminalType::getAttributesInverse( vec_attributes_inverse );
}
void IfcSignalType::setInverseCounterparts( shared_ptr<BuildingEntity> ptr_self_entity )
{
	IfcFlowTerminalType::setInverseCounterparts( ptr_self_entity );
}
void IfcSignalType::unlinkFromInverseCounterparts()
{
