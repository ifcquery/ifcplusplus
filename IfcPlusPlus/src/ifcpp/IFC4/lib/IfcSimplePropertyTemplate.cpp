...
IfcSimplePropertyTemplate::IfcSimplePropertyTemplate() = default;
...
void IfcSimplePropertyTemplate::readStepArguments( const std::vector<std::wstring>& args, const std::map<int,shared_ptr<BuildingEntity> >& map )
{
	...
	if( num_args != 12 ){ std::stringstream err; err << "Wrong parameter count for entity IfcSimplePropertyTemplate, expecting 12, having " << num_args << ". Entity ID: " << m_entity_id << std::endl; throw BuildingException( err.str() ); }
...
void IfcSimplePropertyTemplate::getAttributes( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes ) const
{
	...
	vec_attributes.emplace_back( "PrimaryMeasureType", m_PrimaryMeasureType );
	vec_attributes.emplace_back( "SecondaryMeasureType", m_SecondaryMeasureType );
	vec_attributes.emplace_back( "Enumerators", m_Enumerators );
	vec_attributes.emplace_back( "PrimaryUnit", m_PrimaryUnit );
	vec_attributes.emplace_back( "SecondaryUnit", m_SecondaryUnit );
	vec_attributes.emplace_back( "Expression", m_Expression );
	vec_attributes.emplace_back( "AccessState", m_AccessState );
}
