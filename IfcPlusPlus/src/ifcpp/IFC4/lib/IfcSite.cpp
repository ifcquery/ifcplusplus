...
void IfcSite::readStepArguments( const std::vector<std::wstring>& args, const std::map<int,shared_ptr<BuildingEntity> >& map )
{
	...
	if( num_args != 14 ){ std::stringstream err; err << "Wrong parameter count for entity IfcSite, expecting 14, having " << num_args << ". Entity ID: " << m_entity_id << std::endl; throw BuildingException( err.str() ); }
...
void IfcSite::getAttributes( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes ) const
{
	...
	vec_attributes.emplace_back( "RefLatitude", m_RefLatitude );
	vec_attributes.emplace_back( "RefLongitude", m_RefLongitude );
	vec_attributes.emplace_back( "RefElevation", m_RefElevation );
	vec_attributes.emplace_back( "LandTitleNumber", m_LandTitleNumber );
	vec_attributes.emplace_back( "SiteAddress", m_SiteAddress );
}
