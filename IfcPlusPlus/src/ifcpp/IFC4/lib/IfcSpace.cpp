...
void IfcSpace::readStepArguments( const std::vector<std::wstring>& args, const std::map<int,shared_ptr<BuildingEntity> >& map )
{
	...
	if( num_args != 11 ){ std::stringstream err; err << "Wrong parameter count for entity IfcSpace, expecting 11, having " << num_args << ". Entity ID: " << m_entity_id << std::endl; throw BuildingException( err.str() ); }
...
void IfcSpace::getAttributes( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes ) const
{
	...
	vec_attributes.emplace_back( "ElevationWithFlooring", m_ElevationWithFlooring );
}
void IfcSpace::getAttributesInverse( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes_inverse ) const
{
	...
	if( !m_HasCoverings_inverse.empty() )
	{
		...
		for(const auto & i : m_HasCoverings_inverse)
		{
			if( !i.expired() )
			{
				HasCoverings_inverse_vec_obj->m_vec.push_back( shared_ptr<IfcRelCoversSpaces>( i ) );
			}
		}
	...
	if( !m_BoundedBy_inverse.empty() )
	{
		...
		for(const auto & i : m_BoundedBy_inverse)
		{
			if( !i.expired() )
			{
				BoundedBy_inverse_vec_obj->m_vec.push_back( shared_ptr<IfcRelSpaceBoundary>( i ) );
			}
		}
		vec_attributes_inverse.emplace_back( "BoundedBy_inverse", BoundedBy_inverse_vec_obj );
	}
}
