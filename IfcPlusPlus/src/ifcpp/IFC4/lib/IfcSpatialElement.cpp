...
IfcSpatialElement::IfcSpatialElement() = default;
...
void IfcSpatialElement::readStepArguments( const std::vector<std::wstring>& args, const std::map<int,shared_ptr<BuildingEntity> >& map )
{
	...
	if( num_args != 8 ){ std::stringstream err; err << "Wrong parameter count for entity IfcSpatialElement, expecting 8, having " << num_args << ". Entity ID: " << m_entity_id << std::endl; throw BuildingException( err.str() ); }
...
void IfcSpatialElement::getAttributes( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes ) const
{
	...
	vec_attributes.emplace_back( "LongName", m_LongName );
}
void IfcSpatialElement::getAttributesInverse( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes_inverse ) const
{
	...
	if( !m_ContainsElements_inverse.empty() )
	{
		...
		for(const auto & i : m_ContainsElements_inverse)
		{
			if( !i.expired() )
			{
				ContainsElements_inverse_vec_obj->m_vec.push_back( shared_ptr<IfcRelContainedInSpatialStructure>( i ) );
			}
		}
		vec_attributes_inverse.emplace_back( "ContainsElements_inverse", ContainsElements_inverse_vec_obj );
	}
	if( !m_ServicedBySystems_inverse.empty() )
	{
		...
		for(const auto & i : m_ServicedBySystems_inverse)
		{
			if( !i.expired() )
			{
				ServicedBySystems_inverse_vec_obj->m_vec.push_back( shared_ptr<IfcRelServicesBuildings>( i ) );
			}
		}
		vec_attributes_inverse.emplace_back( "ServicedBySystems_inverse", ServicedBySystems_inverse_vec_obj );
	}
	if( !m_ReferencesElements_inverse.empty() )
	{
		...
		for(const auto & i : m_ReferencesElements_inverse)
		{
			if( !i.expired() )
			{
				ReferencesElements_inverse_vec_obj->m_vec.push_back( shared_ptr<IfcRelReferencedInSpatialStructure>( i ) );
			}
		}
		vec_attributes_inverse.emplace_back( "ReferencesElements_inverse", ReferencesElements_inverse_vec_obj );
	}
}
