...
IfcStructuralActivity::IfcStructuralActivity() = default;
...
void IfcStructuralActivity::readStepArguments( const std::vector<std::wstring>& args, const std::map<int,shared_ptr<BuildingEntity> >& map )
{
	...
	if( num_args != 9 ){ std::stringstream err; err << "Wrong parameter count for entity IfcStructuralActivity, expecting 9, having " << num_args << ". Entity ID: " << m_entity_id << std::endl; throw BuildingException( err.str() ); }
...
void IfcStructuralActivity::getAttributes( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes ) const
{
	...
	vec_attributes.emplace_back( "AppliedLoad", m_AppliedLoad );
	vec_attributes.emplace_back( "GlobalOrLocal", m_GlobalOrLocal );
}
void IfcStructuralActivity::getAttributesInverse( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes_inverse ) const
{
	...
	if( !m_AssignedToStructuralItem_inverse.empty() )
	{
		...
		for(const auto & i : m_AssignedToStructuralItem_inverse)
		{
			if( !i.expired() )
			{
				AssignedToStructuralItem_inverse_vec_obj->m_vec.push_back( shared_ptr<IfcRelConnectsStructuralActivity>( i ) );
			}
		}
		vec_attributes_inverse.emplace_back( "AssignedToStructuralItem_inverse", AssignedToStructuralItem_inverse_vec_obj );
	}
}
