...
IfcStructuralAnalysisModel::IfcStructuralAnalysisModel() = default;
...
void IfcStructuralAnalysisModel::readStepArguments( const std::vector<std::wstring>& args, const std::map<int,shared_ptr<BuildingEntity> >& map )
{
	...
	if( num_args != 10 ){ std::stringstream err; err << "Wrong parameter count for entity IfcStructuralAnalysisModel, expecting 10, having " << num_args << ". Entity ID: " << m_entity_id << std::endl; throw BuildingException( err.str() ); }
...
void IfcStructuralAnalysisModel::getAttributes( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes ) const
{
	...
	vec_attributes.emplace_back( "OrientationOf2DPlane", m_OrientationOf2DPlane );
	if( !m_LoadedBy.empty() )
	{
		...
		vec_attributes.emplace_back( "LoadedBy", LoadedBy_vec_object );
	}
	if( !m_HasResults.empty() )
	{
		...
		vec_attributes.emplace_back( "HasResults", HasResults_vec_object );
	}
	vec_attributes.emplace_back( "SharedPlacement", m_SharedPlacement );
}
...
void IfcStructuralAnalysisModel::setInverseCounterparts( shared_ptr<BuildingEntity> ptr_self_entity )
{
	...
	for(auto & m_HasResult : m_HasResults)
	{
		if( m_HasResult )
		{
			m_HasResult->m_ResultGroupFor_inverse.push_back( ptr_self );
		}
	}
	for(auto & i : m_LoadedBy)
	{
		if( i )
		{
			i->m_LoadGroupFor_inverse.push_back( ptr_self );
		}
	}
}
void IfcStructuralAnalysisModel::unlinkFromInverseCounterparts()
{
	...
	for(auto & m_HasResult : m_HasResults)
	{
		if( m_HasResult )
		{
			std::vector<weak_ptr<IfcStructuralAnalysisModel> >& ResultGroupFor_inverse = m_HasResult->m_ResultGroupFor_inverse;
	...
	for(auto & i : m_LoadedBy)
	{
		if( i )
		{
			std::vector<weak_ptr<IfcStructuralAnalysisModel> >& LoadGroupFor_inverse = i->m_LoadGroupFor_inverse;
