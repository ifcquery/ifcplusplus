...
IfcStructuralConnection::IfcStructuralConnection() = default;
...
void IfcStructuralConnection::readStepArguments( const std::vector<std::wstring>& args, const std::map<int,shared_ptr<BuildingEntity> >& map )
{
	...
	if( num_args != 8 ){ std::stringstream err; err << "Wrong parameter count for entity IfcStructuralConnection, expecting 8, having " << num_args << ". Entity ID: " << m_entity_id << std::endl; throw BuildingException( err.str() ); }
...
void IfcStructuralConnection::getAttributesInverse( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes_inverse ) const
{
	...
	if( !m_ConnectsStructuralMembers_inverse.empty() )
	{
		...
		for(const auto & i : m_ConnectsStructuralMembers_inverse)
		{
			if( !i.expired() )
			{
				ConnectsStructuralMembers_inverse_vec_obj->m_vec.push_back( shared_ptr<IfcRelConnectsStructuralMember>( i ) );
			}
		}
		vec_attributes_inverse.emplace_back( "ConnectsStructuralMembers_inverse", ConnectsStructuralMembers_inverse_vec_obj );
	}
}
