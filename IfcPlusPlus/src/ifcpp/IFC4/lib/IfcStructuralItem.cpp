...
IfcStructuralItem::IfcStructuralItem() = default;
...
void IfcStructuralItem::readStepArguments( const std::vector<std::wstring>& args, const std::map<int,shared_ptr<BuildingEntity> >& map )
{
	...
	if( num_args != 7 ){ std::stringstream err; err << "Wrong parameter count for entity IfcStructuralItem, expecting 7, having " << num_args << ". Entity ID: " << m_entity_id << std::endl; throw BuildingException( err.str() ); }
...
void IfcStructuralItem::getAttributesInverse( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes_inverse ) const
{
	...
	if( !m_AssignedStructuralActivity_inverse.empty() )
	{
		...
		for(const auto & i : m_AssignedStructuralActivity_inverse)
		{
			if( !i.expired() )
			{
				AssignedStructuralActivity_inverse_vec_obj->m_vec.push_back( shared_ptr<IfcRelConnectsStructuralActivity>( i ) );
			}
		}
		vec_attributes_inverse.emplace_back( "AssignedStructuralActivity_inverse", AssignedStructuralActivity_inverse_vec_obj );
	}
}
