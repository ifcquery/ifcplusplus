...
IfcStructuralLoadCase::IfcStructuralLoadCase() = default;
...
shared_ptr<BuildingObject> IfcStructuralLoadCase::getDeepCopy( BuildingCopyOptions& options )
{
	...
	for(auto item_ii : m_SelfWeightCoefficients)
	{
			if( item_ii )
		{
...
void IfcStructuralLoadCase::readStepArguments( const std::vector<std::wstring>& args, const std::map<int,shared_ptr<BuildingEntity> >& map )
{
	...
	if( num_args != 11 ){ std::stringstream err; err << "Wrong parameter count for entity IfcStructuralLoadCase, expecting 11, having " << num_args << ". Entity ID: " << m_entity_id << std::endl; throw BuildingException( err.str() ); }
...
void IfcStructuralLoadCase::getAttributes( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes ) const
{
	...
	if( !m_SelfWeightCoefficients.empty() )
	{
		...
		vec_attributes.emplace_back( "SelfWeightCoefficients", SelfWeightCoefficients_vec_object );
	}
}
