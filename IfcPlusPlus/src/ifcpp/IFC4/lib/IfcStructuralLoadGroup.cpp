...
IfcStructuralLoadGroup::IfcStructuralLoadGroup() = default;
...
void IfcStructuralLoadGroup::readStepArguments( const std::vector<std::wstring>& args, const std::map<int,shared_ptr<BuildingEntity> >& map )
{
	...
	if( num_args != 10 ){ std::stringstream err; err << "Wrong parameter count for entity IfcStructuralLoadGroup, expecting 10, having " << num_args << ". Entity ID: " << m_entity_id << std::endl; throw BuildingException( err.str() ); }
...
void IfcStructuralLoadGroup::getAttributes( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes ) const
{
	...
	vec_attributes.emplace_back( "ActionType", m_ActionType );
	vec_attributes.emplace_back( "ActionSource", m_ActionSource );
	vec_attributes.emplace_back( "Coefficient", m_Coefficient );
...
void IfcStructuralLoadGroup::getAttributesInverse( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes_inverse ) const
{
	...
	if( !m_SourceOfResultGroup_inverse.empty() )
	{
		...
		for(const auto & i : m_SourceOfResultGroup_inverse)
		{
			if( !i.expired() )
			{
				SourceOfResultGroup_inverse_vec_obj->m_vec.push_back( shared_ptr<IfcStructuralResultGroup>( i ) );
			}
		}
		vec_attributes_inverse.emplace_back( "SourceOfResultGroup_inverse", SourceOfResultGroup_inverse_vec_obj );
	}
	if( !m_LoadGroupFor_inverse.empty() )
	{
		shared_ptr<AttributeObjectVector> LoadGroupFor_inverse_vec_obj( new AttributeObjectVector() );
		for(const auto & i : m_LoadGroupFor_inverse)
		{
			if( !i.expired() )
			{
				LoadGroupFor_inverse_vec_obj->m_vec.push_back( shared_ptr<IfcStructuralAnalysisModel>( i ) );
			}
		}
		vec_attributes_inverse.emplace_back( "LoadGroupFor_inverse", LoadGroupFor_inverse_vec_obj );
	}
}
