...
IfcStructuralMember::IfcStructuralMember() = default;
...
void IfcStructuralMember::readStepArguments( const std::vector<std::wstring>& args, const std::map<int,shared_ptr<BuildingEntity> >& map )
{
	...
	if( num_args != 7 ){ std::stringstream err; err << "Wrong parameter count for entity IfcStructuralMember, expecting 7, having " << num_args << ". Entity ID: " << m_entity_id << std::endl; throw BuildingException( err.str() ); }
...
void IfcStructuralMember::getAttributesInverse( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes_inverse ) const
{
	...
	if( !m_ConnectedBy_inverse.empty() )
	{
		...
		for(const auto & i : m_ConnectedBy_inverse)
		{
			if( !i.expired() )
			{
				ConnectedBy_inverse_vec_obj->m_vec.push_back( shared_ptr<IfcRelConnectsStructuralMember>( i ) );
			}
		}
		vec_attributes_inverse.emplace_back( "ConnectedBy_inverse", ConnectedBy_inverse_vec_obj );
	}
}
