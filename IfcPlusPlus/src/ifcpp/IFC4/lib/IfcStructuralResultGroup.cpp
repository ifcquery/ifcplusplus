...
IfcStructuralResultGroup::IfcStructuralResultGroup() = default;
...
void IfcStructuralResultGroup::readStepArguments( const std::vector<std::wstring>& args, const std::map<int,shared_ptr<BuildingEntity> >& map )
{
	...
	if( num_args != 8 ){ std::stringstream err; err << "Wrong parameter count for entity IfcStructuralResultGroup, expecting 8, having " << num_args << ". Entity ID: " << m_entity_id << std::endl; throw BuildingException( err.str() ); }
...
void IfcStructuralResultGroup::getAttributes( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes ) const
{
	...
	vec_attributes.emplace_back( "TheoryType", m_TheoryType );
	vec_attributes.emplace_back( "ResultForLoadGroup", m_ResultForLoadGroup );
	vec_attributes.emplace_back( "IsLinear", m_IsLinear );
}
void IfcStructuralResultGroup::getAttributesInverse( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes_inverse ) const
{
	...
	if( !m_ResultGroupFor_inverse.empty() )
	{
		...
		for(const auto & i : m_ResultGroupFor_inverse)
		{
			if( !i.expired() )
			{
				ResultGroupFor_inverse_vec_obj->m_vec.push_back( shared_ptr<IfcStructuralAnalysisModel>( i ) );
			}
		}
		vec_attributes_inverse.emplace_back( "ResultGroupFor_inverse", ResultGroupFor_inverse_vec_obj );
	}
}
