...
IfcSurfaceTexture::IfcSurfaceTexture() = default;
...
void IfcSurfaceTexture::readStepArguments( const std::vector<std::wstring>& args, const std::map<int,shared_ptr<BuildingEntity> >& map )
{
	...
	if( num_args != 5 ){ std::stringstream err; err << "Wrong parameter count for entity IfcSurfaceTexture, expecting 5, having " << num_args << ". Entity ID: " << m_entity_id << std::endl; throw BuildingException( err.str() ); }
...
void IfcSurfaceTexture::getAttributes( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes ) const
{
	...
	vec_attributes.emplace_back( "RepeatS", m_RepeatS );
	vec_attributes.emplace_back( "RepeatT", m_RepeatT );
	vec_attributes.emplace_back( "Mode", m_Mode );
	vec_attributes.emplace_back( "TextureTransform", m_TextureTransform );
	if( !m_Parameter.empty() )
	{
		...
		vec_attributes.emplace_back( "Parameter", Parameter_vec_object );
	}
}
void IfcSurfaceTexture::getAttributesInverse( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes_inverse ) const
{
	...
	if( !m_IsMappedBy_inverse.empty() )
	{
		...
		for(const auto & i : m_IsMappedBy_inverse)
		{
			if( !i.expired() )
			{
				IsMappedBy_inverse_vec_obj->m_vec.push_back( shared_ptr<IfcTextureCoordinate>( i ) );
			}
		}
		vec_attributes_inverse.emplace_back( "IsMappedBy_inverse", IsMappedBy_inverse_vec_obj );
	}
	if( !m_UsedInStyles_inverse.empty() )
	{
		shared_ptr<AttributeObjectVector> UsedInStyles_inverse_vec_obj( new AttributeObjectVector() );
		for(const auto & i : m_UsedInStyles_inverse)
		{
			if( !i.expired() )
			{
				UsedInStyles_inverse_vec_obj->m_vec.push_back( shared_ptr<IfcSurfaceStyleWithTextures>( i ) );
			}
		}
		vec_attributes_inverse.emplace_back( "UsedInStyles_inverse", UsedInStyles_inverse_vec_obj );
	}
}
