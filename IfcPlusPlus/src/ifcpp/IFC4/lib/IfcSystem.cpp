...
void IfcSystem::readStepArguments( const std::vector<std::wstring>& args, const std::map<int,shared_ptr<BuildingEntity> >& map )
{
	...
	if( num_args != 5 ){ std::stringstream err; err << "Wrong parameter count for entity IfcSystem, expecting 5, having " << num_args << ". Entity ID: " << m_entity_id << std::endl; throw BuildingException( err.str() ); }
...
void IfcSystem::getAttributesInverse( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes_inverse ) const
{
	...
	if( !m_ServicesBuildings_inverse.empty() )
	{
		...
		for(const auto & i : m_ServicesBuildings_inverse)
		{
			if( !i.expired() )
			{
				ServicesBuildings_inverse_vec_obj->m_vec.push_back( shared_ptr<IfcRelServicesBuildings>( i ) );
			}
		}
		vec_attributes_inverse.emplace_back( "ServicesBuildings_inverse", ServicesBuildings_inverse_vec_obj );
	}
}
