...
void IfcTask::readStepArguments( const std::vector<std::wstring>& args, const std::map<int,shared_ptr<BuildingEntity> >& map )
{
	...
	if( num_args != 13 ){ std::stringstream err; err << "Wrong parameter count for entity IfcTask, expecting 13, having " << num_args << ". Entity ID: " << m_entity_id << std::endl; throw BuildingException( err.str() ); }
...
void IfcTask::getAttributes( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes ) const
{
	...
	vec_attributes.emplace_back( "WorkMethod", m_WorkMethod );
	vec_attributes.emplace_back( "IsMilestone", m_IsMilestone );
	vec_attributes.emplace_back( "Priority", m_Priority );
	vec_attributes.emplace_back( "TaskTime", m_TaskTime );
