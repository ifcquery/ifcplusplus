...
void IfcTendon::readStepArguments( const std::vector<std::wstring>& args, const std::map<int,shared_ptr<BuildingEntity> >& map )
{
	...
	if( num_args != 17 ){ std::stringstream err; err << "Wrong parameter count for entity IfcTendon, expecting 17, having " << num_args << ". Entity ID: " << m_entity_id << std::endl; throw BuildingException( err.str() ); }
...
void IfcTendon::getAttributes( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes ) const
{
	...
	vec_attributes.emplace_back( "TensionForce", m_TensionForce );
	vec_attributes.emplace_back( "PreStress", m_PreStress );
	vec_attributes.emplace_back( "FrictionCoefficient", m_FrictionCoefficient );
	vec_attributes.emplace_back( "AnchorageSlip", m_AnchorageSlip );
	vec_attributes.emplace_back( "MinCurvatureRadius", m_MinCurvatureRadius );
}
