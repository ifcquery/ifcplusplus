...
IfcTendonConduit::IfcTendonConduit( int id ) { m_entity_id = id; }
shared_ptr<BuildingObject> IfcTendonConduit::getDeepCopy( BuildingCopyOptions& options )
{
	shared_ptr<IfcTendonConduit> copy_self( new IfcTendonConduit() );
	if( m_GlobalId )
	{
...
void IfcTendonConduit::getStepLine( std::stringstream& stream ) const
{
	stream << "#" << m_entity_id << "= IFCTENDONCONDUIT" << "(";
...
void IfcTendonConduit::getStepParameter( std::stringstream& stream, bool /*is_select_type*/ ) const { stream << "#" << m_entity_id; }
const std::wstring IfcTendonConduit::toString() const { return L"IfcTendonConduit"; }
void IfcTendonConduit::readStepArguments( const std::vector<std::wstring>& args, const std::map<int,shared_ptr<BuildingEntity> >& map )
{
	const size_t num_args = args.size();
	if( num_args != 10 ){ std::stringstream err; err << "Wrong parameter count for entity IfcTendonConduit, expecting 10, having " << num_args << ". Entity ID: " << m_entity_id << std::endl; throw BuildingException( err.str().c_str() ); }
...
void IfcTendonConduit::getAttributes( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes ) const
{
	IfcReinforcingElement::getAttributes( vec_attributes );
	vec_attributes.emplace_back( std::make_pair( "PredefinedType", m_PredefinedType ) );
}
void IfcTendonConduit::getAttributesInverse( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes_inverse ) const
{
	IfcReinforcingElement::getAttributesInverse( vec_attributes_inverse );
}
void IfcTendonConduit::setInverseCounterparts( shared_ptr<BuildingEntity> ptr_self_entity )
{
	IfcReinforcingElement::setInverseCounterparts( ptr_self_entity );
}
void IfcTendonConduit::unlinkFromInverseCounterparts()
{
