...
IfcTendonType::IfcTendonType() = default;
...
void IfcTendonType::readStepArguments( const std::vector<std::wstring>& args, const std::map<int,shared_ptr<BuildingEntity> >& map )
{
	...
	if( num_args != 13 ){ std::stringstream err; err << "Wrong parameter count for entity IfcTendonType, expecting 13, having " << num_args << ". Entity ID: " << m_entity_id << std::endl; throw BuildingException( err.str() ); }
...
void IfcTendonType::getAttributes( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes ) const
{
	...
	vec_attributes.emplace_back( "SheathDiameter", m_SheathDiameter );
}
