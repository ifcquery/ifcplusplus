...
IfcTimePeriod::IfcTimePeriod() = default;
...
void IfcTimePeriod::readStepArguments( const std::vector<std::wstring>& args, const std::map<int,shared_ptr<BuildingEntity> >& map )
{
	...
	if( num_args != 2 ){ std::stringstream err; err << "Wrong parameter count for entity IfcTimePeriod, expecting 2, having " << num_args << ". Entity ID: " << m_entity_id << std::endl; throw BuildingException( err.str() ); }
...
void IfcTimePeriod::setInverseCounterparts( shared_ptr<BuildingEntity>  /*ptr_self*/)
{
}
