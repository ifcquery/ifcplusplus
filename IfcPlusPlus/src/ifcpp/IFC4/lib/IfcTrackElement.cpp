...
IfcTrackElement::IfcTrackElement( int id ) { m_entity_id = id; }
shared_ptr<BuildingObject> IfcTrackElement::getDeepCopy( BuildingCopyOptions& options )
{
	shared_ptr<IfcTrackElement> copy_self( new IfcTrackElement() );
	if( m_GlobalId )
	{
...
void IfcTrackElement::getStepLine( std::stringstream& stream ) const
{
	stream << "#" << m_entity_id << "= IFCTRACKELEMENT" << "(";
...
void IfcTrackElement::getStepParameter( std::stringstream& stream, bool /*is_select_type*/ ) const { stream << "#" << m_entity_id; }
const std::wstring IfcTrackElement::toString() const { return L"IfcTrackElement"; }
void IfcTrackElement::readStepArguments( const std::vector<std::wstring>& args, const std::map<int,shared_ptr<BuildingEntity> >& map )
{
	const size_t num_args = args.size();
	if( num_args != 9 ){ std::stringstream err; err << "Wrong parameter count for entity IfcTrackElement, expecting 9, having " << num_args << ". Entity ID: " << m_entity_id << std::endl; throw BuildingException( err.str().c_str() ); }
	...
	m_PredefinedType = IfcTrackElementTypeEnum::createObjectFromSTEP( args[8], map );
}
void IfcTrackElement::getAttributes( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes ) const
{
	IfcBuiltElement::getAttributes( vec_attributes );
	vec_attributes.emplace_back( std::make_pair( "PredefinedType", m_PredefinedType ) );
}
void IfcTrackElement::getAttributesInverse( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes_inverse ) const
{
	IfcBuiltElement::getAttributesInverse( vec_attributes_inverse );
}
void IfcTrackElement::setInverseCounterparts( shared_ptr<BuildingEntity> ptr_self_entity )
{
	IfcBuiltElement::setInverseCounterparts( ptr_self_entity );
}
void IfcTrackElement::unlinkFromInverseCounterparts()
{
