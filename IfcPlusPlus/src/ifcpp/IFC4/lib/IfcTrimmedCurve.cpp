...
IfcTrimmedCurve::IfcTrimmedCurve() = default;
...
void IfcTrimmedCurve::readStepArguments( const std::vector<std::wstring>& args, const std::map<int,shared_ptr<BuildingEntity> >& map )
{
	...
	if( num_args != 5 ){ std::stringstream err; err << "Wrong parameter count for entity IfcTrimmedCurve, expecting 5, having " << num_args << ". Entity ID: " << m_entity_id << std::endl; throw BuildingException( err.str() ); }
...
void IfcTrimmedCurve::getAttributes( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes ) const
{
	...
	if( !m_Trim1.empty() )
	{
		...
		vec_attributes.emplace_back( "Trim1", Trim1_vec_object );
	}
	if( !m_Trim2.empty() )
	{
		...
		vec_attributes.emplace_back( "Trim2", Trim2_vec_object );
	}
	vec_attributes.emplace_back( "SenseAgreement", m_SenseAgreement );
...
void IfcTrimmedCurve::setInverseCounterparts( shared_ptr<BuildingEntity> ptr_self_entity )
{
