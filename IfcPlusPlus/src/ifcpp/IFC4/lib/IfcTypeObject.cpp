...
IfcTypeObject::IfcTypeObject() = default;
...
void IfcTypeObject::readStepArguments( const std::vector<std::wstring>& args, const std::map<int,shared_ptr<BuildingEntity> >& map )
{
	...
	if( num_args != 6 ){ std::stringstream err; err << "Wrong parameter count for entity IfcTypeObject, expecting 6, having " << num_args << ". Entity ID: " << m_entity_id << std::endl; throw BuildingException( err.str() ); }
...
void IfcTypeObject::getAttributes( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes ) const
{
	...
	vec_attributes.emplace_back( "ApplicableOccurrence", m_ApplicableOccurrence );
	if( !m_HasPropertySets.empty() )
	{
		...
		vec_attributes.emplace_back( "HasPropertySets", HasPropertySets_vec_object );
	}
}
void IfcTypeObject::getAttributesInverse( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes_inverse ) const
{
	...
	if( !m_Types_inverse.empty() )
	{
		...
		for(const auto & i : m_Types_inverse)
		{
			if( !i.expired() )
			{
				Types_inverse_vec_obj->m_vec.push_back( shared_ptr<IfcRelDefinesByType>( i ) );
			}
		}
		vec_attributes_inverse.emplace_back( "Types_inverse", Types_inverse_vec_obj );
	}
}
void IfcTypeObject::setInverseCounterparts( shared_ptr<BuildingEntity> ptr_self_entity )
{
	...
	for(auto & m_HasPropertySet : m_HasPropertySets)
	{
		if( m_HasPropertySet )
		{
			m_HasPropertySet->m_DefinesType_inverse.push_back( ptr_self );
		}
	}
}
void IfcTypeObject::unlinkFromInverseCounterparts()
{
	...
	for(auto & m_HasPropertySet : m_HasPropertySets)
	{
		if( m_HasPropertySet )
		{
			std::vector<weak_ptr<IfcTypeObject> >& DefinesType_inverse = m_HasPropertySet->m_DefinesType_inverse;
