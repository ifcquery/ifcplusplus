...
IfcTypeProduct::IfcTypeProduct() = default;
...
void IfcTypeProduct::readStepArguments( const std::vector<std::wstring>& args, const std::map<int,shared_ptr<BuildingEntity> >& map )
{
	...
	if( num_args != 8 ){ std::stringstream err; err << "Wrong parameter count for entity IfcTypeProduct, expecting 8, having " << num_args << ". Entity ID: " << m_entity_id << std::endl; throw BuildingException( err.str() ); }
...
void IfcTypeProduct::getAttributes( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes ) const
{
	...
	if( !m_RepresentationMaps.empty() )
	{
		...
		vec_attributes.emplace_back( "RepresentationMaps", RepresentationMaps_vec_object );
	}
