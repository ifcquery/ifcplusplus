...
IfcTypeResource::IfcTypeResource() = default;
...
void IfcTypeResource::readStepArguments( const std::vector<std::wstring>& args, const std::map<int,shared_ptr<BuildingEntity> >& map )
{
	...
	if( num_args != 9 ){ std::stringstream err; err << "Wrong parameter count for entity IfcTypeResource, expecting 9, having " << num_args << ". Entity ID: " << m_entity_id << std::endl; throw BuildingException( err.str() ); }
...
void IfcTypeResource::getAttributes( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes ) const
{
	...
	vec_attributes.emplace_back( "ResourceType", m_ResourceType );
}
void IfcTypeResource::getAttributesInverse( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes_inverse ) const
{
	...
	if( !m_ResourceOf_inverse.empty() )
	{
		...
		for(const auto & i : m_ResourceOf_inverse)
		{
			if( !i.expired() )
			{
				ResourceOf_inverse_vec_obj->m_vec.push_back( shared_ptr<IfcRelAssignsToResource>( i ) );
			}
		}
		vec_attributes_inverse.emplace_back( "ResourceOf_inverse", ResourceOf_inverse_vec_obj );
	}
}
