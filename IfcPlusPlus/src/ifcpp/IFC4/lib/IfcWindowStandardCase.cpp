...
IfcWindowStandardCase::IfcWindowStandardCase() = default;
...
void IfcWindowStandardCase::readStepArguments( const std::vector<std::wstring>& args, const std::map<int,shared_ptr<BuildingEntity> >& map )
{
	...
	if( num_args != 13 ){ std::stringstream err; err << "Wrong parameter count for entity IfcWindowStandardCase, expecting 13, having " << num_args << ". Entity ID: " << m_entity_id << std::endl; throw BuildingException( err.str() ); }
