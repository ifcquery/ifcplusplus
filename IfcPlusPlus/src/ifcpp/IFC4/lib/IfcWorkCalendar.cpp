...
IfcWorkCalendar::IfcWorkCalendar() = default;
...
shared_ptr<BuildingObject> IfcWorkCalendar::getDeepCopy( BuildingCopyOptions& options )
{
	...
	for(auto item_ii : m_WorkingTimes)
	{
			if( item_ii )
		{
	...
	for(auto item_ii : m_ExceptionTimes)
	{
			if( item_ii )
		{
...
void IfcWorkCalendar::readStepArguments( const std::vector<std::wstring>& args, const std::map<int,shared_ptr<BuildingEntity> >& map )
{
	...
	if( num_args != 9 ){ std::stringstream err; err << "Wrong parameter count for entity IfcWorkCalendar, expecting 9, having " << num_args << ". Entity ID: " << m_entity_id << std::endl; throw BuildingException( err.str() ); }
...
void IfcWorkCalendar::getAttributes( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes ) const
{
	...
	if( !m_WorkingTimes.empty() )
	{
		...
		vec_attributes.emplace_back( "WorkingTimes", WorkingTimes_vec_object );
	}
	if( !m_ExceptionTimes.empty() )
	{
		...
		vec_attributes.emplace_back( "ExceptionTimes", ExceptionTimes_vec_object );
	}
