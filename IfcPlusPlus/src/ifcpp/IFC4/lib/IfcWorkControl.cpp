...
IfcWorkControl::IfcWorkControl() = default;
...
void IfcWorkControl::readStepArguments( const std::vector<std::wstring>& args, const std::map<int,shared_ptr<BuildingEntity> >& map )
{
	...
	if( num_args != 13 ){ std::stringstream err; err << "Wrong parameter count for entity IfcWorkControl, expecting 13, having " << num_args << ". Entity ID: " << m_entity_id << std::endl; throw BuildingException( err.str() ); }
...
void IfcWorkControl::getAttributes( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes ) const
{
	...
	if( !m_Creators.empty() )
	{
		...
		vec_attributes.emplace_back( "Creators", Creators_vec_object );
	}
	vec_attributes.emplace_back( "Purpose", m_Purpose );
	vec_attributes.emplace_back( "Duration", m_Duration );
	vec_attributes.emplace_back( "TotalFloat", m_TotalFloat );
	vec_attributes.emplace_back( "StartTime", m_StartTime );
	vec_attributes.emplace_back( "FinishTime", m_FinishTime );
}
