...
void IFC4X3::IfcBuilding::getStepLine( std::stringstream& stream, size_t precision ) const
{
	...
	stream << ",";
	if( m_ElevationOfRefHeight ) { m_ElevationOfRefHeight->getStepParameter( stream, false, precision ); } else { stream << "$"; }
	stream << ",";
	if( m_ElevationOfTerrain ) { m_ElevationOfTerrain->getStepParameter( stream, false, precision ); } else { stream << "$"; }
	stream << ",";
	if( m_BuildingAddress ) { stream << "#" << m_BuildingAddress->m_tag; } else { stream << "$"; }
	stream << ");";
}
void IFC4X3::IfcBuilding::getStepParameter( std::stringstream& stream, bool /*is_select_type*/, size_t /*precision*/ ) const { stream << "#" << m_tag; }
void IFC4X3::IfcBuilding::readStepArguments( const std::vector<std::string>& args, const BuildingModelMapType<int,shared_ptr<BuildingEntity> >& map, std::stringstream& errorStream, std::unordered_set<int>& entityIdNotFound )
{
	...
	if( num_args > 9 ){m_ElevationOfRefHeight = IfcLengthMeasure::createObjectFromSTEP( args[9], map, errorStream, entityIdNotFound );}
	if( num_args > 10 ){m_ElevationOfTerrain = IfcLengthMeasure::createObjectFromSTEP( args[10], map, errorStream, entityIdNotFound );}
	if( num_args > 11 ){readEntityReference( args[11], m_BuildingAddress, map, errorStream, entityIdNotFound );}
	if( num_args != 12 ){ errorStream << "Wrong parameter count for entity IfcBuilding, expecting 12, having " << num_args << ". Entity ID: " << m_tag << std::endl; }
}
