...
#include "ifcpp/IFC4X3/include/IfcBuildingElementProxy.h"
...
void IFC4X3::IfcBuildingElementProxy::getStepLine( std::stringstream& stream, size_t precision ) const
{
...
void IFC4X3::IfcBuildingElementProxy::getStepParameter( std::stringstream& stream, bool /*is_select_type*/, size_t /*precision*/ ) const { stream << "#" << m_tag; }
void IFC4X3::IfcBuildingElementProxy::readStepArguments( const std::vector<std::string>& args, const BuildingModelMapType<int,shared_ptr<BuildingEntity> >& map, std::stringstream& errorStream, std::unordered_set<int>& entityIdNotFound )
{
	...
	if( num_args > 8 ){m_PredefinedType = IfcBuildingElementProxyTypeEnum::createObjectFromSTEP( args[8], map, errorStream, entityIdNotFound );}
	if( num_args != 9 ){ errorStream << "Wrong parameter count for entity IfcBuildingElementProxy, expecting 9, having " << num_args << ". Entity ID: " << m_tag << std::endl; }
}
