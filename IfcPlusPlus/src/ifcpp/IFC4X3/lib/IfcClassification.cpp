...
void IFC4X3::IfcClassification::readStepArguments( const std::vector<std::string>& args, const BuildingModelMapType<int,shared_ptr<BuildingEntity> >& map, std::stringstream& errorStream, std::unordered_set<int>& entityIdNotFound )
{
	const size_t num_args = args.size();
	if( num_args > 0 ){m_Source = IfcLabel::createObjectFromSTEP( args[0], map, errorStream, entityIdNotFound );}
	if( num_args > 1 ){m_Edition = IfcLabel::createObjectFromSTEP( args[1], map, errorStream, entityIdNotFound );}
	if( num_args > 2 ){m_EditionDate = IfcDate::createObjectFromSTEP( args[2], map, errorStream, entityIdNotFound );}
	...
	if( num_args > 5 ){m_Specification = IfcURIReference::createObjectFromSTEP( args[5], map, errorStream, entityIdNotFound );}
