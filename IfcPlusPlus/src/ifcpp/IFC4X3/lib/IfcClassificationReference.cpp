...
void IFC4X3::IfcClassificationReference::readStepArguments( const std::vector<std::string>& args, const BuildingModelMapType<int,shared_ptr<BuildingEntity> >& map, std::stringstream& errorStream, std::unordered_set<int>& entityIdNotFound )
{
	...
	if( num_args > 3 ){m_ReferencedSource = IfcClassificationReferenceSelect::createObjectFromSTEP( args[3], map, errorStream, entityIdNotFound );}
	if( num_args > 4 ){m_Description = IfcText::createObjectFromSTEP( args[4], map, errorStream, entityIdNotFound );}
	if( num_args > 5 ){m_Sort = IfcIdentifier::createObjectFromSTEP( args[5], map, errorStream, entityIdNotFound );}
