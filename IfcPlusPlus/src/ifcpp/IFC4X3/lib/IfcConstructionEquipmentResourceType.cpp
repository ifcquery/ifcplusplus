...
#include "ifcpp/IFC4X3/include/IfcConstructionEquipmentResourceType.h"
...
void IFC4X3::IfcConstructionEquipmentResourceType::getStepLine( std::stringstream& stream, size_t precision ) const
{
...
void IFC4X3::IfcConstructionEquipmentResourceType::getStepParameter( std::stringstream& stream, bool /*is_select_type*/, size_t /*precision*/ ) const { stream << "#" << m_tag; }
void IFC4X3::IfcConstructionEquipmentResourceType::readStepArguments( const std::vector<std::string>& args, const BuildingModelMapType<int,shared_ptr<BuildingEntity> >& map, std::stringstream& errorStream, std::unordered_set<int>& entityIdNotFound )
{
	...
	if( num_args > 11 ){m_PredefinedType = IfcConstructionEquipmentResourceTypeEnum::createObjectFromSTEP( args[11], map, errorStream, entityIdNotFound );}
	if( num_args != 12 ){ errorStream << "Wrong parameter count for entity IfcConstructionEquipmentResourceType, expecting 12, having " << num_args << ". Entity ID: " << m_tag << std::endl; }
}
