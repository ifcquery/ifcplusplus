...
void IFC4X3::IfcCostItem::readStepArguments( const std::vector<std::string>& args, const BuildingModelMapType<int,shared_ptr<BuildingEntity> >& map, std::stringstream& errorStream, std::unordered_set<int>& entityIdNotFound )
{
	...
	if( num_args > 6 ){m_PredefinedType = IfcCostItemTypeEnum::createObjectFromSTEP( args[6], map, errorStream, entityIdNotFound );}
	if( num_args > 7 ){readEntityReferenceList( args[7], m_CostValues, map, errorStream, entityIdNotFound );}
	if( num_args > 8 ){readEntityReferenceList( args[8], m_CostQuantities, map, errorStream, entityIdNotFound );}
