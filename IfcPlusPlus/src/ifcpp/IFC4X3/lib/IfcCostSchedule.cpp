...
#include "ifcpp/IFC4X3/include/IfcCostSchedule.h"
...
void IFC4X3::IfcCostSchedule::getStepLine( std::stringstream& stream, size_t precision ) const
{
	...
	stream << ",";
	if( m_SubmittedOn ) { m_SubmittedOn->getStepParameter( stream, false, precision ); } else { stream << "$"; }
	stream << ",";
	if( m_UpdateDate ) { m_UpdateDate->getStepParameter( stream, false, precision ); } else { stream << "$"; }
	stream << ");";
}
void IFC4X3::IfcCostSchedule::getStepParameter( std::stringstream& stream, bool /*is_select_type*/, size_t /*precision*/ ) const { stream << "#" << m_tag; }
void IFC4X3::IfcCostSchedule::readStepArguments( const std::vector<std::string>& args, const BuildingModelMapType<int,shared_ptr<BuildingEntity> >& map, std::stringstream& errorStream, std::unordered_set<int>& entityIdNotFound )
{
	...
	if( num_args > 6 ){m_PredefinedType = IfcCostScheduleTypeEnum::createObjectFromSTEP( args[6], map, errorStream, entityIdNotFound );}
	if( num_args > 7 ){m_Status = IfcLabel::createObjectFromSTEP( args[7], map, errorStream, entityIdNotFound );}
	if( num_args > 8 ){m_SubmittedOn = IfcDateTime::createObjectFromSTEP( args[8], map, errorStream, entityIdNotFound );}
	if( num_args > 9 ){m_UpdateDate = IfcDateTime::createObjectFromSTEP( args[9], map, errorStream, entityIdNotFound );}
	if( num_args != 10 ){ errorStream << "Wrong parameter count for entity IfcCostSchedule, expecting 10, having " << num_args << ". Entity ID: " << m_tag << std::endl; }
}
