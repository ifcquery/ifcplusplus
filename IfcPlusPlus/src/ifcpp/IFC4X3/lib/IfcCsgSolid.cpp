...
void IFC4X3::IfcCsgSolid::readStepArguments( const std::vector<std::string>& args, const BuildingModelMapType<int,shared_ptr<BuildingEntity> >& map, std::stringstream& errorStream, std::unordered_set<int>& entityIdNotFound )
{
	const size_t num_args = args.size();
	if( num_args > 0 ){m_TreeRootExpression = IfcCsgSelect::createObjectFromSTEP( args[0], map, errorStream, entityIdNotFound );}
