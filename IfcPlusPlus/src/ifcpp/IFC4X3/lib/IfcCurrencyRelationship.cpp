...
void IFC4X3::IfcCurrencyRelationship::readStepArguments( const std::vector<std::string>& args, const BuildingModelMapType<int,shared_ptr<BuildingEntity> >& map, std::stringstream& errorStream, std::unordered_set<int>& entityIdNotFound )
{
	...
	if( num_args > 2 ){readEntityReference( args[2], m_RelatingMonetaryUnit, map, errorStream, entityIdNotFound );}
	if( num_args > 3 ){readEntityReference( args[3], m_RelatedMonetaryUnit, map, errorStream, entityIdNotFound );}
	if( num_args > 4 ){m_ExchangeRate = IfcPositiveRatioMeasure::createObjectFromSTEP( args[4], map, errorStream, entityIdNotFound );}
	if( num_args > 5 ){m_RateDateTime = IfcDateTime::createObjectFromSTEP( args[5], map, errorStream, entityIdNotFound );}
	if( num_args > 6 ){readEntityReference( args[6], m_RateSource, map, errorStream, entityIdNotFound );}
