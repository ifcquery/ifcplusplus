...
void IFC4X3::IfcDirectrixDerivedReferenceSweptAreaSolid::readStepArguments( const std::vector<std::string>& args, const BuildingModelMapType<int,shared_ptr<BuildingEntity> >& map, std::stringstream& errorStream, std::unordered_set<int>& entityIdNotFound )
{
	...
	if( num_args != 6 ){ errorStream << "Wrong parameter count for entity IfcDirectrixDerivedReferenceSweptAreaSolid, expecting 6, having " << num_args << ". Entity ID: " << m_tag << std::endl; }
}
