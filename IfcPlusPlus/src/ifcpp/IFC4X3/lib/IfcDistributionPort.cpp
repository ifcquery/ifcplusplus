...
void IFC4X3::IfcDistributionPort::readStepArguments( const std::vector<std::string>& args, const BuildingModelMapType<int,shared_ptr<BuildingEntity> >& map, std::stringstream& errorStream, std::unordered_set<int>& entityIdNotFound )
{
	...
	if( num_args > 7 ){m_FlowDirection = IfcFlowDirectionEnum::createObjectFromSTEP( args[7], map, errorStream, entityIdNotFound );}
	if( num_args > 8 ){m_PredefinedType = IfcDistributionPortTypeEnum::createObjectFromSTEP( args[8], map, errorStream, entityIdNotFound );}
	if( num_args > 9 ){m_SystemType = IfcDistributionSystemEnum::createObjectFromSTEP( args[9], map, errorStream, entityIdNotFound );}
