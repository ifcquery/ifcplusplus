...
void IFC4X3::IfcDoor::getStepLine( std::stringstream& stream, size_t precision ) const
{
	...
	stream << ",";
	if( m_OverallHeight ) { m_OverallHeight->getStepParameter( stream, false, precision ); } else { stream << "$"; }
	stream << ",";
	...
	stream << ",";
	if( m_UserDefinedOperationType ) { m_UserDefinedOperationType->getStepParameter( stream, false, precision ); } else { stream << "$"; }
	stream << ");";
}
void IFC4X3::IfcDoor::getStepParameter( std::stringstream& stream, bool /*is_select_type*/, size_t /*precision*/ ) const { stream << "#" << m_tag; }
void IFC4X3::IfcDoor::readStepArguments( const std::vector<std::string>& args, const BuildingModelMapType<int,shared_ptr<BuildingEntity> >& map, std::stringstream& errorStream, std::unordered_set<int>& entityIdNotFound )
{
	...
	if( num_args > 8 ){m_OverallHeight = IfcPositiveLengthMeasure::createObjectFromSTEP( args[8], map, errorStream, entityIdNotFound );}
	if( num_args > 9 ){m_OverallWidth = IfcPositiveLengthMeasure::createObjectFromSTEP( args[9], map, errorStream, entityIdNotFound );}
	if( num_args > 10 ){m_PredefinedType = IfcDoorTypeEnum::createObjectFromSTEP( args[10], map, errorStream, entityIdNotFound );}
	if( num_args > 11 ){m_OperationType = IfcDoorTypeOperationEnum::createObjectFromSTEP( args[11], map, errorStream, entityIdNotFound );}
	if( num_args > 12 ){m_UserDefinedOperationType = IfcLabel::createObjectFromSTEP( args[12], map, errorStream, entityIdNotFound );}
	if( num_args != 13 ){ errorStream << "Wrong parameter count for entity IfcDoor, expecting 13, having " << num_args << ". Entity ID: " << m_tag << std::endl; }
}
