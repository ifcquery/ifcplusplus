...
void IFC4X3::IfcDoorStyle::getStepLine( std::stringstream& stream, size_t precision ) const
{
...
void IFC4X3::IfcDoorStyle::getStepParameter( std::stringstream& stream, bool /*is_select_type*/, size_t /*precision*/ ) const { stream << "#" << m_tag; }
void IFC4X3::IfcDoorStyle::readStepArguments( const std::vector<std::string>& args, const BuildingModelMapType<int,shared_ptr<BuildingEntity> >& map, std::stringstream& errorStream, std::unordered_set<int>& entityIdNotFound )
{
	...
	if( num_args > 8 ){m_OperationType = IfcDoorStyleOperationEnum::createObjectFromSTEP( args[8], map, errorStream, entityIdNotFound );}
	if( num_args > 9 ){m_ConstructionType = IfcDoorStyleConstructionEnum::createObjectFromSTEP( args[9], map, errorStream, entityIdNotFound );}
	...
	if( num_args != 12 ){ errorStream << "Wrong parameter count for entity IfcDoorStyle, expecting 12, having " << num_args << ". Entity ID: " << m_tag << std::endl; }
}
