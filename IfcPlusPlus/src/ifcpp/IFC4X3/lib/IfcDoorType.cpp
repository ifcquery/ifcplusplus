...
#include "ifcpp/IFC4X3/include/IfcDoorType.h"
...
void IFC4X3::IfcDoorType::getStepLine( std::stringstream& stream, size_t precision ) const
{
...
void IFC4X3::IfcDoorType::getStepParameter( std::stringstream& stream, bool /*is_select_type*/, size_t /*precision*/ ) const { stream << "#" << m_tag; }
void IFC4X3::IfcDoorType::readStepArguments( const std::vector<std::string>& args, const BuildingModelMapType<int,shared_ptr<BuildingEntity> >& map, std::stringstream& errorStream, std::unordered_set<int>& entityIdNotFound )
{
	...
	if( num_args > 9 ){m_PredefinedType = IfcDoorTypeEnum::createObjectFromSTEP( args[9], map, errorStream, entityIdNotFound );}
	if( num_args > 10 ){m_OperationType = IfcDoorTypeOperationEnum::createObjectFromSTEP( args[10], map, errorStream, entityIdNotFound );}
	if( num_args > 11 ){m_ParameterTakesPrecedence = IfcBoolean::createObjectFromSTEP( args[11], map, errorStream, entityIdNotFound );}
	if( num_args > 12 ){m_UserDefinedOperationType = IfcLabel::createObjectFromSTEP( args[12], map, errorStream, entityIdNotFound );}
	if( num_args != 13 ){ errorStream << "Wrong parameter count for entity IfcDoorType, expecting 13, having " << num_args << ". Entity ID: " << m_tag << std::endl; }
}
