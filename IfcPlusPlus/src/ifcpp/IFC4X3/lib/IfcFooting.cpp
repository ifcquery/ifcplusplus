...
void IFC4X3::IfcFooting::getStepLine( std::stringstream& stream, size_t precision ) const
{
...
void IFC4X3::IfcFooting::getStepParameter( std::stringstream& stream, bool /*is_select_type*/, size_t /*precision*/ ) const { stream << "#" << m_tag; }
void IFC4X3::IfcFooting::readStepArguments( const std::vector<std::string>& args, const BuildingModelMapType<int,shared_ptr<BuildingEntity> >& map, std::stringstream& errorStream, std::unordered_set<int>& entityIdNotFound )
{
	...
	if( num_args > 8 ){m_PredefinedType = IfcFootingTypeEnum::createObjectFromSTEP( args[8], map, errorStream, entityIdNotFound );}
	if( num_args != 9 ){ errorStream << "Wrong parameter count for entity IfcFooting, expecting 9, having " << num_args << ". Entity ID: " << m_tag << std::endl; }
}
