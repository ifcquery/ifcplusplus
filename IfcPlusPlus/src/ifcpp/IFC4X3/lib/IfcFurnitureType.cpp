...
#include "ifcpp/IFC4X3/include/IfcFurnitureType.h"
...
void IFC4X3::IfcFurnitureType::getStepLine( std::stringstream& stream, size_t precision ) const
{
	...
	stream << ",";
	if( m_AssemblyPlace ) { m_AssemblyPlace->getStepParameter( stream, false, precision ); } else { stream << "$"; }
	stream << ",";
	if( m_PredefinedType ) { m_PredefinedType->getStepParameter( stream, false, precision ); } else { stream << "$"; }
	stream << ");";
}
void IFC4X3::IfcFurnitureType::getStepParameter( std::stringstream& stream, bool /*is_select_type*/, size_t /*precision*/ ) const { stream << "#" << m_tag; }
void IFC4X3::IfcFurnitureType::readStepArguments( const std::vector<std::string>& args, const BuildingModelMapType<int,shared_ptr<BuildingEntity> >& map, std::stringstream& errorStream, std::unordered_set<int>& entityIdNotFound )
{
	...
	if( num_args > 9 ){m_AssemblyPlace = IfcAssemblyPlaceEnum::createObjectFromSTEP( args[9], map, errorStream, entityIdNotFound );}
	if( num_args > 10 ){m_PredefinedType = IfcFurnitureTypeEnum::createObjectFromSTEP( args[10], map, errorStream, entityIdNotFound );}
	if( num_args != 11 ){ errorStream << "Wrong parameter count for entity IfcFurnitureType, expecting 11, having " << num_args << ". Entity ID: " << m_tag << std::endl; }
}
