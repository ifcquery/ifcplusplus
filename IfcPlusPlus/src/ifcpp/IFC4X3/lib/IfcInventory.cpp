...
void IFC4X3::IfcInventory::readStepArguments( const std::vector<std::string>& args, const BuildingModelMapType<int,shared_ptr<BuildingEntity> >& map, std::stringstream& errorStream, std::unordered_set<int>& entityIdNotFound )
{
	...
	if( num_args > 5 ){m_PredefinedType = IfcInventoryTypeEnum::createObjectFromSTEP( args[5], map, errorStream, entityIdNotFound );}
	if( num_args > 6 ){m_Jurisdiction = IfcActorSelect::createObjectFromSTEP( args[6], map, errorStream, entityIdNotFound );}
	if( num_args > 7 ){readEntityReferenceList( args[7], m_ResponsiblePersons, map, errorStream, entityIdNotFound );}
	if( num_args > 8 ){m_LastUpdateDate = IfcDate::createObjectFromSTEP( args[8], map, errorStream, entityIdNotFound );}
	if( num_args > 9 ){readEntityReference( args[9], m_CurrentValue, map, errorStream, entityIdNotFound );}
	if( num_args > 10 ){readEntityReference( args[10], m_OriginalValue, map, errorStream, entityIdNotFound );}
