...
void IFC4X3::IfcLightSourceSpot::readStepArguments( const std::vector<std::string>& args, const BuildingModelMapType<int,shared_ptr<BuildingEntity> >& map, std::stringstream& errorStream, std::unordered_set<int>& entityIdNotFound )
{
	...
	if( num_args > 9 ){readEntityReference( args[9], m_Orientation, map, errorStream, entityIdNotFound );}
	if( num_args > 10 ){m_ConcentrationExponent = IfcReal::createObjectFromSTEP( args[10], map, errorStream, entityIdNotFound );}
	if( num_args > 11 ){m_SpreadAngle = IfcPositivePlaneAngleMeasure::createObjectFromSTEP( args[11], map, errorStream, entityIdNotFound );}
	if( num_args > 12 ){m_BeamWidthAngle = IfcPositivePlaneAngleMeasure::createObjectFromSTEP( args[12], map, errorStream, entityIdNotFound );}
