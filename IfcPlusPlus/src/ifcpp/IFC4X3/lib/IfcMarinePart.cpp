...
void IFC4X3::IfcMarinePart::getStepLine( std::stringstream& stream, size_t precision ) const
{
...
void IFC4X3::IfcMarinePart::getStepParameter( std::stringstream& stream, bool /*is_select_type*/, size_t /*precision*/ ) const { stream << "#" << m_tag; }
void IFC4X3::IfcMarinePart::readStepArguments( const std::vector<std::string>& args, const BuildingModelMapType<int,shared_ptr<BuildingEntity> >& map, std::stringstream& errorStream, std::unordered_set<int>& entityIdNotFound )
{
	...
	if( num_args > 10 ){m_PredefinedType = IfcMarinePartTypeEnum::createObjectFromSTEP( args[10], map, errorStream, entityIdNotFound );}
	if( num_args != 11 ){ errorStream << "Wrong parameter count for entity IfcMarinePart, expecting 11, having " << num_args << ". Entity ID: " << m_tag << std::endl; }
}
