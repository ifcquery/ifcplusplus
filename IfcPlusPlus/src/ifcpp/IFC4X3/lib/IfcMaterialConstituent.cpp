...
void IFC4X3::IfcMaterialConstituent::readStepArguments( const std::vector<std::string>& args, const BuildingModelMapType<int,shared_ptr<BuildingEntity> >& map, std::stringstream& errorStream, std::unordered_set<int>& entityIdNotFound )
{
	...
	if( num_args > 3 ){m_Fraction = IfcNormalisedRatioMeasure::createObjectFromSTEP( args[3], map, errorStream, entityIdNotFound );}
	if( num_args > 4 ){m_Category = IfcLabel::createObjectFromSTEP( args[4], map, errorStream, entityIdNotFound );}
