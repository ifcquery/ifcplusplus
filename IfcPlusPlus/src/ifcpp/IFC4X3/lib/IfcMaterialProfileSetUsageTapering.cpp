...
void IFC4X3::IfcMaterialProfileSetUsageTapering::readStepArguments( const std::vector<std::string>& args, const BuildingModelMapType<int,shared_ptr<BuildingEntity> >& map, std::stringstream& errorStream, std::unordered_set<int>& entityIdNotFound )
{
	...
	if( num_args > 3 ){readEntityReference( args[3], m_ForProfileEndSet, map, errorStream, entityIdNotFound );}
	if( num_args > 4 ){m_CardinalEndPoint = IfcCardinalPointReference::createObjectFromSTEP( args[4], map, errorStream, entityIdNotFound );}
