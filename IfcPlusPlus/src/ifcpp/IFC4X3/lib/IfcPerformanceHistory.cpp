...
void IFC4X3::IfcPerformanceHistory::getStepLine( std::stringstream& stream, size_t precision ) const
{
	...
	stream << ",";
	if( m_LifeCyclePhase ) { m_LifeCyclePhase->getStepParameter( stream, false, precision ); } else { stream << "$"; }
	stream << ",";
	if( m_PredefinedType ) { m_PredefinedType->getStepParameter( stream, false, precision ); } else { stream << "$"; }
	stream << ");";
}
void IFC4X3::IfcPerformanceHistory::getStepParameter( std::stringstream& stream, bool /*is_select_type*/, size_t /*precision*/ ) const { stream << "#" << m_tag; }
void IFC4X3::IfcPerformanceHistory::readStepArguments( const std::vector<std::string>& args, const BuildingModelMapType<int,shared_ptr<BuildingEntity> >& map, std::stringstream& errorStream, std::unordered_set<int>& entityIdNotFound )
{
	...
	if( num_args > 6 ){m_LifeCyclePhase = IfcLabel::createObjectFromSTEP( args[6], map, errorStream, entityIdNotFound );}
	if( num_args > 7 ){m_PredefinedType = IfcPerformanceHistoryTypeEnum::createObjectFromSTEP( args[7], map, errorStream, entityIdNotFound );}
	if( num_args != 8 ){ errorStream << "Wrong parameter count for entity IfcPerformanceHistory, expecting 8, having " << num_args << ". Entity ID: " << m_tag << std::endl; }
}
