...
void IFC4X3::IfcPhysicalComplexQuantity::readStepArguments( const std::vector<std::string>& args, const BuildingModelMapType<int,shared_ptr<BuildingEntity> >& map, std::stringstream& errorStream, std::unordered_set<int>& entityIdNotFound )
{
	...
	if( num_args > 2 ){readEntityReferenceList( args[2], m_HasQuantities, map, errorStream, entityIdNotFound );}
	if( num_args > 3 ){m_Discrimination = IfcLabel::createObjectFromSTEP( args[3], map, errorStream, entityIdNotFound );}
	if( num_args > 4 ){m_Quality = IfcLabel::createObjectFromSTEP( args[4], map, errorStream, entityIdNotFound );}
	if( num_args > 5 ){m_Usage = IfcLabel::createObjectFromSTEP( args[5], map, errorStream, entityIdNotFound );}
