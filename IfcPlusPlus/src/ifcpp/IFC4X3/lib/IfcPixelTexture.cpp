...
void IFC4X3::IfcPixelTexture::readStepArguments( const std::vector<std::string>& args, const BuildingModelMapType<int,shared_ptr<BuildingEntity> >& map, std::stringstream& errorStream, std::unordered_set<int>& entityIdNotFound )
{
	...
	if( num_args > 5 ){m_Width = IfcInteger::createObjectFromSTEP( args[5], map, errorStream, entityIdNotFound );}
	if( num_args > 6 ){m_Height = IfcInteger::createObjectFromSTEP( args[6], map, errorStream, entityIdNotFound );}
	if( num_args > 7 ){m_ColourComponents = IfcInteger::createObjectFromSTEP( args[7], map, errorStream, entityIdNotFound );}
