...
void IFC4X3::IfcPort::getStepLine( std::stringstream& stream, size_t precision ) const
{
...
void IFC4X3::IfcPort::getStepParameter( std::stringstream& stream, bool /*is_select_type*/, size_t /*precision*/ ) const { stream << "#" << m_tag; }
void IFC4X3::IfcPort::readStepArguments( const std::vector<std::string>& args, const BuildingModelMapType<int,shared_ptr<BuildingEntity> >& map, std::stringstream& errorStream, std::unordered_set<int>& entityIdNotFound )
{
	...
	if( num_args != 7 ){ errorStream << "Wrong parameter count for entity IfcPort, expecting 7, having " << num_args << ". Entity ID: " << m_tag << std::endl; }
}
