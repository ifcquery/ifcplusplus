...
void IFC4X3::IfcPropertyBoundedValue::readStepArguments( const std::vector<std::string>& args, const BuildingModelMapType<int,shared_ptr<BuildingEntity> >& map, std::stringstream& errorStream, std::unordered_set<int>& entityIdNotFound )
{
	...
	if( num_args > 2 ){m_UpperBoundValue = IfcValue::createObjectFromSTEP( args[2], map, errorStream, entityIdNotFound );}
	if( num_args > 3 ){m_LowerBoundValue = IfcValue::createObjectFromSTEP( args[3], map, errorStream, entityIdNotFound );}
	if( num_args > 4 ){m_Unit = IfcUnit::createObjectFromSTEP( args[4], map, errorStream, entityIdNotFound );}
	if( num_args > 5 ){m_SetPointValue = IfcValue::createObjectFromSTEP( args[5], map, errorStream, entityIdNotFound );}
