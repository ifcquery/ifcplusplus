...
void IFC4X3::IfcPropertySetTemplate::readStepArguments( const std::vector<std::string>& args, const BuildingModelMapType<int,shared_ptr<BuildingEntity> >& map, std::stringstream& errorStream, std::unordered_set<int>& entityIdNotFound )
{
	...
	if( num_args > 4 ){m_TemplateType = IfcPropertySetTemplateTypeEnum::createObjectFromSTEP( args[4], map, errorStream, entityIdNotFound );}
	if( num_args > 5 ){m_ApplicableEntity = IfcIdentifier::createObjectFromSTEP( args[5], map, errorStream, entityIdNotFound );}
	if( num_args > 6 ){readEntityReferenceList( args[6], m_HasPropertyTemplates, map, errorStream, entityIdNotFound );}
