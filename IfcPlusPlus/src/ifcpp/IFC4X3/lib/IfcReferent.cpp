...
#include "ifcpp/IFC4X3/include/IfcReferent.h"
...
void IFC4X3::IfcReferent::getStepLine( std::stringstream& stream, size_t precision ) const
{
...
void IFC4X3::IfcReferent::getStepParameter( std::stringstream& stream, bool /*is_select_type*/, size_t /*precision*/ ) const { stream << "#" << m_tag; }
void IFC4X3::IfcReferent::readStepArguments( const std::vector<std::string>& args, const BuildingModelMapType<int,shared_ptr<BuildingEntity> >& map, std::stringstream& errorStream, std::unordered_set<int>& entityIdNotFound )
{
	...
	if( num_args > 7 ){m_PredefinedType = IfcReferentTypeEnum::createObjectFromSTEP( args[7], map, errorStream, entityIdNotFound );}
	if( num_args != 8 ){ errorStream << "Wrong parameter count for entity IfcReferent, expecting 8, having " << num_args << ". Entity ID: " << m_tag << std::endl; }
}
