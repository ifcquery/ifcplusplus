...
void IFC4X3::IfcRelAssignsToActor::readStepArguments( const std::vector<std::string>& args, const BuildingModelMapType<int,shared_ptr<BuildingEntity> >& map, std::stringstream& errorStream, std::unordered_set<int>& entityIdNotFound )
{
	...
	if( num_args > 6 ){readEntityReference( args[6], m_RelatingActor, map, errorStream, entityIdNotFound );}
	if( num_args > 7 ){readEntityReference( args[7], m_ActingRole, map, errorStream, entityIdNotFound );}
