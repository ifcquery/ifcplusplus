...
void IFC4X3::IfcRelAssignsToProcess::readStepArguments( const std::vector<std::string>& args, const BuildingModelMapType<int,shared_ptr<BuildingEntity> >& map, std::stringstream& errorStream, std::unordered_set<int>& entityIdNotFound )
{
	...
	if( num_args > 6 ){m_RelatingProcess = IfcProcessSelect::createObjectFromSTEP( args[6], map, errorStream, entityIdNotFound );}
	if( num_args > 7 ){readEntityReference( args[7], m_QuantityInProcess, map, errorStream, entityIdNotFound );}
