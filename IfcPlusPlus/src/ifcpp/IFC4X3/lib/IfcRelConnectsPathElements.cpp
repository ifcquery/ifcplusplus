...
void IFC4X3::IfcRelConnectsPathElements::readStepArguments( const std::vector<std::string>& args, const BuildingModelMapType<int,shared_ptr<BuildingEntity> >& map, std::stringstream& errorStream, std::unordered_set<int>& entityIdNotFound )
{
	...
	if( num_args > 9 ){m_RelatedConnectionType = IfcConnectionTypeEnum::createObjectFromSTEP( args[9], map, errorStream, entityIdNotFound );}
	if( num_args > 10 ){m_RelatingConnectionType = IfcConnectionTypeEnum::createObjectFromSTEP( args[10], map, errorStream, entityIdNotFound );}
