...
void IFC4X3::IfcRelConnectsStructuralActivity::readStepArguments( const std::vector<std::string>& args, const BuildingModelMapType<int,shared_ptr<BuildingEntity> >& map, std::stringstream& errorStream, std::unordered_set<int>& entityIdNotFound )
{
	...
	if( num_args > 4 ){m_RelatingElement = IfcStructuralActivityAssignmentSelect::createObjectFromSTEP( args[4], map, errorStream, entityIdNotFound );}
	if( num_args > 5 ){readEntityReference( args[5], m_RelatedStructuralActivity, map, errorStream, entityIdNotFound );}
