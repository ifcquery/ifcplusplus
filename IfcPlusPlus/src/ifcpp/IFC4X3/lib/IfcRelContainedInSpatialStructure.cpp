...
void IFC4X3::IfcRelContainedInSpatialStructure::readStepArguments( const std::vector<std::string>& args, const BuildingModelMapType<int,shared_ptr<BuildingEntity> >& map, std::stringstream& errorStream, std::unordered_set<int>& entityIdNotFound )
{
	...
	if( num_args > 4 ){readEntityReferenceList( args[4], m_RelatedElements, map, errorStream, entityIdNotFound );}
