...
void IFC4X3::IfcRelServicesBuildings::readStepArguments( const std::vector<std::string>& args, const BuildingModelMapType<int,shared_ptr<BuildingEntity> >& map, std::stringstream& errorStream, std::unordered_set<int>& entityIdNotFound )
{
	...
	if( num_args > 4 ){readEntityReference( args[4], m_RelatingSystem, map, errorStream, entityIdNotFound );}
	if( num_args > 5 ){readEntityReferenceList( args[5], m_RelatedBuildings, map, errorStream, entityIdNotFound );}
