...
void IFC4X3::IfcResourceTime::readStepArguments( const std::vector<std::string>& args, const BuildingModelMapType<int,shared_ptr<BuildingEntity> >& map, std::stringstream& errorStream, std::unordered_set<int>& entityIdNotFound )
{
	...
	if( num_args > 3 ){m_ScheduleWork = IfcDuration::createObjectFromSTEP( args[3], map, errorStream, entityIdNotFound );}
	if( num_args > 4 ){m_ScheduleUsage = IfcPositiveRatioMeasure::createObjectFromSTEP( args[4], map, errorStream, entityIdNotFound );}
	...
	if( num_args > 7 ){m_ScheduleContour = IfcLabel::createObjectFromSTEP( args[7], map, errorStream, entityIdNotFound );}
	if( num_args > 8 ){m_LevelingDelay = IfcDuration::createObjectFromSTEP( args[8], map, errorStream, entityIdNotFound );}
	if( num_args > 9 ){m_IsOverAllocated = IfcBoolean::createObjectFromSTEP( args[9], map, errorStream, entityIdNotFound );}
	if( num_args > 10 ){m_StatusTime = IfcDateTime::createObjectFromSTEP( args[10], map, errorStream, entityIdNotFound );}
	if( num_args > 11 ){m_ActualWork = IfcDuration::createObjectFromSTEP( args[11], map, errorStream, entityIdNotFound );}
	if( num_args > 12 ){m_ActualUsage = IfcPositiveRatioMeasure::createObjectFromSTEP( args[12], map, errorStream, entityIdNotFound );}
	if( num_args > 13 ){m_ActualStart = IfcDateTime::createObjectFromSTEP( args[13], map, errorStream, entityIdNotFound );}
	if( num_args > 14 ){m_ActualFinish = IfcDateTime::createObjectFromSTEP( args[14], map, errorStream, entityIdNotFound );}
	if( num_args > 15 ){m_RemainingWork = IfcDuration::createObjectFromSTEP( args[15], map, errorStream, entityIdNotFound );}
	if( num_args > 16 ){m_RemainingUsage = IfcPositiveRatioMeasure::createObjectFromSTEP( args[16], map, errorStream, entityIdNotFound );}
	if( num_args > 17 ){m_Completion = IfcPositiveRatioMeasure::createObjectFromSTEP( args[17], map, errorStream, entityIdNotFound );}
