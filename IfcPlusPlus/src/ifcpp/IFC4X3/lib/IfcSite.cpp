...
void IFC4X3::IfcSite::readStepArguments( const std::vector<std::string>& args, const BuildingModelMapType<int,shared_ptr<BuildingEntity> >& map, std::stringstream& errorStream, std::unordered_set<int>& entityIdNotFound )
{
	...
	if( num_args > 9 ){m_RefLatitude = IfcCompoundPlaneAngleMeasure::createObjectFromSTEP( args[9], map, errorStream, entityIdNotFound );}
	if( num_args > 10 ){m_RefLongitude = IfcCompoundPlaneAngleMeasure::createObjectFromSTEP( args[10], map, errorStream, entityIdNotFound );}
	if( num_args > 11 ){m_RefElevation = IfcLengthMeasure::createObjectFromSTEP( args[11], map, errorStream, entityIdNotFound );}
	if( num_args > 12 ){m_LandTitleNumber = IfcLabel::createObjectFromSTEP( args[12], map, errorStream, entityIdNotFound );}
	if( num_args > 13 ){readEntityReference( args[13], m_SiteAddress, map, errorStream, entityIdNotFound );}
	if( num_args != 14 ){ errorStream << "Wrong parameter count for entity IfcSite, expecting 14, having " << num_args << ". Entity ID: " << m_tag << std::endl; }
}
