...
#include "ifcpp/IFC4X3/include/IfcStairFlightType.h"
...
void IFC4X3::IfcStairFlightType::getStepLine( std::stringstream& stream, size_t precision ) const
{
...
void IFC4X3::IfcStairFlightType::getStepParameter( std::stringstream& stream, bool /*is_select_type*/, size_t /*precision*/ ) const { stream << "#" << m_tag; }
void IFC4X3::IfcStairFlightType::readStepArguments( const std::vector<std::string>& args, const BuildingModelMapType<int,shared_ptr<BuildingEntity> >& map, std::stringstream& errorStream, std::unordered_set<int>& entityIdNotFound )
{
	...
	if( num_args > 9 ){m_PredefinedType = IfcStairFlightTypeEnum::createObjectFromSTEP( args[9], map, errorStream, entityIdNotFound );}
	if( num_args != 10 ){ errorStream << "Wrong parameter count for entity IfcStairFlightType, expecting 10, having " << num_args << ". Entity ID: " << m_tag << std::endl; }
}
