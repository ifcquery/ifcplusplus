...
void IFC4X3::IfcStructuralCurveConnection::getStepLine( std::stringstream& stream, size_t precision ) const
{
...
void IFC4X3::IfcStructuralCurveConnection::getStepParameter( std::stringstream& stream, bool /*is_select_type*/, size_t /*precision*/ ) const { stream << "#" << m_tag; }
void IFC4X3::IfcStructuralCurveConnection::readStepArguments( const std::vector<std::string>& args, const std::map<int,shared_ptr<BuildingEntity> >& map, std::stringstream& errorStream )
{
	...
	if( num_args > 8 ){readEntityReference( args[8], m_AxisDirection, map, errorStream );}
	if( num_args != 9 ){ errorStream << "Wrong parameter count for entity IfcStructuralCurveConnection, expecting 9, having " << num_args << ". Entity ID: " << m_tag << std::endl; }
}
