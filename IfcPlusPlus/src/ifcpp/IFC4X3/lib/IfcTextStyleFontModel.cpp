...
void IFC4X3::IfcTextStyleFontModel::readStepArguments( const std::vector<std::string>& args, const BuildingModelMapType<int,shared_ptr<BuildingEntity> >& map, std::stringstream& errorStream, std::unordered_set<int>& entityIdNotFound )
{
	...
	if( num_args > 2 ){m_FontStyle = IfcFontStyle::createObjectFromSTEP( args[2], map, errorStream, entityIdNotFound );}
	if( num_args > 3 ){m_FontVariant = IfcFontVariant::createObjectFromSTEP( args[3], map, errorStream, entityIdNotFound );}
	if( num_args > 4 ){m_FontWeight = IfcFontWeight::createObjectFromSTEP( args[4], map, errorStream, entityIdNotFound );}
	if( num_args > 5 ){m_FontSize = IfcSizeSelect::createObjectFromSTEP( args[5], map, errorStream, entityIdNotFound );}
