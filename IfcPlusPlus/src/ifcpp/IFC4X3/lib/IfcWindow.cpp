...
void IFC4X3::IfcWindow::getStepLine( std::stringstream& stream, size_t precision ) const
{
...
void IFC4X3::IfcWindow::getStepParameter( std::stringstream& stream, bool /*is_select_type*/, size_t /*precision*/ ) const { stream << "#" << m_tag; }
void IFC4X3::IfcWindow::readStepArguments( const std::vector<std::string>& args, const BuildingModelMapType<int,shared_ptr<BuildingEntity> >& map, std::stringstream& errorStream, std::unordered_set<int>& entityIdNotFound )
{
	...
	if( num_args > 10 ){m_PredefinedType = IfcWindowTypeEnum::createObjectFromSTEP( args[10], map, errorStream, entityIdNotFound );}
	if( num_args > 11 ){m_PartitioningType = IfcWindowTypePartitioningEnum::createObjectFromSTEP( args[11], map, errorStream, entityIdNotFound );}
	if( num_args > 12 ){m_UserDefinedPartitioningType = IfcLabel::createObjectFromSTEP( args[12], map, errorStream, entityIdNotFound );}
	if( num_args != 13 ){ errorStream << "Wrong parameter count for entity IfcWindow, expecting 13, having " << num_args << ". Entity ID: " << m_tag << std::endl; }
}
