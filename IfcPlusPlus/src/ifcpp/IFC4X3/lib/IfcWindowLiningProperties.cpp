...
void IFC4X3::IfcWindowLiningProperties::readStepArguments( const std::vector<std::string>& args, const BuildingModelMapType<int,shared_ptr<BuildingEntity> >& map, std::stringstream& errorStream, std::unordered_set<int>& entityIdNotFound )
{
	...
	if( num_args > 6 ){m_TransomThickness = IfcNonNegativeLengthMeasure::createObjectFromSTEP( args[6], map, errorStream, entityIdNotFound );}
	if( num_args > 7 ){m_MullionThickness = IfcNonNegativeLengthMeasure::createObjectFromSTEP( args[7], map, errorStream, entityIdNotFound );}
	if( num_args > 8 ){m_FirstTransomOffset = IfcNormalisedRatioMeasure::createObjectFromSTEP( args[8], map, errorStream, entityIdNotFound );}
	if( num_args > 9 ){m_SecondTransomOffset = IfcNormalisedRatioMeasure::createObjectFromSTEP( args[9], map, errorStream, entityIdNotFound );}
	if( num_args > 10 ){m_FirstMullionOffset = IfcNormalisedRatioMeasure::createObjectFromSTEP( args[10], map, errorStream, entityIdNotFound );}
	if( num_args > 11 ){m_SecondMullionOffset = IfcNormalisedRatioMeasure::createObjectFromSTEP( args[11], map, errorStream, entityIdNotFound );}
	if( num_args > 12 ){readEntityReference( args[12], m_ShapeAspectStyle, map, errorStream, entityIdNotFound );}
	if( num_args > 13 ){m_LiningOffset = IfcLengthMeasure::createObjectFromSTEP( args[13], map, errorStream, entityIdNotFound );}
	if( num_args > 14 ){m_LiningToPanelOffsetX = IfcLengthMeasure::createObjectFromSTEP( args[14], map, errorStream, entityIdNotFound );}
	if( num_args > 15 ){m_LiningToPanelOffsetY = IfcLengthMeasure::createObjectFromSTEP( args[15], map, errorStream, entityIdNotFound );}
