...
void IFC4X3::IfcWindowStyle::getStepLine( std::stringstream& stream, size_t precision ) const
{
	...
	stream << ",";
	if( m_ConstructionType ) { m_ConstructionType->getStepParameter( stream, false, precision ); } else { stream << "$"; }
	stream << ",";
	...
	stream << ",";
	if( m_Sizeable ) { m_Sizeable->getStepParameter( stream, false, precision ); } else { stream << "$"; }
	stream << ");";
}
void IFC4X3::IfcWindowStyle::getStepParameter( std::stringstream& stream, bool /*is_select_type*/, size_t /*precision*/ ) const { stream << "#" << m_tag; }
void IFC4X3::IfcWindowStyle::readStepArguments( const std::vector<std::string>& args, const BuildingModelMapType<int,shared_ptr<BuildingEntity> >& map, std::stringstream& errorStream, std::unordered_set<int>& entityIdNotFound )
{
	...
	if( num_args > 8 ){m_ConstructionType = IfcWindowStyleConstructionEnum::createObjectFromSTEP( args[8], map, errorStream, entityIdNotFound );}
	if( num_args > 9 ){m_OperationType = IfcWindowStyleOperationEnum::createObjectFromSTEP( args[9], map, errorStream, entityIdNotFound );}
	if( num_args > 10 ){m_ParameterTakesPrecedence = IfcBoolean::createObjectFromSTEP( args[10], map, errorStream, entityIdNotFound );}
	if( num_args > 11 ){m_Sizeable = IfcBoolean::createObjectFromSTEP( args[11], map, errorStream, entityIdNotFound );}
	if( num_args != 12 ){ errorStream << "Wrong parameter count for entity IfcWindowStyle, expecting 12, having " << num_args << ". Entity ID: " << m_tag << std::endl; }
}
