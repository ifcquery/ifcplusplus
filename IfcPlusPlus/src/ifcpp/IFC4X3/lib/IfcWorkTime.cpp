...
void IFC4X3::IfcWorkTime::readStepArguments( const std::vector<std::string>& args, const BuildingModelMapType<int,shared_ptr<BuildingEntity> >& map, std::stringstream& errorStream, std::unordered_set<int>& entityIdNotFound )
{
	...
	if( num_args > 3 ){readEntityReference( args[3], m_RecurrencePattern, map, errorStream, entityIdNotFound );}
	if( num_args > 4 ){m_StartDate = IfcDate::createObjectFromSTEP( args[4], map, errorStream, entityIdNotFound );}
	if( num_args > 5 ){m_FinishDate = IfcDate::createObjectFromSTEP( args[5], map, errorStream, entityIdNotFound );}
