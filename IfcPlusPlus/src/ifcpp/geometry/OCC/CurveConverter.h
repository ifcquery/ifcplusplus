...
//\brief class to convert different kinds of IFC curve representations into OCC input geometry
class IFCPP_EXPORT CurveConverter : public StatusCallback
{
public:	
	...
	CurveConverter( shared_ptr<GeometrySettings>& gs, shared_ptr<UnitConverter>& uc, shared_ptr<SplineConverter>& sc )
	...
	void convertIfcCurve( const shared_ptr<IfcCurve>& ifc_curve, TopoDS_Wire& target_wire, std::vector<shared_ptr<IfcTrimmingSelect> >& trim1_vec, std::vector<shared_ptr<IfcTrimmingSelect> >& trim2_vec, bool sense_agreement ) const
	{
		...
		if( bounded_curve )
		{
			...
			if( composite_curve )
			{
				...
				for( size_t i_segments = 0; i_segments < vec_segements.size(); ++i_segments )
				{
					...
					{
						GeomUtils::appendWireToWire( segment_wire, target_wire );
					}
				}
				return;
			}
			...
			if( poly_line )
			{
				...
				if( points.size() > 0 )
				{
					PointConverter::convertIfcCartesianPointVector( points, target_wire, length_factor );
				}
				return;
			}
			...
			if( trimmed_curve )
			{
				...
				GeomUtils::appendWireToWire( basis_curve_wire, target_wire );
				return;
			}
		...
		if( conic )
		{
			...
			if( ifc_circle )
			{
				...
				if( trim_points.size() > 1 )
				{
					...
					GeomUtils::applyMatrixToShape( edge, conic_position_matrix );
					GeomUtils::appendEdgeToWire( edge, target_wire );
				}
				else
				{
		...
		if( line )
		{
			...
			if( PointConverter::convertIfcCartesianPoint( ifc_line_point, line_origin, length_factor ) )
			{
				...
				PointConverter::convertIfcDirection( ifc_line_direction, line_direction );
	...
	{
		double length_factor = m_unit_converter->getLengthInMeterFactor();
		PointConverter::convertIfcCartesianPointVector( poly_line->m_Points, loop, length_factor );
	}
	...
	void convertIfcLoop( const shared_ptr<IfcLoop>& loop, TopoDS_Wire& target_wire ) const
	{
		...
		if( poly_loop )
		{
			...
			double length_factor = m_unit_converter->getLengthInMeterFactor();
			PointConverter::convertIfcCartesianPointVectorSkipDuplicates( ifc_points, target_wire, true, length_factor );
			GeomUtils::closeWire( target_wire );

#ifdef _DEBUG
			if( !target_wire.Closed() )
			{
				GeomDebugUtils::dumpShape( target_wire, vec4( 0.5, 0.5, 0.5, 1.0 ), true, true );
			}
#endif
			return;
		}
