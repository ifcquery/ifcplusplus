...
public:
		...
		if( predefined_color )
		{
			...
			if( draughting_predefined_color )
			{
				if( draughting_predefined_color->m_Name )
				{
					...
					if( boost::iequals( predefined_name, L"black" ) )			color = vec4( 0.0f, 0.0f, 0.0f, 1.f );
					else if( boost::iequals( predefined_name, L"red" ) )		color = vec4( 1.0f, 0.0f, 0.0f, 1.f );
					else if( boost::iequals( predefined_name, L"green" ) )		color = vec4( 0.0f, 1.0f, 0.0f, 1.f );
					else if( boost::iequals( predefined_name, L"blue" ) )		color = vec4( 0.0f, 0.0f, 1.0f, 1.f );
					else if( boost::iequals( predefined_name, L"yellow" ) )		color = vec4( 1.0f, 1.0f, 0.0f, 1.f );
					else if( boost::iequals( predefined_name, L"magenta" ) )	color = vec4( 1.0f, 0.0f, 1.0f, 1.f );
					else if( boost::iequals( predefined_name, L"cyan" ) )		color = vec4( 0.0f, 1.0f, 1.0f, 1.f );
					else if( boost::iequals( predefined_name, L"white" ) )		color = vec4( 1.0f, 1.0f, 1.0f, 1.f );
				}
			}
			return;
		}
	...
	void convertIfcSurfaceStyle( shared_ptr<IfcSurfaceStyle> surface_style, shared_ptr<AppearanceData>& appearance_data )
	{
		...
		for( size_t ii_styles = 0; ii_styles < vec_styles.size(); ++ii_styles )
		{
			...
			{
				vec4 surface_color( 0.8, 0.82, 0.84, 1.f );
				...
				{
					surface_color = vec4( 0.1, 0.12, 0.15, surface_color.a() );
				}
				...
				float transparency = surface_color.a();
				...
				appearance_data->m_color_ambient = vec4( ambient_color.r()*0.8f, ambient_color.g()*0.8f, ambient_color.b()*0.8f, transparency );
				appearance_data->m_color_diffuse = vec4( diffuse_color.r(), diffuse_color.g(), diffuse_color.b(), transparency );
				appearance_data->m_color_specular = vec4( specular_color.r()*0.1, specular_color.g()*0.1, specular_color.b()*0.1, transparency );
	...
	void convertIfcCurveStyle( shared_ptr<IfcCurveStyle> curve_style, shared_ptr<AppearanceData>& appearance_data )
	{
		...
		if( curve_color )
		{
			...
			appearance_data->m_color_ambient = vec4( color.r()*0.8, color.g()*0.8, color.b()*0.8, color.a() );
			appearance_data->m_color_diffuse = vec4( color.r(), color.g(), color.b(), color.a() );
			appearance_data->m_color_specular = vec4( color.r()*0.1, color.g()*0.1, color.b()*0.1, color.a() );
