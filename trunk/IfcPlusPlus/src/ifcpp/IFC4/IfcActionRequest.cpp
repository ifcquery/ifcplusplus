...
IfcActionRequest::~IfcActionRequest() {}
shared_ptr<IfcPPObject> IfcActionRequest::getDeepCopy()
{
	...
	if( m_PredefinedType ) { copy_self->m_PredefinedType = dynamic_pointer_cast<IfcActionRequestTypeEnum>( m_PredefinedType->getDeepCopy() ); }
...
void IfcActionRequest::readStepArguments( const std::vector<std::wstring>& args, const std::map<int,shared_ptr<IfcPPEntity> >& map )
{
	...
	if( num_args<9 ){ std::stringstream strserr; strserr << "Wrong parameter count for entity IfcActionRequest, expecting 9, having " << num_args << ". Object id: " << getId() << std::endl; throw IfcPPException( strserr.str().c_str() ); }
	#ifdef _DEBUG
	...
	m_PredefinedType = IfcActionRequestTypeEnum::createObjectFromStepData( args[6] );
