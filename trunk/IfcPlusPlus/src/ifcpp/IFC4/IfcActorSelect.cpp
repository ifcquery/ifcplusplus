...
IfcActorSelect::~IfcActorSelect() {}
shared_ptr<IfcActorSelect> IfcActorSelect::createObjectFromStepData( const std::wstring& arg, const std::map<int,shared_ptr<IfcPPEntity> >& map )
{
	// Read SELECT TYPE
	...
	else
	{
		...
		if( result_object )
		{
			...
			shared_ptr<IfcActorSelect> result_ptr_self = dynamic_pointer_cast<IfcActorSelect>( result_ptr );
			if( result_ptr_self )
			{
