...
IfcBendingParameterSelect::~IfcBendingParameterSelect() {}
shared_ptr<IfcBendingParameterSelect> IfcBendingParameterSelect::createObjectFromStepData( const std::wstring& arg, const std::map<int,shared_ptr<IfcPPEntity> >& map )
{
	// Read SELECT TYPE
	...
	else
	{
		...
		strs << "unhandled inline argument: " << arg << " in function IfcBendingParameterSelect::readStepData" << std::endl;
		throw IfcPPException( strs.str() );
	}
	return shared_ptr<IfcBendingParameterSelect>();
}
