...
IfcBuildingElement::IfcBuildingElement( int id ) { m_id = id; }
IfcBuildingElement::~IfcBuildingElement() {}

// method setEntity takes over all attributes from another instance of the class
void IfcBuildingElement::setEntity( shared_ptr<IfcPPEntity> other_entity )
{
...
void IfcBuildingElement::readStepArguments( const std::vector<std::wstring>& args, const std::map<int,shared_ptr<IfcPPEntity> >& map )
{
	...
	if( num_args<8 ){ std::stringstream strserr; strserr << "Wrong parameter count for entity IfcBuildingElement, expecting 8, having " << num_args << ". Object id: " << getId() << std::endl; throw IfcPPException( strserr.str().c_str() ); }
	#ifdef _DEBUG
...
void IfcBuildingElement::getAttributesInverse( std::vector<std::pair<std::string, shared_ptr<IfcPPObject> > >& vec_attributes_inverse )
{
	...
	for( int i=0; i<m_HasCoverings_inverse.size(); ++i ) { HasCoverings_inverse_vec_obj->m_vec.push_back( shared_ptr<IfcRelCoversBldgElements>( m_HasCoverings_inverse[i] ) ); }
