...
IfcColour::IfcColour() {}
IfcColour::~IfcColour() {}
shared_ptr<IfcColour> IfcColour::createObjectFromStepData( const std::wstring& arg, const std::map<int,shared_ptr<IfcPPEntity> >& map )
{
	// Read SELECT TYPE
	...
	else
	{
		...
		strs << "unhandled inline argument: " << arg << " in function IfcColour::readStepData" << std::endl;
		throw IfcPPException( strs.str() );
	}
	return shared_ptr<IfcColour>();
}
