...
void IfcConstructionEquipmentResource::setEntity( shared_ptr<IfcPPEntity> other_entity )
{
...
void IfcConstructionEquipmentResource::readStepArguments( const std::vector<std::string>& args, const std::map<int,shared_ptr<IfcPPEntity> >& map )
{
	const int num_args = (int)args.size();
	if( num_args<11 ){ std::stringstream strserr; strserr << "Wrong parameter count for entity IfcConstructionEquipmentResource, expecting 11, having " << num_args << ". Object id: " << getId() << std::endl; throw IfcPPException( strserr.str().c_str() ); }
	#ifdef _DEBUG
	...
	m_PredefinedType = IfcConstructionEquipmentResourceTypeEnum::createObjectFromStepData( args[10] );
}
