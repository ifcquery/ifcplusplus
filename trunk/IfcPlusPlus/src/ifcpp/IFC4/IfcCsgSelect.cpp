...
IfcCsgSelect::~IfcCsgSelect() {}
shared_ptr<IfcCsgSelect> IfcCsgSelect::createObjectFromStepData( const std::wstring& arg, const std::map<int,shared_ptr<IfcPPEntity> >& map )
{
	// Read SELECT TYPE
	...
	else
	{
		...
		if( result_object )
		{
			...
			shared_ptr<IfcCsgSelect> result_ptr_self = dynamic_pointer_cast<IfcCsgSelect>( result_ptr );
			if( result_ptr_self )
			{
		...
		strs << "unhandled inline argument: " << arg << " in function IfcCsgSelect::readStepData" << std::endl;
		throw IfcPPException( strs.str() );
	}
	return shared_ptr<IfcCsgSelect>();
}
