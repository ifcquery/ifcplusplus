...
IfcDuctSilencerType::IfcDuctSilencerType( int id ) { m_id = id; }
IfcDuctSilencerType::~IfcDuctSilencerType() {}

// method setEntity takes over all attributes from another instance of the class
void IfcDuctSilencerType::setEntity( shared_ptr<IfcPPEntity> other_entity )
{
...
void IfcDuctSilencerType::readStepArguments( const std::vector<std::string>& args, const std::map<int,shared_ptr<IfcPPEntity> >& map )
{
	const int num_args = (int)args.size();
	if( num_args<10 ){ std::stringstream strserr; strserr << "Wrong parameter count for entity IfcDuctSilencerType, expecting 10, having " << num_args << ". Object id: " << getId() << std::endl; throw IfcPPException( strserr.str().c_str() ); }
	#ifdef _DEBUG
	...
	m_PredefinedType = IfcDuctSilencerTypeEnum::createObjectFromStepData( args[9] );
}
