...
IfcElementAssembly::~IfcElementAssembly() {}
shared_ptr<IfcPPObject> IfcElementAssembly::getDeepCopy()
{
	...
	if( m_PredefinedType ) { copy_self->m_PredefinedType = dynamic_pointer_cast<IfcElementAssemblyTypeEnum>( m_PredefinedType->getDeepCopy() ); }
	return copy_self;
}
...
void IfcElementAssembly::readStepArguments( const std::vector<std::wstring>& args, const std::map<int,shared_ptr<IfcPPEntity> >& map )
{
	...
	if( num_args<10 ){ std::stringstream strserr; strserr << "Wrong parameter count for entity IfcElementAssembly, expecting 10, having " << num_args << ". Object id: " << getId() << std::endl; throw IfcPPException( strserr.str().c_str() ); }
	#ifdef _DEBUG
