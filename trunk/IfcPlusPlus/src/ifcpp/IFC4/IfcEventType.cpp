...
void IfcEventType::setEntity( shared_ptr<IfcPPEntity> other_entity )
{
...
void IfcEventType::readStepArguments( const std::vector<std::string>& args, const std::map<int,shared_ptr<IfcPPEntity> >& map )
{
	const int num_args = (int)args.size();
	if( num_args<12 ){ std::stringstream strserr; strserr << "Wrong parameter count for entity IfcEventType, expecting 12, having " << num_args << ". Object id: " << getId() << std::endl; throw IfcPPException( strserr.str().c_str() ); }
	#ifdef _DEBUG
	...
	m_PredefinedType = IfcEventTypeEnum::createObjectFromStepData( args[9] );
	m_EventTriggerType = IfcEventTriggerTypeEnum::createObjectFromStepData( args[10] );
	m_UserDefinedEventTriggerType = IfcLabel::createObjectFromStepData( args[11] );
}
