...
IfcFurnitureType::IfcFurnitureType( int id ) { m_id = id; }
IfcFurnitureType::~IfcFurnitureType() {}
shared_ptr<IfcPPObject> IfcFurnitureType::getDeepCopy()
{
	...
	if( m_AssemblyPlace ) { copy_self->m_AssemblyPlace = dynamic_pointer_cast<IfcAssemblyPlaceEnum>( m_AssemblyPlace->getDeepCopy() ); }
	if( m_PredefinedType ) { copy_self->m_PredefinedType = dynamic_pointer_cast<IfcFurnitureTypeEnum>( m_PredefinedType->getDeepCopy() ); }
	return copy_self;
}
...
void IfcFurnitureType::readStepArguments( const std::vector<std::wstring>& args, const std::map<int,shared_ptr<IfcPPEntity> >& map )
{
	...
	if( num_args<11 ){ std::stringstream strserr; strserr << "Wrong parameter count for entity IfcFurnitureType, expecting 11, having " << num_args << ". Object id: " << getId() << std::endl; throw IfcPPException( strserr.str().c_str() ); }
	#ifdef _DEBUG
	...
	m_AssemblyPlace = IfcAssemblyPlaceEnum::createObjectFromStepData( args[9] );
	m_PredefinedType = IfcFurnitureTypeEnum::createObjectFromStepData( args[10] );
}
