...
IfcLayeredItem::~IfcLayeredItem() {}
shared_ptr<IfcLayeredItem> IfcLayeredItem::createObjectFromStepData( const std::wstring& arg, const std::map<int,shared_ptr<IfcPPEntity> >& map )
{
	// Read SELECT TYPE
	...
	else
	{
		...
		if( result_object )
		{
			...
			shared_ptr<IfcLayeredItem> result_ptr_self = dynamic_pointer_cast<IfcLayeredItem>( result_ptr );
			if( result_ptr_self )
			{
		...
		strs << "unhandled inline argument: " << arg << " in function IfcLayeredItem::readStepData" << std::endl;
		throw IfcPPException( strs.str() );
	}
	return shared_ptr<IfcLayeredItem>();
}
