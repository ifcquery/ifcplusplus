...
IfcObjectReferenceSelect::~IfcObjectReferenceSelect() {}
shared_ptr<IfcObjectReferenceSelect> IfcObjectReferenceSelect::createObjectFromStepData( const std::wstring& arg, const std::map<int,shared_ptr<IfcPPEntity> >& map )
{
	// Read SELECT TYPE
	...
	else
	{
		...
		if( result_object )
		{
			...
			shared_ptr<IfcObjectReferenceSelect> result_ptr_self = dynamic_pointer_cast<IfcObjectReferenceSelect>( result_ptr );
			if( result_ptr_self )
			{
		...
		strs << "unhandled inline argument: " << arg << " in function IfcObjectReferenceSelect::readStepData" << std::endl;
		throw IfcPPException( strs.str() );
	}
	return shared_ptr<IfcObjectReferenceSelect>();
}
