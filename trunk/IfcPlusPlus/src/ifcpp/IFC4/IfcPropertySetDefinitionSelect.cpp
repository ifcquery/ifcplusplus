...
IfcPropertySetDefinitionSelect::~IfcPropertySetDefinitionSelect() {}
shared_ptr<IfcPropertySetDefinitionSelect> IfcPropertySetDefinitionSelect::createObjectFromStepData( const std::wstring& arg, const std::map<int,shared_ptr<IfcPPEntity> >& map )
{
	// Read SELECT TYPE
	...
	else
	{
		...
		if( result_object )
		{
			...
			shared_ptr<IfcPropertySetDefinitionSelect> result_ptr_self = dynamic_pointer_cast<IfcPropertySetDefinitionSelect>( result_ptr );
			if( result_ptr_self )
			{
