...
// ENTITY IfcSystem 
IfcSystem::IfcSystem() {}
IfcSystem::IfcSystem( int id ) { m_id = id; }
IfcSystem::~IfcSystem() {}
...
void IfcSystem::readStepArguments( const std::vector<std::wstring>& args, const std::map<int,shared_ptr<IfcPPEntity> >& map )
{
	...
	if( num_args != 5 ){ std::stringstream strserr; strserr << "Wrong parameter count for entity IfcSystem, expecting 5, having " << num_args << ". Object id: " << m_id << std::endl; throw IfcPPException( strserr.str().c_str() ); }
