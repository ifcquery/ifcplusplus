...
IfcTransportElement::IfcTransportElement( int id ) { m_id = id; }
IfcTransportElement::~IfcTransportElement() {}

// method setEntity takes over all attributes from another instance of the class
void IfcTransportElement::setEntity( shared_ptr<IfcPPEntity> other_entity )
{
...
void IfcTransportElement::readStepArguments( const std::vector<std::string>& args, const std::map<int,shared_ptr<IfcPPEntity> >& map )
{
	const int num_args = (int)args.size();
	if( num_args<9 ){ std::stringstream strserr; strserr << "Wrong parameter count for entity IfcTransportElement, expecting 9, having " << num_args << ". Object id: " << getId() << std::endl; throw IfcPPException( strserr.str().c_str() ); }
	#ifdef _DEBUG
	...
	m_PredefinedType = IfcTransportElementTypeEnum::createObjectFromStepData( args[8] );
}
