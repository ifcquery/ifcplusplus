...
void computeKnotVector( const size_t num_ctrl_points, const size_t order, std::vector<double>& knot_vec )
{
...
void computRationalBasisFunctions(
	const size_t order,
	const double t,
...
SplineConverter::SplineConverter(shared_ptr<PointConverter>& pt_converter) : m_point_converter(pt_converter)
{
}
...
void SplineConverter::convertBSplineCurve( const shared_ptr<IfcBSplineCurve>& bspline_curve, std::vector<carve::geom::vector<3> >& target_vec, std::vector<carve::geom::vector<3> >& segment_start_points ) const
{
...
void SplineConverter::convertIfcBSplineSurface( const shared_ptr<IfcBSplineSurface>& ifc_bspline_surface, shared_ptr<carve::input::PolylineSetData>& polyline_data )
{
	// IfcBSplineSurface -----------------------------------------------------------
	// attributes:
